...
export async function POST(req: NextRequest) {
  try {
    ...
    if (String(parent_name).length > MAX_NAME || String(parent_email).length > MAX_EMAIL) {
      return NextResponse.json({ error: 'Field too long' }, { status: 400 })
    }
    if (String(message).length > MAX_MESSAGE) {
      return NextResponse.json({ error: 'Message too long (max 5000 chars)' }, { status: 400 })
    }
    if (child_age && String(child_age).length > MAX_SHORT) {
      return NextResponse.json({ error: 'Field too long' }, { status: 400 })
    }
    ...
    if (school.admin_email && process.env.RESEND_API_KEY) {
      ...
      const html = `
        <div style="font-family:-apple-system,sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#1B3252">
          ...
          <div style="font-size:13px;color:#6B7280;margin-bottom:28px">A parent has sent an enquiry about ${esc(school.name)}</div>
          ...
          <div style="background:#F6F8FA;border-radius:10px;padding:20px 22px;margin-bottom:24px;font-size:14px;line-height:1.7">
            ...
            ${details ? details.split('\n').map(d => `<strong>${esc(d.split(':')[0])}:</strong>${esc(d.split(':').slice(1).join(':'))}<br/>`).join('') : ''}
          </div>
      ...
      fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          subject: `New enquiry from ${String(parent_name).slice(0, 100)} — ${String(school.name).slice(0, 100)}`,
          html,
        }),
