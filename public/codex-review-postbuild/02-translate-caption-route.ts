...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (code !== 0) {
    // Translation failed — row is unchanged. Surface the script's stderr
    // (validation message, Claude error, etc.) so the UI can show it.
    const message = (stderr.split('\n').filter(Boolean).pop() || `Translator exited ${code}`).slice(0, 300)
    return NextResponse.json({
      error: message,
