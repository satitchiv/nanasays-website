...
import {
  rankCandidates,
  scoreForBuildMode,
  // 2026-05-24 Yoko slice — exported resolver helpers + NONNEG_FILTERS for direct unit testing
  resolveBoardingPref,
  resolveHomeRegion,
  resolveCurriculumPref,
  matchedNonnegFilters,
  NONNEG_FILTERS,
  // 2026-05-24 Slice A — curriculum row-time resolver
  matchesCurriculumPreference,
} from './score-for-build-mode.ts'
// Phase 2 (2026-07-06 scorer pool bugs) — graded-boarding classification
import {
...
test('gender filter drops boys-only school when child is a girl', () => {
  const out = rankCandidates(
    ...
    {
      parent: { home_region: 'anywhere', budget_range: '40k-50k' },
      child: {
...
test('boarding_pref=day drops known full-boarding schools', () => {
  ...
  // should not see it. budget_range + goal_orientation give both schools
  // independent positive signals so no-signal filter can't mask a
  // boarding-filter regression.
  const out = rankCandidates(
    [WYCOMBE_ABBEY, A_DAY_SCHOOL],
    structMap(STRUCT_WYCOMBE_ACADEMIC, STRUCT_DAY_EMPTY),
    {
      parent: { boarding_pref: 'day', home_region: 'anywhere', budget_range: '40k-50k' },
  ...
  assert.ok(slugs.includes('a-day-school'), 'A_DAY_SCHOOL must survive — confirms hard filter dropped Wycombe specifically, not the whole pool')
})
...
test('rationale_seed is non-empty and contains the school name when signals exist', () => {
  const out = rankCandidates(
    ...
    {
      // Bug #3 (2026-05-22) — was 'south-east'; Warwickshire is in midlands bucket
      // and would be dropped by the new region hard filter under south-east.
  ...
  const out = rankCandidates(
    ...
    structMap(STRUCT_LOADED),
    {
      ...
      child:  {
        ...
        interests_sports:  [{ sport: 'rugby', level: 'county' }],
        nonnegotiables:    ['full boarding school'],   // fires 'boarding-required' nonneg → effectiveBoardingPref='full'
      },
      excludeSlugs: [],
    },
    10,
...
test('Phase 4: "must be co-ed" drops boys-only and girls-only schools', () => {
  const out = rankCandidates(
    ...
    {
      parent: { home_region: 'anywhere', top_priority: 'academic', budget_range: '40k-50k' },
      child:  {
  ...
  assert.ok(slugs.includes('coeduc-day'),
...
test('Phase 4: "boys only" drops girls + co-ed schools (keeps boys)', () => {
  ...
  assert.ok(slugs.includes('sherborne-school'),
...
test('Phase 4: "not religious" drops schools with religious ethos_label', () => {
  ...
  assert.ok(slugs.includes('secular-coed'),
...
test('Phase 4: NULL gender_split passes through must-be-coed (no-penalty rule)', () => {
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('unknown-gender'),
...
// ── Known-faith curated fallback (2026-07-07 not-religious stopgap) ──
//
// 83/152 of the live pool has NO ethos facts, so the NULL-ethos
// pass-through above let famous faith schools reach "no faith schools"
// parents (Lily / Francis Holland P0, 2026-07-06 gate). The predicate
// now consults KNOWN_FAITH_SCHOOL_NAMES/SLUGS before the label.

const FAITH_NULL_ETHOS = {
  slug: 'francis-holland-school-uk',
  name: 'Francis Holland School',
  gender_split: 'Girls',
  fees_usd_min: 35000,
  fees_usd_max: 35000,
  sen_support: false,
  strengths: ['academic', 'arts'],
  confidence_score: 80,
  age_min: 11, age_max: 18,
  region: 'London',
}
const STRUCT_FAITH_NULL_ETHOS = {
  school_slug: 'francis-holland-school-uk',
  sports_profile: null, exam_results: null, university_destinations: null,
  student_community: null, isi_deep_facts: null,
  wellbeing_staffing: null, ethos_facts: null, subject_strengths: null,
}

const FAITH_SLUG_ONLY = {
  slug: 'kings-college-school-uk',
  name: "King's College School",     // normalizes to bare "kings" — slug hatch must catch it
  gender_split: 'Boys',
  fees_usd_min: 35000,
  fees_usd_max: 35000,
  sen_support: false,
...
const STRUCT_FAITH_SLUG_ONLY = {
  school_slug: 'kings-college-school-uk',
  sports_profile: null, exam_results: null, university_destinations: null,
  student_community: null, isi_deep_facts: null,
  wellbeing_staffing: null, ethos_facts: null, subject_strengths: null,
}

test('known-faith: "not religious" drops Francis Holland despite NULL ethos_label', () => {
  const out = rankCandidates(
    [FAITH_NULL_ETHOS, SECULAR_COED],
    structMap(STRUCT_FAITH_NULL_ETHOS, STRUCT_SECULAR_COED),
    {
      parent: { home_region: 'anywhere', top_priority: 'academic', budget_range: '40k-50k' },
      child:  {
        nonnegotiables: ['not religious'],
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('francis-holland-school-uk'),
    `curated faith school must drop even with no ethos facts. Got: ${JSON.stringify(slugs)}`)
  assert.ok(slugs.includes('secular-coed'),
    `secular school survives. Got: ${JSON.stringify(slugs)}`)
})

test('known-faith: slug escape hatch drops KCS (name normalizes to bare "kings")', () => {
  // Codex r1 P1 — 'no faith schools' is deliberately the ONLY entry
  // here: it must fire the not-religious filter by itself (the phrase
  // never matched the pattern before r1).
  const out = rankCandidates(
    [FAITH_SLUG_ONLY, SECULAR_COED],
    structMap(STRUCT_FAITH_SLUG_ONLY, STRUCT_SECULAR_COED),
    {
      parent: { home_region: 'anywhere', top_priority: 'academic', budget_range: '40k-50k' },
      child:  {
        nonnegotiables: ['no faith schools'],
        goal_orientation: 'university_track',
      },
      excludeSlugs: [],
    },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('kings-college-school-uk'),
    `KCS must drop via the slug hatch. Got: ${JSON.stringify(slugs)}`)
})

test('known-faith (Codex r1 P1): "no faith schools" phrasings fire the not-religious filter', () => {
  for (const phrase of ['no faith schools', 'no faith school', 'not a faith school', 'no faith-based schools']) {
    const fired = matchedNonnegFilters([phrase])
    assert.ok(fired.some(f => f.name === 'not-religious'),
      `"${phrase}" must fire not-religious. Fired: ${JSON.stringify(fired.map(f => f.name))}`)
  }
})

test('known-faith (Codex r1 P1): bare "no faith" without a school noun does NOT fire', () => {
  // Guards the false-positive: "no faith in league tables" is not a
  // religion nonneg.
  const fired = matchedNonnegFilters(['we have no faith in league tables'])
  assert.ok(!fired.some(f => f.name === 'not-religious'),
    `bare "no faith in X" must not fire. Fired: ${JSON.stringify(fired.map(f => f.name))}`)
})

test('known-faith: curated list beats a wrong "secular" extracted label', () => {
  const out = rankCandidates(
    [FAITH_NULL_ETHOS, SECULAR_COED],
    structMap(
      { ...STRUCT_FAITH_NULL_ETHOS, ethos_facts: { ethos_label: 'secular' } },
      STRUCT_SECULAR_COED,
    ),
    {
      parent: { home_region: 'anywhere', top_priority: 'academic', budget_range: '40k-50k' },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('francis-holland-school-uk'),
    `curated verdict must beat a mislabeled secular ethos. Got: ${JSON.stringify(slugs)}`)
})

test('known-faith: non-curated NULL-ethos school still passes (no-penalty rule intact)', () => {
  const out = rankCandidates(
    [COEDUC_DAY, FAITH_NULL_ETHOS],
    structMap(STRUCT_COEDUC_DAY, STRUCT_FAITH_NULL_ETHOS),
    {
      parent: { home_region: 'anywhere', top_priority: 'academic', budget_range: '40k-50k' },
      child:  {
        nonnegotiables: ['not religious'],
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('coeduc-day'),
    `non-curated NULL-ethos school keeps failing open. Got: ${JSON.stringify(slugs)}`)
})
...
test('Phase 4 item #3 (updated Phase 2 2026-07-06): boarding_pref_from_prose="full" boosts full-boarding schools via the GRADE-AWARE tilt', () => {
  // WYCOMBE_ABBEY is in KNOWN_FULL_BOARDING_NAMES with no boarding_grade in
  // the fixture (→ column 'unknown'). effectiveBoardingGrade floors the
  // name-list match up to 'offers-full', so the grade-aware tilt boosts it
  // and emits the HEDGED 'offers full boarding' signal — NOT the old bare
  // 'full boarding' chip, which over-claimed (Codex r1 P1/P2). prose 'full'
  // reaches the tilt via resolveBoardingPref → effectiveBoardingPref='full'.
  const out = rankCandidates(
    [WYCOMBE_ABBEY],
    structMap(STRUCT_WYCOMBE_ACADEMIC),
    {
  ...
  assert.ok(wa.signals.includes('offers full boarding'),
    `prose 'full' must trigger the hedged boarding signal. Got: ${JSON.stringify(wa.signals)}`)
  assert.ok(!wa.signals.includes('full boarding'),
    'the old bare "full boarding" over-claim chip must be gone')
})
...
test('Yoko slice (Codex r1 Q7): drill_focus WINS over wizard top_priority on conflict', () => {
  // Yoko slice 2026-05-24 (Codex r1 Q7) — precedence flipped from
  // wizard-wins to drill_focus-wins. drill_focus is child-specific
  // (read from THIS child's prose); wizard top_priority is parent-
  // account-shared. Reversing precedence avoids contaminating siblings
  // (e.g. Sam getting Rugby's sport-priority bonus because his parent's
  // wizard says 'sport' even though Sam's notes say academic+pastoral).
  // Wycombe has academic signal; wizard='sport' (no sport signal here);
  // drill_focus='academic' wins → 'academic-priority match' DOES fire.
  const out = rankCandidates(
    [WYCOMBE_ABBEY],
    structMap(STRUCT_WYCOMBE_ACADEMIC),
    {
  ...
  const wa = out.find(c => c.slug === 'wycombe-abbey')
  assert.ok(wa, 'Wycombe must rank from academic-strong signal')
  assert.ok(wa.signals.includes('academic-strong'),
    `academic-strong should fire. Got: ${JSON.stringify(wa.signals)}`)
  assert.ok(wa.signals.includes('academic-priority match'),
    `drill_focus='academic' must override wizard='sport' to add academic-priority match (Codex r1 Q7). Got: ${JSON.stringify(wa.signals)}`)
})
...
test('Phase 4 item #3 (Codex r1): boarding_pref_from_prose="rejects" filters KNOWN_FULL_BOARDING when wizard boarding_pref is null', () => {
  ...
  const out = rankCandidates(
    ...
    structMap(STRUCT_WYCOMBE_ACADEMIC, STRUCT_DAY_EMPTY),
    {
      parent: { home_region: 'anywhere', budget_range: '40k-50k' },  // boarding_pref=null
      child:  {},
...
test('Phase 4 item #3 (Codex r1): wizard boarding_pref="full" WINS over prose="rejects" (no-erase rule)', () => {
  ...
  const out = rankCandidates(
    ...
    structMap(STRUCT_WYCOMBE_ACADEMIC, STRUCT_DAY_EMPTY),
    {
      parent: { home_region: 'anywhere', boarding_pref: 'full', budget_range: '40k-50k' },
      child:  { goal_orientation: 'university_track' },
      intent: { boarding_pref_from_prose: 'rejects' },
      excludeSlugs: [],
      childGender: 'girl',
...
test('Phase 4 item #3 (Codex r2/r3): boarding_pref_from_prose="full" filters KNOWN_DAY_ONLY_NAMES when wizard boarding_pref is null', () => {
  ...
  const out = rankCandidates(
    ...
    {
      parent: { home_region: 'anywhere', budget_range: '40k-50k' },  // wizard boarding_pref=null
      child:  { goal_orientation: 'university_track' },
      intent: { boarding_pref_from_prose: 'full' },
      excludeSlugs: [],
...
test('Phase 4 item #3 (Codex r2): boarding_pref_from_prose="weekly" resolves to boarder budget (fees_usd_max)', () => {
  ...
  const out = rankCandidates(
    ...
    structMap(STRUCT_WYCOMBE_ACADEMIC),
    {
      parent: { home_region: 'anywhere', budget_range: '40k-50k' },  // null wizard boarding_pref
      child:  { goal_orientation: 'university_track' },
...
// ════════════════════════════════════════════════════════════════════
// Bug #3 (2026-05-22) — Region HARD filter + case-insensitive
// regionInBucket() everywhere. Plus Bug #2 — Warwick School day-only.
// ════════════════════════════════════════════════════════════════════

const A_DAY_SCHOOL_LOWERCASE_REGION = {
  slug: 'a-day-school-lowercase',
  name: 'A Day School Lowercase',
  gender_split: 'Co-ed',
  fees_usd_min: 30000, fees_usd_max: null,
  sen_support: false, strengths: [],
  confidence_score: 50, age_min: 11, age_max: 18,
  region: 'london',          // lowercase — exercises regionInBucket() folding
}
const STRUCT_DAY_LC_EMPTY = {
  school_slug: 'a-day-school-lowercase',
  sports_profile: null, exam_results: null,
  university_destinations: null, student_community: null,
  isi_deep_facts: null, wellbeing_staffing: null, ethos_facts: null,
  subject_strengths: null,
}

const ENGLAND_TAGGED_SCHOOL = {
  slug: 'england-school', name: 'England School',
  gender_split: 'Co-ed', fees_usd_min: 40000, fees_usd_max: null,
  sen_support: false, strengths: [], confidence_score: 60,
  age_min: 11, age_max: 18, region: 'England',
}
const STRUCT_ENGLAND_EMPTY = {
  school_slug: 'england-school',
  sports_profile: null, exam_results: null,
  university_destinations: null, student_community: null,
  isi_deep_facts: null, wellbeing_staffing: null, ethos_facts: null,
  subject_strengths: null,
...
const NULL_REGION_SCHOOL = {
  slug: 'null-region-school', name: 'Null Region School',
  gender_split: 'Co-ed', fees_usd_min: 40000, fees_usd_max: null,
  sen_support: false, strengths: ['sport'], confidence_score: 70,
  age_min: 11, age_max: 18, region: null,
}
const STRUCT_NULL_REGION_EMPTY = {
  school_slug: 'null-region-school',
  sports_profile: null, exam_results: null,
  university_destinations: null, student_community: null,
  isi_deep_facts: null, wellbeing_staffing: null, ethos_facts: null,
  subject_strengths: null,
...
const WARWICK_DAY = {
  slug: 'warwick-school', name: 'Warwick School',
  gender_split: 'Boys', fees_usd_min: 45000, fees_usd_max: null,
  sen_support: false, strengths: ['sport', 'academic'],
  confidence_score: 75, age_min: 11, age_max: 18, region: 'Warwickshire',
}
const STRUCT_WARWICK_EMPTY = {
  school_slug: 'warwick-school',
  sports_profile: null, exam_results: null,
  university_destinations: null, student_community: null,
  isi_deep_facts: null, wellbeing_staffing: null, ethos_facts: null,
  subject_strengths: null,
...
test('Bug #3 region hard filter: drops Warwickshire (midlands) when home_region=south-east', () => {
  // Phase 3 (2026-07-06): a no-boarding-pref (day-ish) parent gets NO floor
  // re-admit, so the wrong-bucket hard-drop holds even with a thin pool —
  // Rugby School (Warwickshire = midlands) is dropped, not re-admitted.
  const out = rankCandidates(
    [RUGBY_SCHOOL, WYCOMBE_ABBEY],
    structMap(STRUCT_RUGBY_SCHOOL_RUGBY_ELITE, STRUCT_WYCOMBE_ACADEMIC),
    {
      parent: { home_region: 'south-east' },
      child:  { interests_sports: [{ sport: 'rugby', level: 'county' }], goal_orientation: 'university_track' },
      excludeSlugs: [],
      childGender: 'girl',
    },
    10,
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('rugby-school'),
    'Rugby School (Warwickshire = midlands) must be DROPPED for south-east parent')
  assert.ok(slugs.includes('wycombe-abbey'),
    'Wycombe Abbey (Buckinghamshire = south-east) must survive (academic-strong via STRUCT)')
})

test('Bug #3 region hard filter: drops Dorset (south-west) when home_region=london', () => {
  // Codex r7 Q2: harden Sherborne with rugby interest + budget signal so
  // the no-signal filter can't explain the drop.
  const out = rankCandidates(
    [SHERBORNE, A_DAY_SCHOOL],
    structMap(STRUCT_SHERBORNE_RUGBY_NATIONAL, STRUCT_DAY_EMPTY),
    {
      parent: { home_region: 'london', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'rugby', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
    },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('sherborne-school'),
    'Sherborne (Dorset = south-west) must be DROPPED for london parent (not just no-signal)')
  assert.ok(slugs.includes('a-day-school'),
    'A_DAY_SCHOOL (London, in budget) must survive — drop is specific to Sherborne')
})

test('Bug #3 region hard filter: NULL-region school SURVIVES (Bug #4 forward investment)', () => {
  // Codex r7 Q2: budget_range so NULL_REGION_SCHOOL gets "in budget" chip
  // (rugby interest doesn't score against STRUCT_NULL_REGION_EMPTY).
  const out = rankCandidates(
    [NULL_REGION_SCHOOL],
    structMap(STRUCT_NULL_REGION_EMPTY),
    {
      parent: { home_region: 'london', budget_range: '40k-50k' },
      child:  null,
      excludeSlugs: [],
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('null-region-school'),
    'Null-region school must SURVIVE the hard filter so Bug #4 backfill catches up')
})

test('Bug #3 region hard filter: England-tagged school SURVIVES (broad country tag)', () => {
  const out = rankCandidates(
    [ENGLAND_TAGGED_SCHOOL],
    structMap(STRUCT_ENGLAND_EMPTY),
    {
      parent: { home_region: 'london', budget_range: '40k-50k' },
      child:  null,
      excludeSlugs: [],
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('england-school'),
    'England-tagged school must SURVIVE the hard filter (broad country tag is not bucket-disqualifying)')
})

test('Bug #3 region hard filter: SKIPPED when home_region=anywhere (no region restriction)', () => {
  const out = rankCandidates(
    [SHERBORNE, RUGBY_SCHOOL, A_DAY_SCHOOL],
    structMap(STRUCT_SHERBORNE_RUGBY_NATIONAL, STRUCT_RUGBY_SCHOOL_RUGBY_ELITE, STRUCT_DAY_EMPTY),
    {
      parent: { home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'rugby', level: 'county' }] },
      excludeSlugs: [],
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('sherborne-school'),
    'home_region=anywhere must NOT drop schools — Sherborne survives')
  assert.ok(slugs.includes('rugby-school'),
    'home_region=anywhere must NOT drop schools — Rugby survives')
})

test('Bug #3 region hard filter: SKIPPED when home_region is null (parent didn\'t answer)', () => {
  const out = rankCandidates(
    [SHERBORNE],
    structMap(STRUCT_SHERBORNE_RUGBY_NATIONAL),
    {
      parent: { home_region: null, budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'rugby', level: 'county' }] },
      excludeSlugs: [],
      childGender: 'boy',
    },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('sherborne-school'),
    'home_region=null must NOT drop schools — Sherborne survives')
})

test('Bug #3 + Q10 #1: case-insensitive regionInBucket matches lowercase region', () => {
  const out = rankCandidates(
    [A_DAY_SCHOOL_LOWERCASE_REGION],
    structMap(STRUCT_DAY_LC_EMPTY),
    {
      parent: { home_region: 'london', budget_range: '40k-50k' },
      child:  null,
      excludeSlugs: [],
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('a-day-school-lowercase'),
    'Lowercase region="london" must match home_region=london (case-insensitive)')
  const school = out.find(c => c.slug === 'a-day-school-lowercase')
  assert.ok(
    school.signals.some(s => s.includes('region')),
    `expected a region chip on lowercase match, got: ${JSON.stringify(school.signals)}`,
  )
})

test('Bug #3 region hard filter: unknown home_region (typo / legacy) does NOT drop everything', () => {
  // Defensive: typo or schema-drift value not in REGION_BUCKETS would
  // otherwise drop every non-null-region school. Object.hasOwn guard
  // skips the hard filter for unknown buckets.
  const out = rankCandidates(
    [SHERBORNE, RUGBY_SCHOOL, WYCOMBE_ABBEY],
    structMap(STRUCT_SHERBORNE_RUGBY_NATIONAL, STRUCT_RUGBY_SCHOOL_RUGBY_ELITE, STRUCT_WYCOMBE_ACADEMIC),
    {
      parent: { home_region: 'south-east-england', budget_range: '40k-50k' },   // typo
      child:  { interests_sports: [{ sport: 'rugby', level: 'county' }] },
      excludeSlugs: [],
    },
    10,
  ...
  assert.ok(slugs.length >= 1,
    'Unknown home_region must NOT drop every school — at least one survives')
})

test('Bug #2: KNOWN_DAY_ONLY_NAMES drops Warwick School for full-boarding parent', () => {
  const out = rankCandidates(
    [WARWICK_DAY, RUGBY_SCHOOL],
    structMap(STRUCT_WARWICK_EMPTY, STRUCT_RUGBY_SCHOOL_RUGBY_ELITE),
    {
      parent: { boarding_pref: 'full', home_region: 'midlands' },
      child:  { interests_sports: [{ sport: 'rugby', level: 'county' }] },
      excludeSlugs: [],
      childGender: 'boy',
    },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('warwick-school'),
    'Warwick School (day-only via "warwick" override) must be dropped for full-boarding parent')
  assert.ok(slugs.includes('rugby-school'),
    'Rugby School (full-boarding) must survive')
})

test('Bug #3 + Q3: home_region with mixed case + whitespace normalizes correctly', () => {
  // Codex r7 Q2: harden Sherborne with rugby + budget so no-signal
  // filter can't explain its drop.
  const out = rankCandidates(
    [SHERBORNE, A_DAY_SCHOOL],
    structMap(STRUCT_SHERBORNE_RUGBY_NATIONAL, STRUCT_DAY_EMPTY),
    {
      parent: { home_region: ' LONDON ', budget_range: '40k-50k' },   // mixed case + whitespace
      child:  { interests_sports: [{ sport: 'rugby', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
    },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('sherborne-school'),
    `" LONDON " (whitespace + caps) must normalize to 'london' and drop Sherborne. Got: ${JSON.stringify(slugs)}`)
  assert.ok(slugs.includes('a-day-school'),
    `London-region A_DAY_SCHOOL must survive normalized ' LONDON ' input`)
})

// ─────────────────────────────────────────────────────────────────────
// Yoko slice 2026-05-24 — resolver helpers + nonneg additions + A* penalty.
// Codex r1-r5 reviewed; v5 sketches converged with GREEN for apply.
// ─────────────────────────────────────────────────────────────────────

const INTENT_NONE = {
  academic_intent: 'none', top_uni_intent: 'none', pastoral_priority: 'none',
  inclusive_priority: 'none', small_env_pref: 'none', boarding_pref_from_prose: 'none',
  current_school_pain: 'none', parent_drill_focus: 'none',
  sport_focus: 'none',
  classification_version: 'phase-2-8-sport-focus-v2-flexi',
}

// ── Yoko: resolveBoardingPref ───────────────────────────────────────

test('Yoko slice: resolveBoardingPref — nonneg no-boarding overrides wizard full → day', () => {
  const fired = matchedNonnegFilters(['day school only'])
  assert.ok(fired.some(f => f.name === 'no-boarding'), 'no-boarding must fire')
  assert.equal(resolveBoardingPref({ boarding_pref: 'full' }, INTENT_NONE, fired), 'day')
})

test('Yoko slice: resolveBoardingPref — nonneg boarding-required overrides wizard day → full', () => {
  const fired = matchedNonnegFilters(['full boarding only'])
  assert.equal(resolveBoardingPref({ boarding_pref: 'day' }, INTENT_NONE, fired), 'full')
})

test('Yoko slice: resolveBoardingPref — no nonneg, wizard wins (regression guard)', () => {
  assert.equal(resolveBoardingPref({ boarding_pref: 'full' }, INTENT_NONE, []), 'full')
})

test('Yoko slice: resolveBoardingPref — no nonneg + no wizard, prose fills', () => {
  assert.equal(
    resolveBoardingPref({ boarding_pref: null }, { ...INTENT_NONE, boarding_pref_from_prose: 'full' }, []),
    'full',
  )
})

// ── Yoko: resolveHomeRegion ─────────────────────────────────────────

test('Yoko slice: resolveHomeRegion — must-be-london overrides parent midlands', () => {
  const fired = matchedNonnegFilters(['London zone 1-3'])
  assert.equal(resolveHomeRegion({ home_region: 'midlands' }, fired), 'london')
})

test('Yoko slice (r4 F1): resolveHomeRegion — no-london suppresses region filter (returns null)', () => {
  const fired = matchedNonnegFilters(['no London'])
  assert.equal(resolveHomeRegion({ home_region: 'london' }, fired), null,
    'no-london must return null so predicate (not region filter) owns London-exclusion')
})

test('Yoko slice: resolveHomeRegion — no nonneg, parent midlands preserved', () => {
  assert.equal(resolveHomeRegion({ home_region: 'midlands' }, []), 'midlands')
})

test('Yoko slice: resolveHomeRegion — parent anywhere → null', () => {
  assert.equal(resolveHomeRegion({ home_region: 'anywhere' }, []), null)
})

test('Yoko slice: resolveHomeRegion — unknown region (legacy) → null', () => {
  assert.equal(resolveHomeRegion({ home_region: 'mars' }, []), null)
})

// ── Yoko: resolveCurriculumPref ─────────────────────────────────────

test('Yoko slice: resolveCurriculumPref — any-curriculum overrides ib → either', () => {
  const fired = matchedNonnegFilters(['any curriculum'])
  assert.equal(resolveCurriculumPref({ curriculum_pref: 'ib' }, fired), 'either')
})

test('Yoko slice: resolveCurriculumPref — no nonneg, parent ib preserved', () => {
  assert.equal(resolveCurriculumPref({ curriculum_pref: 'ib' }, []), 'ib')
})

// ── Yoko: must-be-london positive ───────────────────────────────────

test('Yoko slice: must-be-london matches positive phrasings', () => {
  const cases = ['London zone 1-3', 'London or nearby', 'in London only', 'near London',
                 'must be London', 'day school in London', 'London day school',
                 'commutable from London', 'London catchment']
  for (const c of cases) {
    const fired = matchedNonnegFilters([c])
    const names = fired.map(f => f.name)
    assert.ok(names.includes('must-be-london'), `"${c}" should trigger must-be-london`)
  }
})

// ── Yoko (r3+r4): negative-context safety ───────────────────────────

test('Yoko slice (r3+r4): must-be-london does NOT fire for any negative phrasing', () => {
  const cases = [
    // r3 cases
    'not in London', 'not near London', 'no school in London',
    'not London', 'anywhere but London', 'outside London',
    // r4 cases
    'not looking in London', 'not interested in London',
    'not a school in London', 'not a London day school',
    'no day school in London', 'no independent school in London',
    // r5 helper-only cases (verb forms)
    'not considering London', 'not moving to London', 'not going to London',
    'not looking at London', 'not wanting London',
  ]
  for (const c of cases) {
    const fired = matchedNonnegFilters([c])
    const names = fired.map(f => f.name)
    assert.ok(!names.includes('must-be-london'), `"${c}" must NOT trigger must-be-london`)
    assert.ok(names.includes('no-london'), `"${c}" must trigger no-london (helper-authoritative)`)
  }
})

// ── Yoko: false-positive avoidance ──────────────────────────────────

test('Yoko slice (r4 F3): "no preference for boarding" does NOT trigger any London filter', () => {
  const fired = matchedNonnegFilters(['no preference for boarding'])
  const names = fired.map(f => f.name)
  assert.ok(!names.includes('no-london'))
  assert.ok(!names.includes('must-be-london'))
})

// ── Yoko: predicate alias-awareness ─────────────────────────────────

test('Yoko slice (r2 F1b): must-be-london predicate — Wimbledon passes (regionInBucket alias)', () => {
  const filter = NONNEG_FILTERS.find(f => f.name === 'must-be-london')
  assert.equal(filter.predicate({ region: 'Wimbledon', name: 'X' }, null), true)
})

test('Yoko slice (r2 F1b): no-london predicate — Wimbledon DROPS (alias-aware)', () => {
  const filter = NONNEG_FILTERS.find(f => f.name === 'no-london')
  assert.equal(filter.predicate({ region: 'Wimbledon', name: 'X' }, null), false,
    'Wimbledon must be dropped by no-london (London bucket alias)')
})

test('Yoko slice: must-be-london predicate — Yorkshire DROPS', () => {
  const filter = NONNEG_FILTERS.find(f => f.name === 'must-be-london')
  assert.equal(filter.predicate({ region: 'Yorkshire', name: 'X' }, null), false)
})

// ── Yoko: any-curriculum ────────────────────────────────────────────

// ─────────────────────────────────────────────────────────────────────
// Slice A 2026-05-24 — curriculum row-time resolver
// Codex r1 verdict: row-time helper prefers ssd.curriculum (extracted
// truth) over legacy schools.curriculum (manual rot). Fixes Charterhouse.
// ─────────────────────────────────────────────────────────────────────

test('Slice A: matchesCurriculumPreference — pref=either always passes', () => {
  assert.equal(matchesCurriculumPreference({ schoolsCurriculum: null, ssdCurriculum: null, pref: 'either' }), true)
  assert.equal(matchesCurriculumPreference({ schoolsCurriculum: ['IB'], ssdCurriculum: null, pref: 'either' }), true)
})

test('Slice A: matchesCurriculumPreference — pref=null always passes', () => {
  assert.equal(matchesCurriculumPreference({ schoolsCurriculum: null, ssdCurriculum: null, pref: null }), true)
})

test('Slice A: IB strict — SSD has IB → pass', () => {
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: ['GCSE', 'A Level', 'IB Diploma'], pref: 'ib',
  }), true)
})

test('Slice A: IB strict — SSD has International Baccalaureate (long form) → pass', () => {
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: ['GCSE', 'International Baccalaureate Diploma Programme'], pref: 'ib',
  }), true)
})

test('Slice A: IB strict — SSD has IBDP → pass', () => {
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: ['GCSE', 'IBDP'], pref: 'ib',
  }), true)
})

test('Slice A: IB strict — SSD does NOT list IB → fail (e.g. Eton)', () => {
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: ['GCSE', 'IGCSE', 'A Level', 'Pre-U'], pref: 'ib',
  }), false)
})

test('Slice A (Charterhouse case): legacy schools=["IB"] but SSD missing → fail for IB', () => {
  // The killer case: legacy column lies but ssd has no extraction yet.
  // Per Codex Q1 verdict, IB-strict rejects when SSD missing — does NOT
  // fall back to schools.curriculum. This prevents Charterhouse-style
  // false positives.
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: ['IB'],  // legacy lies
    ssdCurriculum:     null,    // no extraction
    pref:              'ib',
  }), false)
})

test('Slice A (Box Hill case): legacy schools=null but SSD lists IB → pass for IB', () => {
  // The complement case: legacy column is missing the IB tag but ssd
  // extracted it. Helper trusts ssd and lets the school surface.
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null,
    ssdCurriculum:     ['GCSE', 'A Level', 'IB Diploma'],
    pref:              'ib',
  }), true)
})

test('Slice A: A-Level permissive — SSD has A Level → pass', () => {
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: ['GCSE', 'A Level'], pref: 'a-level',
  }), true)
})

test('Slice A: A-Level permissive — SSD missing → pass (UK default)', () => {
  // Per Codex Q4: UK default means most schools teach A-Level even if
  // not tagged. With country='United Kingdom' SQL filter + UK-evidence
  // gating, this fallback is safe.
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: null, pref: 'a-level',
  }), true)
})

test('Slice A: A-Level permissive — SSD has IB only (no A Level) → fail', () => {
  // Sevenoaks-pattern: pure IB school. A-Level parent shouldn't get it.
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null,
    ssdCurriculum:     ['GCSE', 'IGCSE', 'IB Diploma'],
    pref:              'a-level',
  }), false)
})

test('Slice A: A-Level permissive — Pre-U alone does NOT count as A-Level (Codex Q3)', () => {
  assert.equal(matchesCurriculumPreference({
    schoolsCurriculum: null, ssdCurriculum: ['GCSE', 'Pre-U'], pref: 'a-level',
  }), false)
})

test('Yoko slice (r2 F4): any-curriculum matches "preferred" phrasings', () => {
  const cases = ['any curriculum', 'either curriculum', "curriculum doesn't matter",
                 'no strong curriculum preference', 'no curriculum preferred',
                 'no qualification preferred', 'open to IB or A-level']
  for (const c of cases) {
    const fired = matchedNonnegFilters([c])
    const names = fired.map(f => f.name)
    assert.ok(names.includes('any-curriculum'), `"${c}" should trigger any-curriculum`)
  }
})


// ─── Phase 2.8.7 — focus-sport specialist bonus ─────────────────────
//
// When intent.sport_focus matches the scored sport AND school tier is
// >= national, add a specialist bonus on top of the standard sport
// boost. Closes the weighting bug Codex traced: mixed academic+sport
// briefs (drill_focus='academic' + sport_focus='tennis') were ranking
// sport-elite schools BELOW academic-elite schools because the academic
// stack (+2.5) outweighed the tennis specialty (+1.75 capped). The
// specialist bonus tilts the composite toward the rare specialty.

test('Phase 2.8.7: rankCandidates — sport_focus=tennis school with national-elite tennis gets +1.0 specialist bonus', () => {
  const tennisEliteSchool = {
    slug: 'reeds-test', name: "Test Reed's",
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 10, age_max: 19,
    region: 'Surrey',
  }
  const struct = {
    school_slug: 'reeds-test',
    sports_profile: { tennis: { competitive_tier: 'national-elite' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  // Note: child has interests_sports=[tennis] which resolveSportFocus
  // promotes via structured-pick precedence. So focusResolution.sport='tennis'
  // regardless of intent value — that's the correct Codex r1 P2 behavior.
  // Assert signal presence directly.
  const scored = rankCandidates(
    [tennisEliteSchool],
    new Map([['reeds-test', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'tennis', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
      intent: { ...INTENT_NONE, sport_focus: 'tennis' },
    },
    10,
  )
  assert.ok(scored[0]?.signals.includes('specialist tennis'),
    `national-elite tennis + tennis interest → specialist tennis signal; got: ${scored[0]?.signals.join(', ')}`)
})

test('Phase 2.8.7: national-strong tennis gets +0.5 specialist bonus', () => {
  const tennisStrongSchool = {
    slug: 'strong-tennis', name: 'Strong Tennis School',
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 11, age_max: 18,
    region: 'Hertfordshire',
  }
  const struct = {
    school_slug: 'strong-tennis',
    sports_profile: { tennis: { competitive_tier: 'national-strong' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const scored = rankCandidates(
    [tennisStrongSchool], new Map([['strong-tennis', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'tennis', level: 'national' }] },
      excludeSlugs: [],
  ...
  assert.ok(scored[0]?.signals.includes('specialist tennis'),
    `national-strong tennis + tennis interest → specialist tennis signal; got: ${scored[0]?.signals.join(', ')}`)
})

test('Phase 2.8.7: regional tennis gets NO specialist bonus (only national+)', () => {
  const regionalTennisSchool = {
    slug: 'regional-tennis', name: 'Regional Tennis School',
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 11, age_max: 18,
    region: 'Kent',
  }
  const struct = {
    school_slug: 'regional-tennis',
    sports_profile: { tennis: { competitive_tier: 'regional' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const base = rankCandidates(
    [regionalTennisSchool], new Map([['regional-tennis', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'tennis', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
      intent: { ...INTENT_NONE, sport_focus: 'none' },
    }, 10,
  )
  const withFocus = rankCandidates(
    [regionalTennisSchool], new Map([['regional-tennis', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'tennis', level: 'national' }] },
  ...
  const delta = withFocus[0].total_score - base[0].total_score
  assert.equal(delta, 0,
    `regional tennis must get NO specialist bonus; got delta=${delta}`)
  assert.ok(!withFocus[0].signals.includes('specialist tennis'),
    `signals should NOT include 'specialist tennis': ${withFocus[0].signals.join(', ')}`)
})

test('Phase 2.8.7 (Codex r3 P2): alias "soccer" + sport_focus=football + national-elite football → specialist football fires', () => {
  const footballEliteSchool = {
    slug: 'football-elite', name: 'Football Elite School',
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 11, age_max: 18,
    region: 'Kent',
  }
  const struct = {
    school_slug: 'football-elite',
    sports_profile: { football: { competitive_tier: 'national-elite' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const scored = rankCandidates(
    [footballEliteSchool], new Map([['football-elite', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'soccer', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
      intent: { ...INTENT_NONE, sport_focus: 'football' },
    }, 10,
  )
  assert.ok(scored[0]?.signals.includes('specialist football'),
    `alias 'soccer' should canonicalize to football for tier lookup AND specialist; signals: ${scored[0]?.signals.join(', ')}`)
})

test('Phase 2.8.7 (Codex r3 P2): alias "field hockey" + sport_focus=hockey + national-elite hockey → specialist hockey fires', () => {
  const hockeyEliteSchool = {
    slug: 'hockey-elite', name: 'Hockey Elite School',
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 11, age_max: 18,
    region: 'Sussex',
  }
  const struct = {
    school_slug: 'hockey-elite',
    sports_profile: { hockey: { competitive_tier: 'national-elite' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const scored = rankCandidates(
    [hockeyEliteSchool], new Map([['hockey-elite', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'field hockey', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
      intent: { ...INTENT_NONE, sport_focus: 'hockey' },
    }, 10,
  )
  assert.ok(scored[0]?.signals.includes('specialist hockey'),
    `alias 'field hockey' should canonicalize to hockey for tier lookup AND specialist; signals: ${scored[0]?.signals.join(', ')}`)
})

test('Phase 2.8.7 (Codex r2 P2): alias structured-sport "rugby union" + sport_focus=rugby → specialist rugby fires', () => {
  // Codex r2 found that sp.key can be an alias from SPORT_SCORER_BY_LABEL
  // ('rugby union'/'rugby league'/'soccer'/'field hockey'), but focusSportKey
  // from resolveSportFocus is always canonical. Specialist block must
  // canonicalize sp.key before comparison or alias entries miss the bonus.
  const rugbyEliteSchool = {
    slug: 'rugby-alias-test', name: 'Rugby Alias Test',
    gender_split: 'Boys', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 13, age_max: 18,
    region: 'London',
  }
  const struct = {
    school_slug: 'rugby-alias-test',
    sports_profile: { rugby: { competitive_tier: 'national-elite' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const scored = rankCandidates(
    [rugbyEliteSchool], new Map([['rugby-alias-test', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      // ALIAS sport label — resolveSportFocus canonicalizes 'rugby union' → 'rugby',
      // so focusResolution.sport='rugby'. Specialist must still fire against
      // sp.key='rugby union' (the alias the parent typed).
      child:  { interests_sports: [{ sport: 'rugby union', level: 'national' }] },
      excludeSlugs: [],
      childGender: 'boy',
      intent: { ...INTENT_NONE, sport_focus: 'rugby' },
    }, 10,
  )
  assert.ok(scored[0]?.signals.includes('specialist rugby'),
    `alias 'rugby union' should still trigger specialist rugby; signals: ${scored[0]?.signals.join(', ')}`)
})

test('Phase 2.8.7 (Codex r1 P1): duplicate sportsInterest entries do NOT stack the specialist bonus', () => {
  const tennisEliteSchool = {
    slug: 'reeds-dup', name: "Reed's Dup",
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 10, age_max: 19,
    region: 'Surrey',
  }
  const struct = {
    school_slug: 'reeds-dup',
    sports_profile: { tennis: { competitive_tier: 'national-elite' } },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const oneEntry = rankCandidates(
    [tennisEliteSchool], new Map([['reeds-dup', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [{ sport: 'tennis', level: 'national' }] },
      excludeSlugs: [],
  ...
  const dupEntries = rankCandidates(
    [tennisEliteSchool], new Map([['reeds-dup', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [
        { sport: 'tennis', level: 'national' },
        { sport: 'tennis', level: 'national' },
      ] },
      excludeSlugs: [],
  ...
  // Both should have at most ONE 'specialist tennis' signal even with duplicate
  // sport entries. The score delta between dup and single is bounded by the
  // SPORT_TOTAL_CAP — but the specialist bonus itself should NOT double up.
  const oneSpecialistCount = oneEntry[0].signals.filter(s => s === 'specialist tennis').length
  const dupSpecialistCount = dupEntries[0].signals.filter(s => s === 'specialist tennis').length
  assert.equal(oneSpecialistCount, 1, 'single-entry case: specialist tennis emitted once')
  assert.equal(dupSpecialistCount, 1, 'duplicate-entry case: specialist tennis still emitted only once (no stacking)')
})

test('Phase 2.8.7: sport_focus=tennis + scoring includes rugby + school is rugby-elite → NO specialist for rugby (focus mismatch)', () => {
  // School has BOTH sports scored, but sport_focus is tennis. Specialist
  // should only fire for the FOCUS sport (tennis), not rugby — even when
  // rugby is the school's stronger sport. Locks: focus precedence over
  // school's loudest sport.
  const mixedSchool = {
    slug: 'mixed-school', name: 'Mixed Sports School',
    gender_split: 'Co-ed', fees_usd_min: 45000, sen_support: false,
    strengths: ['sport'], confidence_score: 100, age_min: 13, age_max: 18,
    region: 'Berkshire',
  }
  const struct = {
    school_slug: 'mixed-school',
    sports_profile: {
      tennis: { competitive_tier: 'regional' },          // tennis = weak
      rugby:  { competitive_tier: 'national-elite' },    // rugby = elite
    },
    exam_results: null, university_destinations: null,
    student_community: null, isi_deep_facts: null,
  }
  const withTennisFocus = rankCandidates(
    [mixedSchool], new Map([['mixed-school', struct]]),
    {
      parent: { boarding_pref: 'full', home_region: 'anywhere', budget_range: '40k-50k' },
      child:  { interests_sports: [
        { sport: 'tennis', level: 'national' },
  ...
  const sigs = withTennisFocus[0]?.signals ?? []
  assert.ok(!sigs.includes('specialist tennis'),
    `regional tennis (below threshold) should NOT trigger specialist; signals: ${sigs.join(', ')}`)
  assert.ok(!sigs.includes('specialist rugby'),
    `rugby-elite with sport_focus=tennis should NOT trigger specialist rugby (focus mismatch); signals: ${sigs.join(', ')}`)
})

// ═══════════════════════════════════════════════════════════════════════
// Phase 2 (2026-07-06 scorer pool bugs) — GRADED boarding hard filter + tilt
// ═══════════════════════════════════════════════════════════════════════
//
// The filter-critical boundary is "offers full boarding at all?" derived from
// schools.boarding_grade, LAYERED with the curated name-lists. These tests
// pin: (a) the effective-grade helper (name-list-over-column, the Merchiston
// invariant), (b) the full/weekly/day hard-filter keep/drop behaviour, and
// (c) the small full-seeker ranker tilt. Every fixture is co-ed with strong
// exam data so the only variable is boarding — an 'academic-strong' signal
// keeps each school past the withSignals drop regardless of its boarding.

// ── Direct helper unit tests (Merchiston invariant lives here) ──────────

test('effectiveBoardingGrade: bare column passes through when no name-list match', () => {
  assert.equal(effectiveBoardingGrade('Some Unlisted School', 'full-dominant'), 'full-dominant')
  assert.equal(effectiveBoardingGrade('Some Unlisted School', 'offers-full'),   'offers-full')
  assert.equal(effectiveBoardingGrade('Some Unlisted School', 'weekly-only'),   'weekly-only')
  assert.equal(effectiveBoardingGrade('Some Unlisted School', 'day-only'),      'day-only')
})

test('effectiveBoardingGrade: NULL / garbage column → unknown (fails open)', () => {
  assert.equal(effectiveBoardingGrade('Some Unlisted School', null),        'unknown')
  assert.equal(effectiveBoardingGrade('Some Unlisted School', undefined),   'unknown')
  assert.equal(effectiveBoardingGrade('Some Unlisted School', 'garbage'),   'unknown')
})

test('effectiveBoardingGrade: MERCHISTON INVARIANT — name-list floors weekly-only column up to offers-full', () => {
  // Merchiston Castle is in KNOWN_FULL_BOARDING_NAMES but its keyword-less
  // "houses" prose grades the column 'weekly-only'. The override must floor
  // it to offers-full so a full-seeker keeps it.
  assert.equal(effectiveBoardingGrade('Merchiston Castle School', 'weekly-only'), 'offers-full')
  assert.equal(effectiveBoardingGrade('Merchiston Castle School', null),          'offers-full')
})

test('effectiveBoardingGrade: full-boarding name-list PRESERVES a stronger full-dominant column', () => {
  // The override is a floor, not a clamp — a KNOWN full-boarder graded
  // full-dominant keeps full-dominant so the ranker tilt still sees it.
  assert.equal(effectiveBoardingGrade('Harrow School', 'full-dominant'), 'full-dominant')
})

test('effectiveBoardingGrade: KNOWN_DAY_ONLY name overrides even an offers-full column', () => {
  // Curated day-only beats any column reading (defence against a mis-graded
  // London day school).
  assert.equal(effectiveBoardingGrade('Westminster School', 'offers-full'),   'day-only')
  assert.equal(effectiveBoardingGrade('Godolphin and Latymer', 'full-dominant'), 'day-only')
})

test('offersFullBoarding / offersAnyBoarding predicate truth table', () => {
  // full-seeker keeps offers-full / full-dominant / unknown; drops day/weekly.
  assert.equal(offersFullBoarding('full-dominant'), true)
  assert.equal(offersFullBoarding('offers-full'),   true)
  assert.equal(offersFullBoarding('unknown'),       true)  // fails open
  assert.equal(offersFullBoarding('weekly-only'),   false)
  assert.equal(offersFullBoarding('day-only'),      false)
  // weekly/flexi-seeker drops only day-only.
  assert.equal(offersAnyBoarding('weekly-only'),    true)
  assert.equal(offersAnyBoarding('offers-full'),    true)
  assert.equal(offersAnyBoarding('full-dominant'),  true)
  assert.equal(offersAnyBoarding('unknown'),        true)  // fails open
  assert.equal(offersAnyBoarding('day-only'),       false)
})

// Codex r2 NIT — effectiveBoardingGrade gives KNOWN_FULL_BOARDING_NAMES
// precedence over KNOWN_DAY_ONLY_NAMES. A name in BOTH would silently resolve
// to offers-full and mask a day-only school. Pin the invariant so a future
// accidental duplicate is caught here, not in prod.
test('invariant: KNOWN_FULL_BOARDING_NAMES and KNOWN_DAY_ONLY_NAMES do not overlap', () => {
  const overlap = [...KNOWN_FULL_BOARDING_NAMES].filter(n => KNOWN_DAY_ONLY_NAMES.has(n))
  assert.deepEqual(overlap, [], `name-lists must be disjoint; overlap: ${JSON.stringify(overlap)}`)
})

// ── Integration fixtures (co-ed + strong academics = boarding is the only var) ──

function boardingRow(slug, name, grade, extra = {}) {
  return {
    slug, name,
    gender_split: 'Co-ed',
    fees_usd_min: 40000, fees_usd_max: 45000,
    sen_support: false,
    strengths: ['academic'],
    confidence_score: 80,
    age_min: 11, age_max: 18,
    region: 'Somerset',
    boarding_grade: grade,
    ...extra,
  }
}
function academicStruct(slug) {
  return {
    school_slug: slug,
    sports_profile: null,
    exam_results: { a_level: { pct_a_star: 55, pct_a_star_a: 82 }, gcse: { pct_9: 45 } },
    university_destinations: { oxford_count: 3, cambridge_count: 3, russell_group_count: 40 },
    student_community: null,
    isi_deep_facts: null,
  }
}
const B_FULLDOM  = boardingRow('b-fulldom',   'Northgate Country School', 'full-dominant')
const B_OFFERS   = boardingRow('b-offers',    'Riverside Hall School',     'offers-full')
const B_WEEKLY   = boardingRow('b-weekly',    'Midweek House School',      'weekly-only')
const B_DAYONLY  = boardingRow('b-dayonly',   'Townsend Day School',       'day-only')
const B_UNKNOWN  = boardingRow('b-unknown',   'Mystery Grange School',      null)
// Merchiston: column weekly-only but KNOWN_FULL_BOARDING_NAMES → offers-full.
const B_MERCH    = boardingRow('merchiston-castle-school', 'Merchiston Castle School', 'weekly-only')
// Curated day-only name with a contrived offers-full column → still day-only.
const B_WESTMIN  = boardingRow('westminster-school', 'Westminster School', 'offers-full')

const BOARDING_STRUCTS = structMap(
  academicStruct('b-fulldom'), academicStruct('b-offers'), academicStruct('b-weekly'),
  academicStruct('b-dayonly'), academicStruct('b-unknown'),
  academicStruct('merchiston-castle-school'), academicStruct('westminster-school'),
)
const UNIV_CHILD = { goal_orientation: 'university_track' }

test('full-seeker: keeps offers-full/full-dominant/unknown, drops weekly-only + day-only', () => {
  const out = rankCandidates(
    [B_FULLDOM, B_OFFERS, B_WEEKLY, B_DAYONLY, B_UNKNOWN],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('b-fulldom'), 'full-dominant kept')
  assert.ok(slugs.includes('b-offers'),  'offers-full kept')
  assert.ok(slugs.includes('b-unknown'), 'unknown must fail OPEN (no-data never hard-dropped)')
  assert.ok(!slugs.includes('b-weekly'), 'weekly-only dropped for a full-seeker')
  assert.ok(!slugs.includes('b-dayonly'),'day-only dropped for a full-seeker')
})

test('full-seeker: MERCHISTON INVARIANT — weekly-only column but name-listed full is KEPT', () => {
  const out = rankCandidates(
    [B_MERCH, B_DAYONLY],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('merchiston-castle-school'),
    'Merchiston (KNOWN_FULL_BOARDING_NAMES) must survive despite a weekly-only column grade')
  assert.ok(!slugs.includes('b-dayonly'), 'confirms the filter still drops a genuine day-only school')
})

test('full-seeker: curated day-only name dropped even with an offers-full column', () => {
  const out = rankCandidates(
    [B_WESTMIN, B_OFFERS],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
  ...
  const slugs = out.map(c => c.slug)
  assert.ok(!slugs.includes('westminster-school'),
    'Westminster (KNOWN_DAY_ONLY_NAMES) dropped for a full-seeker regardless of column')
  assert.ok(slugs.includes('b-offers'), 'a genuine offers-full school is still kept')
})

test('weekly-seeker: keeps weekly-only + offers-full + unknown, drops only day-only', () => {
  const out = rankCandidates(
    [B_WEEKLY, B_OFFERS, B_UNKNOWN, B_DAYONLY],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'weekly', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('b-weekly'),  'weekly-only kept for a weekly-seeker')
  assert.ok(slugs.includes('b-offers'),  'offers-full kept for a weekly-seeker')
  assert.ok(slugs.includes('b-unknown'), 'unknown fails open for a weekly-seeker')
  assert.ok(!slugs.includes('b-dayonly'),'day-only dropped for a weekly-seeker')
})

test('flexi-seeker behaves like weekly-seeker (drops only day-only)', () => {
  const out = rankCandidates(
    [B_WEEKLY, B_DAYONLY],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'flexi', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('b-weekly'),   'weekly-only kept for a flexi-seeker')
  assert.ok(!slugs.includes('b-dayonly'), 'day-only dropped for a flexi-seeker')
})

test('day-seeker: graded column is NOT a drop criterion (full-dominant day-taker survives)', () => {
  // Canford-style full-dominant schools still take day pupils — the day path
  // must keep dropping only KNOWN_FULL_BOARDING_NAMES, never column grade.
  const out = rankCandidates(
    [B_FULLDOM, B_DAYONLY],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'day', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  const slugs = out.map(c => c.slug)
  assert.ok(slugs.includes('b-fulldom'), 'an unlisted full-dominant school must NOT be dropped for a day-seeker')
  assert.ok(slugs.includes('b-dayonly'), 'day-only school kept for a day-seeker')
})

test('no boarding preference: graded filter is inert (all grades survive)', () => {
  const out = rankCandidates(
    [B_FULLDOM, B_OFFERS, B_WEEKLY, B_DAYONLY, B_UNKNOWN],
    BOARDING_STRUCTS,
    { parent: { home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  assert.equal(out.length, 5, 'with no boarding pref, boarding grade must not filter anything')
})

test('full-seeker ranker tilt: full-dominant > offers-full > unknown, with grade-aware signals', () => {
  const out = rankCandidates(
    [B_FULLDOM, B_OFFERS, B_UNKNOWN],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  const bySlug = new Map(out.map(c => [c.slug, c]))
  const fd = bySlug.get('b-fulldom'), of = bySlug.get('b-offers'), un = bySlug.get('b-unknown')
  assert.ok(fd && of && un, 'all three kept')
  assert.ok(fd.total_score > of.total_score, 'full-dominant tilts above offers-full')
  assert.ok(of.total_score > un.total_score, 'offers-full tilts above unknown (unknown penalised)')
  assert.ok(fd.signals.includes('boarding-focused'), 'full-dominant emits the boarding-focused signal')
  assert.ok(of.signals.includes('offers full boarding'), 'offers-full emits the HEDGED offers-full signal')
  assert.ok(!of.signals.includes('boarding-focused'), 'offers-full is NOT boarding-focused (that is full-dominant only)')
  assert.ok(!un.signals.includes('boarding-focused') && !un.signals.includes('offers full boarding'),
    'unknown emits no boarding signal (score-only penalty)')
})

// Codex r1 P1/P2 — the old KNOWN_FULL_BOARDING_NAMES prose bonus emitted a
// bare 'full boarding' signal for offers-full schools (Wellington/Oakham),
// which lands in the deterministic rationale_seed and over-claims. The tilt
// is now grade-aware: an offers-full KNOWN full-boarder must read 'offers
// full boarding', never bare 'full boarding'.
const B_WELLINGTON = boardingRow('wellington-college', 'Wellington College', 'offers-full')
test('offers-full KNOWN full-boarder (Wellington) uses HEDGED signal, never bare "full boarding"', () => {
  const out = rankCandidates(
    [B_WELLINGTON],
    structMap(academicStruct('wellington-college')),
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    10,
  )
  const w = out.find(c => c.slug === 'wellington-college')
  assert.ok(w, 'Wellington kept for a full-seeker')
  assert.ok(w.signals.includes('offers full boarding'), 'must surface the hedged offers-full signal')
  assert.ok(!w.signals.includes('full boarding'), 'must NOT surface the old bare "full boarding" over-claim signal')
  // The deterministic rationale_seed embeds signals verbatim — confirm the
  // hedged phrase is what parents would see, not a bare full-boarding claim.
  assert.ok(w.rationale_seed.includes('offers full boarding'),
    `rationale_seed should carry the hedged phrase; got: ${w.rationale_seed}`)
})

// Codex r1 #6 — behaviour-change pin: the full-dominant 'boarding-focused'
// signal is a positive signal, so a full-dominant school that matched
// nothing else now survives the zero-signal drop for a full-seeker. This is
// intended (a full boarder IS relevant to a full-seeker); pin it so it can't
// silently regress.
test('full-dominant boarding-focused signal rescues an otherwise zero-signal school (full-seeker)', () => {
  const bare = boardingRow('bare-fulldom', 'Quietvale Country School', 'full-dominant')
  const emptyStruct = { school_slug: 'bare-fulldom', sports_profile: null, exam_results: null,
    university_destinations: null, student_community: null, isi_deep_facts: null }
  const out = rankCandidates(
    [bare],
    structMap(emptyStruct),
    // no academic/region/budget/sport signal — only boarding grade
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: null, excludeSlugs: [] },
    10,
  )
  const b = out.find(c => c.slug === 'bare-fulldom')
  assert.ok(b, 'a full-dominant school survives for a full-seeker on the boarding-focused signal alone')
  assert.ok(b.signals.includes('boarding-focused'))
})

test('offers-full non-name-list zero-signal school ALSO survives via "offers full boarding" (full-seeker)', () => {
  // Codex r2 NIT — behaviour-change pin companion to the full-dominant rescue:
  // a column-derived offers-full school (not on any name list) with no other
  // signal now survives for a full-seeker on the hedged offers-full signal.
  const bare = boardingRow('bare-offers', 'Sundale Park School', 'offers-full')
  const emptyStruct = { school_slug: 'bare-offers', sports_profile: null, exam_results: null,
    university_destinations: null, student_community: null, isi_deep_facts: null }
  const out = rankCandidates(
    [bare],
    structMap(emptyStruct),
  ...
  const b = out.find(c => c.slug === 'bare-offers')
  assert.ok(b, 'an offers-full school survives for a full-seeker on the offers-full signal alone')
  assert.ok(b.signals.includes('offers full boarding'))
})

test('unknown-grade school with no other signal is STILL dropped for a full-seeker (no rescue)', () => {
  // Counterpart to the rescue test: the unknown penalty emits no signal, so a
  // no-data school with nothing else does NOT survive (keeps Eton-type
  // unknowns off the list unless they earn a real signal).
  const bare = boardingRow('bare-unknown', 'Nowhere Grange School', null)
  const emptyStruct = { school_slug: 'bare-unknown', sports_profile: null, exam_results: null,
    university_destinations: null, student_community: null, isi_deep_facts: null }
  const out = rankCandidates(
    [bare],
    structMap(emptyStruct),
  ...
  assert.equal(out.length, 0, 'unknown-grade + no other signal → dropped (fails open on filter, no signal to survive on)')
})

test('ranker tilt does NOT fire for day/weekly seekers (full-seeker-only)', () => {
  const out = rankCandidates(
    [B_FULLDOM],
    BOARDING_STRUCTS,
    { parent: { boarding_pref: 'weekly', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
  ...
  const fd = out.find(c => c.slug === 'b-fulldom')
  assert.ok(fd, 'full-dominant school kept for a weekly-seeker')
  assert.ok(!fd.signals.includes('boarding-focused'),
    'boarding-focused tilt is full-seeker-only — must not fire for a weekly-seeker')
})

// ════════════════════════════════════════════════════════════════════════════
// Phase 3 (2026-07-06 scorer pool bugs) — adaptive region floor.
// The region hard filter drops out-of-bucket schools; the floor re-admits some
// of them (boarding-aware, flagged honestly) when a constrained bucket
// (north / scotland-wales) narrows below REGION_FLOOR_TRIGGER (default 8).
// Defaults: TRIGGER 8, REFILL 12 (env NANA_REGION_FLOOR_{TRIGGER,REFILL}).
// ════════════════════════════════════════════════════════════════════════════

// Build N out-of-region boarders in one bucket. No struct needed — the floor's
// honest signal keeps them past the no-signal filter on its own.
function boarders(prefix, n, grade, region) {
  return Array.from({ length: n }, (_, i) =>
    boardingRow(`${prefix}-${i}`, `${prefix.replace(/-/g, ' ')} ${i}`, grade, { region }))
}
const OUT_OF_REGION_FLAG = 'outside your region — boards'

test('region floor: DOES NOT fire when the in-region pool is healthy (>= trigger)', () => {
  // 8 in-bucket (south-west / Somerset) schools clears the trigger, so the
  // one Yorkshire (north) school is hard-dropped, not re-admitted.
  const inRegionPool = boarders('sw', 8, 'offers-full', 'Somerset')
  const outsider     = boardingRow('north-1', 'North One', 'full-dominant', { region: 'Yorkshire' })
  const out = rankCandidates(
    [...inRegionPool, outsider],
    structMap(),
    { parent: { boarding_pref: 'full', home_region: 'south-west' }, child: UNIV_CHILD, excludeSlugs: [] },
    20,
  )
  assert.equal(out.length, 8, 'exactly the 8 in-region schools (no re-admit)')
  assert.ok(!out.some(c => c.slug === 'north-1'), 'out-of-region school hard-dropped when pool healthy')
  assert.ok(!out.some(c => c.signals.includes(OUT_OF_REGION_FLAG)),
    'no floor signal should appear when the floor did not fire')
})

test('region floor: boarding-seeker re-admits out-of-region boarders, flagged honestly', () => {
  // Constrained north pool (1 in-region) + 3 out-of-region boarders → floor
  // engages and re-admits all 3, each flagged "outside your region — boards".
  const northSchool = boardingRow('north-home', 'Northern Home School', 'full-dominant', { region: 'Yorkshire' })
  const outsiders   = boarders('sw', 3, 'offers-full', 'Somerset')
  const out = rankCandidates(
    [northSchool, ...outsiders],
    structMap(),
    { parent: { boarding_pref: 'full', home_region: 'north' }, child: UNIV_CHILD, excludeSlugs: [] },
    20,
  )
  assert.ok(out.some(c => c.slug === 'north-home'), 'in-region school retained')
  const readmitted = out.filter(c => c.slug.startsWith('sw-'))
  assert.equal(readmitted.length, 3, 'all 3 out-of-region boarders re-admitted')
  assert.ok(readmitted.every(c => c.signals[0] === OUT_OF_REGION_FLAG),
    `every re-admit leads with the honest flag; got ${JSON.stringify(readmitted.map(c => c.signals))}`)
  // A re-admit gets NO region boost, so it must not carry a "<region> region" chip.
  assert.ok(readmitted.every(c => !c.signals.some(s => s.endsWith(' region'))),
    're-admitted schools must not claim an in-region chip')
})

test('region floor: refill caps at REFILL and prefers genuine boarders (grade priority)', () => {
  // 1 in-region + 13 out-of-region (12 offers-full first, 1 full-dominant LAST).
  // Cap = REFILL(12) - inRegion(1) = 11 re-admits. Pure array order would drop
  // the trailing full-dominant; grade-priority must keep it.
  const northSchool = boardingRow('north-home', 'Northern Home School', 'offers-full', { region: 'Yorkshire' })
  const offersFull  = boarders('of', 12, 'offers-full', 'Somerset')
  const fullDom     = boardingRow('fd-last', 'Full Dominant Last', 'full-dominant', { region: 'Somerset' })
  const out = rankCandidates(
    [northSchool, ...offersFull, fullDom],
    structMap(),
    { parent: { boarding_pref: 'full', home_region: 'north' }, child: UNIV_CHILD, excludeSlugs: [] },
    20,
  )
  const readmitted = out.filter(c => c.signals.includes(OUT_OF_REGION_FLAG))
  assert.equal(readmitted.length, 11, 'refill caps at REFILL(12) − inRegion(1) = 11')
  assert.equal(out.length, 12, 'total pool = 1 in-region + 11 re-admits')
  assert.ok(out.some(c => c.slug === 'fd-last'),
    'grade priority keeps the trailing full-dominant over an offers-full school')
})

test('region floor: day-seeker gets NO re-admit (honest short in-region list)', () => {
  // North DAY family with a thin pool: the floor does NOT re-admit anything —
  // coarse region buckets can't guarantee a day school is commutable (a
  // Birmingham family being offered a Cumbria school ~3hr away was the exact
  // gate failure that drove the boarding-only design). Both out-of-region
  // schools are dropped; only the in-region north school survives.
  const northSchool = boardingRow('north-home', 'Northern Home School', 'day-only', { region: 'Yorkshire' })
  const midlands    = boardingRow('mid-1',  'Midlands Day',  'day-only', { region: 'Worcestershire' })
  const london      = boardingRow('lon-1',  'London Day',    'day-only', { region: 'London' })
  const out = rankCandidates(
    [northSchool, midlands, london],
    structMap(academicStruct('north-home'), academicStruct('mid-1'), academicStruct('lon-1')),
    { parent: { boarding_pref: 'day', home_region: 'north' }, child: UNIV_CHILD, excludeSlugs: [] },
    20,
  )
  assert.ok(out.some(c => c.slug === 'north-home'), 'in-region day school retained')
  assert.ok(!out.some(c => c.slug === 'mid-1'), 'out-of-region midlands day school NOT re-admitted for a day family')
  assert.ok(!out.some(c => c.slug === 'lon-1'), 'out-of-region London day school NOT re-admitted for a day family')
  assert.ok(!out.some(c => c.signals.includes(OUT_OF_REGION_FLAG)),
    'no boarding re-admit flag on a day family')
})

test('region floor: honest flag survives the top-5 signal slice (unshift, not push)', () => {
  // A re-admitted boarder loaded with sport + academic signals must still
  // surface the honesty flag — it is unshifted so the .slice(0,5) can't cut it.
  const richStruct = {
    school_slug: 'rich-sw',
    sports_profile: { rugby: { competitive_tier: 'national-elite' }, tennis: { competitive_tier: 'national-strong' } },
    exam_results: { a_level: { pct_a_star: 55 } },
    university_destinations: { oxford_count: 4, cambridge_count: 4 },
    student_community: { total_pupils: 350 },
    isi_deep_facts: null,
  }
  const rich = boardingRow('rich-sw', 'Rich South-West School', 'full-dominant', { region: 'Somerset' })
  const out = rankCandidates(
    [rich],
    structMap(richStruct),
    {
      parent: { boarding_pref: 'full', home_region: 'north', class_size_pref: 'very-important' },
      child:  { goal_orientation: 'university_track', interests_sports: [{ sport: 'rugby', level: 'national' }, { sport: 'tennis', level: 'regional' }] },
      excludeSlugs: [],
    },
    20,
  )
  const r = out.find(c => c.slug === 'rich-sw')
  assert.ok(r, 'the sole re-admitted school is returned')
  assert.ok(r.signals.length <= 5, `signals still capped at 5, got ${r.signals.length}`)
  assert.ok(r.signals.includes(OUT_OF_REGION_FLAG),
    `honesty flag survives the top-5 slice; got ${JSON.stringify(r.signals)}`)
})

test('region floor: home_region "anywhere" skips region filter entirely (no floor, no flags)', () => {
  // explicitHomeRegion resolves to null for 'anywhere' → the whole region
  // block (filter + floor) is bypassed and every boarding-capable school stays.
  const pool = [
    boardingRow('a', 'A School', 'offers-full', { region: 'Yorkshire' }),
    boardingRow('b', 'B School', 'offers-full', { region: 'London' }),
    boardingRow('c', 'C School', 'offers-full', { region: 'Somerset' }),
  ]
  const out = rankCandidates(
    pool, structMap(academicStruct('a'), academicStruct('b'), academicStruct('c')),
    { parent: { boarding_pref: 'full', home_region: 'anywhere' }, child: UNIV_CHILD, excludeSlugs: [] },
    20,
  )
  assert.equal(out.length, 3, 'all three kept — no region filter under anywhere')
  assert.ok(!out.some(c => c.signals.includes(OUT_OF_REGION_FLAG)),
    'no floor flags when the region block is skipped')
})

test('region floor: same-grade/same-confidence re-admits break ties by slug (deterministic slice)', () => {
  // Codex r1 P2 regression guard. 13 identical-grade, identical-confidence
  // out-of-region boarders. inRegion=1 → need = REFILL(12) − 1 = 11, so 2 must
  // drop. Without the slug tie-break the survivors would depend on DB row
  // order; with it, the lexicographically-largest slugs (tie-l, tie-m) drop
  // deterministically.
  const north = boardingRow('north-home', 'Northern Home School', 'offers-full', { region: 'Yorkshire' })
  const pool = 'abcdefghijklm'.split('').map(ch =>
    boardingRow(`tie-${ch}`, `Tie ${ch}`, 'offers-full', { region: 'Somerset', confidence_score: 50 }))
  const out = rankCandidates(
    [north, ...pool],
    structMap(),
    { parent: { boarding_pref: 'full', home_region: 'north' }, child: UNIV_CHILD, excludeSlugs: [] },
  ...
  const readmitted = out.filter(c => c.slug.startsWith('tie-')).map(c => c.slug)
  assert.equal(readmitted.length, 11, 'REFILL(12) − inRegion(1) = 11 kept')
  assert.ok(!readmitted.includes('tie-l') && !readmitted.includes('tie-m'),
    `largest-slug ties drop deterministically; got ${JSON.stringify(readmitted.sort())}`)
  assert.ok(readmitted.includes('tie-a') && readmitted.includes('tie-k'),
    'smallest-slug ties are retained')
})
