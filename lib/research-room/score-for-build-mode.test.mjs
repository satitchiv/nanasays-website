...
test('Codex r1 P2: priority + ethos match survive top-5 slice when many other signals fire', () => {
  // Regression guard for the 2026-05-21 truncation bug.
  // Before unshift fix: a school with 5+ pre-existing region/sport/academic
  // signals would drop its `sport-priority match` and `ethos match` chips
  // because they were pushed last and chopped by `signals.slice(0, 5)`.
  // After fix: both chips MUST appear at the front of the signals array.
  // Using `.some(...)` to assert presence (as the existing tests do) would
  // still pass under the old buggy push order — hence this order assertion.
  const STRUCT_LOADED = {
    school_slug:            'rugby-school',
  ...
  const out = rankCandidates(
    ...
    structMap(STRUCT_LOADED),
    {
      parent: {
        ...
        home_region:  'midlands', // Warwickshire is in midlands bucket → adds region chip
      },
      child:  { interests_sports: [{ sport: 'rugby', level: 'county' }] },
      excludeSlugs: [],
    },
  ...
  // Order invariant — priority unshifts last so it leads; ethos unshifts before it so it's second.
  assert.equal(rugby.signals[0], 'sport-priority match',          `expected signals[0] to be sport-priority match, got: ${JSON.stringify(rugby.signals)}`)
  assert.equal(rugby.signals[1], 'ethos match (roman catholic)',  `expected signals[1] to be ethos match, got: ${JSON.stringify(rugby.signals)}`)
})
