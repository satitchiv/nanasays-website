...
export async function rankSchools(supabase, args, ctx = {}) {
  ...
  const NEW_DIMS_NEEDING_FACTS = new Set(['safeguarding_integrity', 'weekend_life', 'ethos_match', 'intl_share', 'device_policy']);
  ...
  // Coverage counters surface for the LLM so it can frame "based on N schools
  // that disclose X" honestly when the cohort is thin (P5).
  let eligible      = 0;  // rows considered post-UK filter
  let coveredCount  = 0;  // rows with the required-data fields populated
  const scored = [];
  ...
  return {
    result: {
      ...
      count: ranked.length,
      schools: ranked,
      // P5: coverage transparency. Lets the LLM say "based on the 38 schools
      // that disclose X" instead of implying the whole cohort was ranked.
      coverage_meta: {
        eligible,                 // schools considered in this dimension's UK cohort
        covered: coveredCount,    // schools with the required fields to be scored
        scored : scored.length,   // schools that produced a non-zero score
        returned: ranked.length,  // schools surfaced to the parent
      },
    },
    citations,
    summary: `Ranked ${ranked.length} of ${coveredCount} schools that disclose ${dim.label.toLowerCase()} (${eligible} eligible)`,
  };
}
...
export async function compareSchools(supabase, args, ctx = {}) {
  ...
  // P5 coverage tracking — per-dimension count of how many of the named
  // schools actually had a usable row to score against.
  const dimCovered = Object.fromEntries(dimNames.map((d) => [d, 0]));
  const compared = slugs.map(slug => {
    ...
    for (const dimName of dimNames) {
      ...
      // Only count as covered if the row actually has the required data for
      // this dimension. Mirrors rankSchools' coverage gate (Codex r4 P2).
      if (hasRequiredData(enrichedRow, dimName)) dimCovered[dimName]++;
    }
  ...
  return {
    result: {
      ...
      dimensions: dimNames,
      schools: compared,
      // P5: per-dimension data coverage so the LLM can flag dimensions where
      // one of the named schools has no row to compare against ("Marlborough's
      // pastoral_care data is missing — comparing on academics only").
      coverage_meta: {
        requested: slugs.length,
        per_dimension: dimCovered,
      },
    },
    citations,
...
export async function getSchoolFacts(supabase, args) {
  ...
  const cols = ['school_slug', ...requested].join(', ');
...
export const TOOL_DESCRIPTIONS = {
  ...
  searchSchoolText: {
    description: 'Vector search across school website text. With slug = single school. Without slug = all UK schools. Use for narrative/qualitative questions ("what is the culture like").',
  ...
  getSchoolFacts: {
    description: 'Fetch specific structured fields for one school. Use AFTER rankSchools/compareSchools to enrich top candidates. ALWAYS pass `fields` matching the question — for sports questions include `sports_profile` explicitly (it is NOT in the default; sports_profile alone is ~25KB). Common fields: exam_results, sports_profile, university_destinations, fees_by_grade, scholarships_available, bursary_note, admissions_format, pastoral_care, pastoral_model, wellbeing_staffing, policies_summary, location_profile, facilities. For safeguarding/red-flag questions, use searchSafeguarding first.',
    args: { slug: 'string', fields: 'string[] from allowed set (default if omitted: exam_results, fees_min, fees_max, university_destinations — sports_profile is NOT in default)' },
  },
  searchSafeguarding: {
