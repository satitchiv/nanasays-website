...
const ALL_STRUCTURED_COLS = [
  ...
  'location_profile',
  // D-step-1 fix (2026-05-26): subject_strengths column is required for the
  // now-enabled subject_strengths dim to read items[] + category. Without it
  // in the SELECT, hasRequiredData filters every row out and rankSchools
  // returns 0 schools for "best schools for maths"-style questions. Smoke
  // surfaced this immediately — the original D-step-1 commit only flipped
  // enabled + wired ctx, missing the SELECT.
  'subject_strengths',
];
