...
async function loadSchoolMeta(supabase, slugs) {
  ...
  // Include gender + age_min/max so getSchoolFacts can surface them in
  ...
  const { data: rows } = await supabase
    .from('schools')
    .select('slug, name, country, boarding_type, gender, age_min, age_max')
    .in('slug', slugs);
...
// ── rankSchools ─────────────────────────────────────────────────────────────
export async function rankSchools(supabase, args) {
  ...
  for (const row of rows || []) {
    ...
    if (!hasRequiredData(row, dimension)) continue;
    const score = dim.rank(row);
    if (score <= 0) continue;
    scored.push({ slug: row.school_slug, score, row });
  }
...
export async function filterSchools(supabase, args) {
  ...
  const [{ data: status }, { data: schools }] = await Promise.all([
    ...
    supabase
      .from('schools')
      .select('slug, name, boarding_type')
      .in('slug', ssdSlugs),
  ]);
  ...
  for (const row of ssd || []) {
    ...
    // Gender filter: only reject on EXPLICIT mismatch — most UK rows have null
    // gender (data quality gap). Including unknowns is preferred over returning
    // an empty list. When the data fills in, this becomes strict automatically.
    if (gender) {
      const want = gender.toLowerCase().replace(/[^a-z]/g, '');
      const got  = (meta.gender || '').toLowerCase().replace(/[^a-z]/g, '');
      if (got) {
    ...
    // Boarding filter: reject if EXPLICITLY day-only; include unknowns.
    if (boarding === true) {
      ...
      if (bt && /day school only|day only/.test(bt))  continue;
    }
...
// ── compareSchools ──────────────────────────────────────────────────────────
export async function compareSchools(supabase, args) {
  ...
  const compared = slugs.map(slug => {
    ...
    for (const dimName of dimNames) {
      ...
      if (!dim || !row) { perDim[dimName] = null; continue; }
      perDim[dimName] = {
        score:     Math.round(dim.rank(row) * 10) / 10,
        summary:   dim.format(row, school),
        citations: dim.citations(row) || [],
      };
    }
...
export const TOOL_DESCRIPTIONS = {
  rankSchools: {
    description: 'Top-N UK schools ranked by a dimension. Dimensions: tennis_strength, rugby_standing, academic_strength, fees_value, pastoral_model. Use when picking the BEST out of MANY — or out of a filtered/shortlisted scope via restrict_to_slugs. Returns score + summary per school.',
