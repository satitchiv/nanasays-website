...
// Dimension-aware column selection (Codex P1.2 2026-05-26). Helpers live
// in structured-cols.mjs so the contract test can import them without
// dragging tools.js's transitive .ts deps. See that file for design notes.
export {
  BASE_STRUCTURED_COLS,
  BIG_COLS_BY_DIM,
  BIG_COLS,
  colsForRankSchools,
...
import { detectGenericSport } from './generic-sport.mjs';
...
// ── Notion class-size bundle loader (2026-05-25 small_class_size slice) ────
// rankSchools needs class_size data from school_notion_backfill.parsed, NOT
// from school_structured_data. We can't add `parsed` to ALL_STRUCTURED_COLS
// because (a) it lives on a different table and (b) the full Notion blob is
// big + contains fees we strip server-side. So we mirror loadDimFactsBundles
// here: fetch + project on-demand for the slug set, fold into the row before
// rank() runs.
//
// Returns Map<slug, { class_size_facts: { senior?, sixth?, average? } }>.
// Slugs with no clean Notion row OR no class_size sub-fields are absent
// from the map (so the rankSchools `score <= 0` filter drops them via the
// dimension's null short-circuit).
//
// Status filter mirrors getSchoolFacts (clean OR matched). The projector
// applies the same SSD-wins + scalar/range normalisation that the chat
// surface uses, so ranking sees identical numbers to what parents read in
// the verified-facts block.
export async function loadNotionClassSizeBundles(supabase, slugs) {
  ...
  if (error) {
    console.error('[loadNotionClassSizeBundles]', error.message);
    return bundles;
  }
  for (const r of rows || []) {
    const proj = projectNotionBackfill(r.parsed, null);
    if (!proj || !proj.class_size || typeof proj.class_size !== 'object') continue;
    // Empty {} after projector drops invalid sub-fields → skip (the dim
    // null-short-circuits, but skipping here avoids storing dead entries).
    if (Object.keys(proj.class_size).length === 0) continue;
    bundles.set(r.school_slug, { class_size_facts: proj.class_size });
  }
  return bundles;
}
...
// 2026-07-05 latency fix: loadUkSlugSet was 26 SEQUENTIAL round-trips
// (~6.1s live, measured) on EVERY global rankSchools call — the single
// largest parent-visible delay in chat, dwarfing the actual data fetch
// (~0.4s). Two changes:
//   1. count-first pagination → remaining pages fetch in PARALLEL
//   2. module-scope TTL cache — the UK-evidence set only changes when an
//      extractor build lands, so 10-minute staleness is harmless. Cache
//      persists per warm serverless instance; cold starts pay once.
// On any page error the partial set is still returned (same failure mode
// as the old sequential break) but NOT cached, so the next call retries.
// Exported for tests (uk-slug-cache.test.mjs); _resetUkSlugCache is
// test-only.
let _ukSlugCache = null; // { set: Set<string>, at: number }
const UK_SLUG_TTL_MS = 10 * 60 * 1000;

export function _resetUkSlugCache() {
  _ukSlugCache = null;
}

export async function loadUkSlugSet(supabase) {
  if (_ukSlugCache && Date.now() - _ukSlugCache.at < UK_SLUG_TTL_MS) {
    return _ukSlugCache.set;
  }
  const PAGE = 1000;
  const { data: first, count, error } = await supabase
    .from('schools_status')
    .select('school_slug', { count: 'exact' })
    .eq('is_uk_evidence', true)
    .range(0, PAGE - 1);
  if (error || !first?.length) return new Set();
  const all = [...first];
  let pageFailed = false;
  if (typeof count === 'number' && count > PAGE) {
    const pageQueries = [];
    for (let offset = PAGE; offset < count; offset += PAGE) {
      pageQueries.push(
        supabase
          .from('schools_status')
          .select('school_slug')
    ...
    for (const r of await Promise.all(pageQueries)) {
      if (r.error || !r.data) { pageFailed = true; continue; }
      all.push(...r.data);
    }
  }
  const set = new Set(all.map(r => r.school_slug));
  if (!pageFailed) _ukSlugCache = { set, at: Date.now() };
  return set;
}
...
export async function rankSchools(supabase, args, ctx = {}) {
  ...
  // Codex P1.2 (2026-05-26): dimension-aware SELECT — fetch only the
  // columns this dim actually reads. Drops sports_profile / subject_strengths
  // JSONB (25-30KB/school each) when the dim doesn't need them.
  const cols = colsForRankSchools(dimension);
  let q = supabase.from('school_structured_data').select(cols.join(', ')).range(0, 9999);
  ...
  // 2026-05-25 small_class_size: needs Notion class_size bundle folded onto
  // each row. Separate loader from school_facts because the source table is
  // school_notion_backfill (different schema, hand-curated, projector applies
  // SSD-wins). Kept as its own NEW_DIMS_NEEDING_NOTION set so future Notion-
  // backed dims can opt-in without colliding with the school_facts loader.
  const NEW_DIMS_NEEDING_NOTION = new Set(['small_class_size']);
  let notionBundles = new Map();
  if (NEW_DIMS_NEEDING_NOTION.has(dimension)) {
    const candidateSlugs = (rows || []).map((r) => r.school_slug);
    notionBundles = await loadNotionClassSizeBundles(supabase, candidateSlugs);
  }

  const scored = [];
  for (const row of rows || []) {
    if (ukSlugs && !ukSlugs.has(row.school_slug)) continue;
    // Fold _facts + Notion bundles into the row before required-data + rank checks.
    const factsBundle  = factsBundles.get(row.school_slug);
    const notionBundle = notionBundles.get(row.school_slug);
    const enrichedRow  = (factsBundle || notionBundle)
      ? { ...row, ...(factsBundle || {}), ...(notionBundle || {}) }
      : row;
  ...
  const ranked = top.map(({ slug, score, row }) => {
    ...
    return {
      ...
      // Pass ctx so dims that key off it (subject_strengths reads
      // ctx.subject_intents) can emit per-school evidence strings. Without
      // ctx, subject_strengths.format fell back to "School — subjects." with
      // no item counts, and the model lost the signal it needed to rank.
      summary: dim.format(row, school, ctx),
      citations: dim.citations(row, ctx) || [],
    };
  });
...
// ── filterSchools ───────────────────────────────────────────────────────────
// has_sport reads schools_status has_<sport>_extracted flags, which exist
// ONLY for the five extracted sports. 2026-07-05 guardrail: any other sport
// (golf, swimming, …) used to silently match zero schools — the model then
// answered "no schools offer golf", which is false. Now the tool returns an
// explicit redirect to the <sport>_offering rankSchools path instead.
const HAS_SPORT_FLAG_SPORTS = new Set(['tennis', 'football', 'rugby', 'cricket', 'hockey']);

export async function filterSchools(supabase, args) {
  const { fees_max, fees_min, gender, boarding, has_sport } = args || {};

  if (has_sport && !HAS_SPORT_FLAG_SPORTS.has(String(has_sport).toLowerCase().trim())) {
    // Resolve alias forms through the registry ('ping pong' → table_tennis_offering,
    // 'horse riding' → equestrian_offering) — naive slugification would name
    // dimensions that don't exist and burn a recovery turn.
    const generic = detectGenericSport(String(has_sport));
    return {
      result: {
        error:
          `has_sport supports only: ${[...HAS_SPORT_FLAG_SPORTS].join(', ')}. ` +
          (generic
            ? `For "${has_sport}", call rankSchools with dimension "${generic.dimension}" ` +
              `(generic sport-offering dimension) — optionally after filterSchools WITHOUT has_sport, chaining via restrict_to_slugs.`
            : `"${has_sport}" matches no tracked sport dimension — do not guess a dimension name; ` +
              `tell the parent we have no ranked data for that sport.`),
      },
      citations: [],
      summary: `filterSchools: has_sport '${has_sport}' not flag-backed — use <sport>_offering ranking`,
    };
  }
...
export async function compareSchools(supabase, args, ctx = {}) {
  ...
  // Codex P1.2 (2026-05-26): dimension-aware SELECT — union of cols needed
  // by the requested dims. Multi-dim comparisons including a sports + a
  // subject dim still pull both blobs; single-dim comparisons drop the
  // unused blob.
  const compareCols = colsForCompareSchools(dimNames);
  const { data: rows } = await supabase
    .from('school_structured_data')
    .select(compareCols.join(', '))
    .in('school_slug', slugs);
  ...
  // 2026-05-25 small_class_size: parity with rankSchools — fold Notion
  // class_size bundle so compareSchools can score the new dim. Without this,
  // a shortlist comparison including small_class_size returns score=0 for
  // every school (Codex r1 P1b).
  const NEW_DIMS_NEEDING_NOTION = new Set(['small_class_size']);
  const needsNotion = dimNames.some((d) => NEW_DIMS_NEEDING_NOTION.has(d));
  const notionBundles = needsNotion ? await loadNotionClassSizeBundles(supabase, slugs) : new Map();

  const compared = slugs.map(slug => {
    const row = rowBy.get(slug);
    const school = meta.get(slug) || { slug, name: slug };
    const factsBundle  = factsBundles.get(slug);
    const notionBundle = notionBundles.get(slug);
    const bundleMerge  = { ...(factsBundle || {}), ...(notionBundle || {}) };
    const hasBundle    = Object.keys(bundleMerge).length > 0;
    const enrichedRow = (row && hasBundle)
      ? { ...row, ...bundleMerge }
      : (row || (hasBundle ? { school_slug: slug, ...bundleMerge } : null));
    const perDim = {};
    for (const dimName of dimNames) {
      ...
      perDim[dimName] = {
        ...
        // Parity with rankSchools: pass ctx so subject_strengths.format
        // can emit per-subject evidence (the 5-arg legacy signature was
        // never reached from here either).
        summary:   dim.format(enrichedRow, school, ctx),
        citations: dim.citations(enrichedRow, ctx) || [],
      };
    }
...
export const TOOL_DESCRIPTIONS = {
  rankSchools: {
    description: 'Top-N UK schools ranked by a dimension. Dimensions: tennis_strength, rugby_standing, football_strength, cricket_strength, hockey_strength, academic_strength, fees_value, pastoral_care, inclusive_culture, pastoral_model, proximity_to_heathrow (closest schools to Heathrow Airport — uses SSD location_profile.airports[]), small_class_size (smallest typical class size — uses Notion class_size senior/sixth/average), plus <sport>_offering for any OTHER sport (golf_offering, swimming_offering, rowing_offering, netball_offering, … — see DIMENSIONS list), plus sport_breadth (overall sport-programme strength when NO specific sport is named). Use when picking the BEST out of MANY — or out of a filtered/shortlisted scope via restrict_to_slugs. Returns score + summary per school.',
  ...
  filterSchools: {
    description: 'Hard filter UK schools by criteria. Returns matching slugs/names only — no scores or detailed evidence. For recommendation/ranking questions, chain into rankSchools(restrict_to_slugs). For pure list/count questions, filterSchools alone may be enough. has_sport accepts ONLY the five extracted sports; for any other sport (golf, swimming, rowing, …) apply the non-sport filters here, then rankSchools with the <sport>_offering dimension + restrict_to_slugs.',
    args: { fees_max: 'number', fees_min: 'number', gender: 'co-ed|boys|girls', boarding: 'boolean', has_sport: 'tennis|football|rugby|cricket|hockey ONLY (other sports: use rankSchools <sport>_offering instead)' },
  },
  searchSchoolText: {
