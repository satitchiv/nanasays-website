...
// 2026-05-25 PM (browser smoke #5, Codex follow-up): tool-name URL deterministic
// scrubber. Browser smoke caught the model writing `([source](https://rankSchools))`
// despite the prose system prompt's HARD RULES forbidding hallucinated URLs.
// Codex r1 said prompt-only fix is insufficient — model violated similar rules
// before. This scrubber runs AFTER extractMeta and BEFORE the prose is returned
// to the caller; it strips any inline `([source](URL))` whose host is a tool
// name OR whose URL isn't in the caller-supplied allowedUrls set.
//
// Used by `runProseFromIntent` post-meta-extract. Exported for unit tests.
const FORBIDDEN_CITATION_HOSTS = new Set([
  'rankschools',
  'getschoolfacts',
  'compareschools',
  'filterschools',
...
// Inline `[source](X)` matcher — captures both:
//   - Parenthesized form:  "Eton is X ([source](...))" → typical
//   - Bare form:           "Eton [source](...)"        → Codex r3 P1
// Codex r4 P1.1 (2026-05-25 PM): regex now captures ANY non-space, non-paren
// target inside the `()` — not just `https?://`. The replacer validates the
// target via `new URL()` and drops non-URL shapes like `[source](slug-name)`
// or `[source](rankSchools)` (which the system-prompt explicitly forbids but
// the earlier https-only regex didn't catch at all). Outer "(...)" wrapper
// stays optional. Leading whitespace + the whole match consumed when invalid
// so trailing punctuation stays clean.
const INLINE_SOURCE_RE = /\s*\(?\[source\]\(([^\s)]+)\)\)?/gi;

// 2026-05-25 PM Codex r2 P1: empty allowedUrls now means "strip ALL inline
// citations" (not "no enforcement"). The invariant is "only URLs that
// appear verbatim in tool results" — when tools returned no URLs (e.g.
// pure proximity_to_heathrow rank), the model MUST NOT cite anything.
// Previously a hallucinated-but-plausible URL like https://etoncollege.com
// would pass when allowedUrls was empty; that's exactly the failure mode
// the system-prompt rule was supposed to prevent.
...
// empty allow-list means "no URLs valid").
