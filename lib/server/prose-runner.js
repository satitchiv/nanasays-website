...
- Cite source URLs inline as markdown links: ([source](URL)). HARD RULES
  ...
  · TOOL NAMES are NOT URLs. Never write "[source](https://rankSchools)",
    "[source](https://getSchoolFacts)", "[source](https://compareSchools)",
    or any URL whose host is a tool name from this prompt. If the tool's
    "citations" array is empty for a given fact (e.g. the proximity_to_heathrow
    dimension computes distance from internal SSD coordinates and has no
    public source URL), state the fact with NO "[source](...)" markup.
    Example: WRONG — "Eton is 7 miles from Heathrow ([source](https://rankSchools))".
    RIGHT — "Eton is 7 miles from Heathrow."
...
- No headings, no JSON, no code fences, no schema language ("confidence",
  "watch out", "what we don't know"). Write like a human adviser. A plain
  markdown table IS allowed when the parent explicitly asks for a table.
...
// RRV-8 — gap→alternative retrieval routing. getSchoolFacts' `summary`
// always reads "Fetched N fields for <name>" even when every field came
// back null (tools.js has no concept of "found nothing" in its summary
// text). A neighbour block fetched purely for gap-routing must NOT reach
// the model in that state — a confident-sounding header sitting on top of
// null data is exactly the bait that produces an invented alternative.
// This checks the getSchoolFacts result shape ({ data, notion_backfill })
// for any real (non-null) value; only role:'neighbour' tool results are
// ever checked against this (see the tool-result loop in
// runProseFromIntent) since it's the only tool neighbourFactTools() uses.
export function neighbourHasData(result) {
  if (!result || typeof result !== 'object') return false;
  // Recurse — these fields are structured JSONB objects (pastoral_model,
  // wellbeing_staffing, …), not scalars. A non-recursive "has keys" check
  // would let a skeleton like { summary: null, source_url: null } pass as
  // "real data" purely because the key exists. Empty strings don't count
  // as real either.
  ...
  if (data && typeof data === 'object') {
    for (const [k, v] of Object.entries(data)) {
      if (k === 'school_slug') continue;
      if (hasRealValue(v)) return true;
    }
  }
  const nb = result.notion_backfill;
  if (nb && typeof nb === 'object') {
    for (const v of Object.values(nb)) {
      if (hasRealValue(v)) return true;
    }
  }
  return false;
...
// 2026-07-05 — comparison/ranking completeness. Manual testing showed
// comparison answers sometimes covered only the schools with the richest
// data and silently dropped the rest — the exact failure parents read as
// "the table is incomplete". Deterministic fix: for comparison-shaped
// intents, the pass-1 user message carries an explicit coverage rule.
// Exported for the unit test.
export const COMPARE_COMPLETENESS_NOTE =
  'COVERAGE RULE: cover EVERY school in the tool results — one line or bullet each. ' +
  'For each school give its value/evidence on the asked dimension(s); if a school has ' +
  'no data for a dimension, say "no data" for that school explicitly instead of omitting it. ' +
  'Never drop a school because its data is thin.';

const COMPARE_SHAPED_INTENTS = new Set([
  'compare_two_on_dim',
  'shortlist_rank_or_compare',
  'shortlist_quant_facts',
  'top_n_for_dim',
]);

// 2026-07-05 — deterministic table handling. When the parent explicitly
// asks for a table ("build me a table comparing …" / Thai ตาราง), the
// pass-1 message tells the model to actually produce one; without this the
// prose voice rules win and the parent gets paragraphs instead of the
// requested table. Exported for the unit test.
// (?![\s-]+tennis) keeps "table tennis" / "table-tennis" — a registry sport —
// from being read as a table-format request.
export const TABLE_REQUEST_RE = /\btables?\b(?![\s-]+tennis)|ตาราง/i;
export const TABLE_FORMAT_NOTE =
  'TABLE RULE: the parent explicitly asked for a table. Present the core comparison as a ' +
  'markdown table — one row per school, one column per asked dimension (add a short "Why" ' +
  'column when the parent asked why). Fill EVERY cell; write "no data" where the tool ' +
  'results carry nothing for that school+dimension. Keep cell text short (facts, not prose); ' +
  'after the table add at most two sentences of takeaway.';

// RRV-8 — counselor reply shape for single-school worry/fit/fact questions.
// Mirrors the COMPARE_COMPLETENESS_NOTE pattern: a conditional note appended
// to the user message rather than a global system-prompt rule, so it only
// competes for attention on the intents it actually applies to. Comparison-
// shaped intents keep COMPARE_COMPLETENESS_NOTE instead — this is not a
// replacement for that.
const SINGLE_SCHOOL_INTENTS = new Set([
  'safeguarding_or_pastoral',
  'tell_me_about_school',
  'fact_lookup',
]);

export const SINGLE_SCHOOL_SHAPE_NOTE =
  "REPLY SHAPE: this is a single-school question. Acknowledge the parent's concern or question " +
  'in one short sentence, then answer with the strongest fact from the tool results, citing it ' +
  'inline. If there is a genuine gap on the SPECIFIC point asked (not the whole topic), name it ' +
  'honestly in one sentence — never invent to fill it. Close with exactly one concrete next step: ' +
  'a specific tour/admissions question to ask the school directly, unless the note below gives ' +
  'you a better one.';

// Only injected when at least one SHORTLIST NEIGHBOUR tool block survived
// (prose-runner drops neighbour blocks with no real data before they reach
// here — see the tool-result loop below). So "a NEIGHBOUR block above" in
// this note is never a lie the model has to fact-check itself.
export const GAP_ROUTING_NOTE =
  'GAP → ALTERNATIVE: one or more SHORTLIST NEIGHBOUR tool blocks above have real retrieved data. ' +
  'If the honest gap you just named is covered by a NEIGHBOUR block, use that as your one next ' +
  'step instead of a tour question — offer it by name ("[school] does have [X] documented — want ' +
  'me to pull that up?"). Only ever name a school as an alternative when a NEIGHBOUR block backs ' +
  'it; never invent one, and never use a NEIGHBOUR block for anything except gap-routing on THIS ' +
  'question — it is not an invitation to compare schools generally.';

export function buildUserMessage(question, intentMatch, toolBlobs, parentContext, historyContext) {
  const ctxLines = [];
  ...
  const toolBlock = toolBlobs.length
    ? toolBlobs.map(t => {
        const roleTag = t.role === 'neighbour'
          ? ' — SHORTLIST NEIGHBOUR (gap-routing only)'
          : t.role === 'primary'
          ? ' — SCHOOL ASKED ABOUT'
          : '';
        return `═══ TOOL [${t.name}] (${t.summary || ''})${roleTag} ═══\n${t.compact}`;
      }).join('\n\n')
    : '(no tool results)';

  const hasNeighbourData = toolBlobs.some(t => t.role === 'neighbour');
  ...
  return [
    ...
    `\n${toolBlock}\n`,
    ...(COMPARE_SHAPED_INTENTS.has(intentMatch.intent) ? [COMPARE_COMPLETENESS_NOTE] : []),
    ...(SINGLE_SCHOOL_INTENTS.has(intentMatch.intent) ? [SINGLE_SCHOOL_SHAPE_NOTE] : []),
    ...(SINGLE_SCHOOL_INTENTS.has(intentMatch.intent) && hasNeighbourData ? [GAP_ROUTING_NOTE] : []),
    ...(TABLE_REQUEST_RE.test(question) ? [TABLE_FORMAT_NOTE] : []),
...
const COMPARISON_INTENTS = new Set([
  'compare_two_on_dim',
  'shortlist_rank_or_compare',
  // Codex r3 P1.2 (2026-05-25): notion-sidecar quant-fact short-circuit also
  // produces a shortlist-comparison-style answer ("rank them by pupil count",
  // "compare on class size"). Without this entry, Pass-2 skips proposed-action
  // extraction so Slice 6 re-rank / lens / add-row proposals never appear for
  // quant-fact shortlist asks.
  'shortlist_quant_facts',
  'top_n_for_dim',
]);
...
// 2026-05-25 PM (browser smoke #5, Codex r5 P1.2 extract): scrubInvalidCitations
// + the FORBIDDEN_CITATION_HOSTS + INLINE_SOURCE_RE helpers now live in the
// client-safe `lib/prose-citation-scrubber.js` so the streaming bubble
// (NanaBubble.tsx) can apply the same forbidden-host strip to `streamBuf`,
// eliminating the ~1s flash of `[source](https://rankSchools)` parents saw
// during streaming. The strict server-side scrub is unchanged — it still runs
// post-extractMeta with the trusted allow-list. See module header for the
// strict-vs-lax mode distinction.
import {
  FORBIDDEN_CITATION_HOSTS,
  scrubInvalidCitations,
} from '../prose-citation-scrubber.js';

// Re-export the server-strict helper so existing callers + the sandboxed
// unit-test file (which reads prose-runner.js source) keep their import path.
export { scrubInvalidCitations };

// Filter a model-emitted citations array down to URLs we trust. Same allow-list
// + tool-name rejection as scrubInvalidCitations (strict empty-list semantics:
// empty allow-list means "no URLs valid"). Lives in prose-runner.js (not the
// shared module) because it's only consumed server-side post-meta-extract.
export function filterCitations(modelCitations, allowedUrls) {
  ...
  return modelCitations.filter(u => {
    ...
    if (FORBIDDEN_CITATION_HOSTS.has(host)) return false;
    if (!allowed.has(u)) return false;
    return true;
  });
}
...
export async function* runProseFromIntent({ supabase, question, intentMatch, callClaude, opts = {} }) {
  ...
  //
  // D-step-1 follow-up fix (2026-05-26): also thread subject_intents derived
  // from the parent's question text. Smoke surfaced that the chat surface
  // routes through THIS prose-runner path (not agentic-loop) for shortlist-
  // locked sessions, so the subject_strengths dim was reading ctx with no
  // subject_intents and rank() short-circuited to 0 for all but one school.
  // Mirrors the agentic-loop toolCtx shape.
  const toolCtx = {
    parent: opts.pack?.parent ?? null,
    subject_intents: extractSubjectIntents(question),
  };
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    if (s.status === 'fulfilled') {
      ...
      // RRV-8: a neighbour block with no real data gets dropped before it
      // ever reaches the model — see neighbourHasData() doc comment. Still
      // counts toward successCount (the fetch itself succeeded) so it can't
      // trip the "all tools failed" error path.
      if (def.role === 'neighbour' && !neighbourHasData(r.result)) {
        yield {
          type: 'tool_call',
          name: def.name,
          args: def.args,
          status: 'completed',
          result_summary: `${r.summary} — no usable data, dropped from context`,
        };
        successCount++;
        continue;
      }
  ...
  // P4 (Codex 2026-05-08): telemetry-only citation validation. We don't
  // reject answers yet — just log when the model cites URLs / slugs that
  // aren't in the pack allowlist. Promote to rejection in a follow-up PR.
  if (opts.pack) {
    try {
      ...
      if (typeof validator === 'function') {
        ...
        if (!result.ok) {
          console.warn(
            `[prose-runner:p4-validator] ok=false hallucinated_urls=${result.hallucinated_urls.length} out_of_scope_slugs=${result.out_of_scope_slugs.length} notes=${result.notes.join('; ')}`,
          );
        }
      }
    } catch (e) {
      // Validator failure must NEVER break the chat. Log + carry on.
      console.error('[prose-runner:p4-validator] failed:', e?.message ?? e);
    }
  }
  ...
  const { prose: rawProse, meta } = extractMeta(raw);
  // 2026-05-25 PM (browser smoke #5, Codex r1 follow-up): deterministic
  // scrubber for inline `([source](URL))` markdown the model may have
  // hallucinated (e.g. `https://rankSchools`). Prompt rule alone was
  // insufficient — model violated despite "only use URLs that appear
  // verbatim in tool results" directive. allCitations is the trusted
  // allow-list of URLs the tools actually returned.
  const prose = scrubInvalidCitations(rawProse, allCitations);
  ...
  const citations        = Array.isArray(meta?.citations)
    ? filterCitations(meta.citations, allCitations)
    : [...allCitations];
