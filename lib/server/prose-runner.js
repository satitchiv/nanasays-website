...
D. {"proposed_actions": {"<proposal_id>": {"kind": "propose_create_topic_lens", "topic_name": "Rugby", "lens_name": "Rugby", "base_lens_kind": "general", "embedded_rows": [{"row_name": "...", "group_name": "...", "cell_data": {"<slug>": {"value": "..."}, ...}}, ...], "visible_base_rows": ["School name", "Day fees", ...]}}}
...
- embedded_rows: 2..8 entries. Each entry = a NEW row about the topic. SAME field rules as B (row_name 1..80, group_name 1..40, cell_data with shortlist slugs and ≥ 2 real values). Use group_name = topic_name (e.g. "Rugby") so the rows cluster visually.
...
EXAMPLE D — parent asks "create a lens for rugby" and answer surfaces rugby-specific rows:
...
async function extractProposedActionsTwoPass({
  ...
  const empty = (reason, usage = null) => ({
    ...
    kept:    { total: 0, byKind: { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0, propose_create_topic_lens: 0 } },
    usage,
  });
  ...
  if (parsed2 == null) {
    return {
      ...
      kept:  result1.kept || { total: 0, byKind: { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0, propose_create_topic_lens: 0 } },
      usage: usageBoth,
    };
  }
...
// Slice 6 r2 P2: prompt explicitly says "Choose ONE of THREE outputs / At most
...
function validateCreateTopicLensProposal(p, ctx) {
  ...
  for (const r of p.embedded_rows) {
    ...
    if (Object.keys(cleaned).length === 0)                   return { ok: false, reason: 'embedded_row_empty_after_filter' };
    ...
    if (realCellCount < 2)                                   return { ok: false, reason: 'embedded_row_too_few_real_cells' };
...
export function validateProposedActions(meta, ctx) {
  ...
  const reasons = {};
  const keptByKind = { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0, propose_create_topic_lens: 0 };
  let totalSeen = 0;
