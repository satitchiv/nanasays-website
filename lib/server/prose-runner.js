...
Choose ONE of FOUR outputs. At most ONE proposal per call (cap = 2 globally, but emit 1).
...
D. {"proposed_actions": {"<proposal_id>": {"kind": "propose_create_lens", "lens_name": "...", "lens_question": "...", "view_spec": {"base_lens_kind": "general", "weights": {"<row_name>": 0..5, ...}}}}}
...
— C (propose_re_rank) when the answer reorders the shortlist using EXISTING comparison rows the parent is already viewing — different weighting of the SAME dimensions, no new dimension introduced. Example: "rank these by academics + value-for-money" → reweight GCSE 9-7 + per-term fees, no new row. Default to C over D unless the parent literally asked to "save" or "create" a lens.

— D (propose_create_lens) ONLY when the parent's question explicitly asks for a NAMED, SAVED perspective ("create a lens that prioritises pastoral care", "save a lens for boarding-distance"). Otherwise prefer C.
...
D. propose_create_lens:
- kind: "propose_create_lens"
- lens_name: 1..40 chars. Title-case, distinctive ("Pastoral-first", "Academics + value"). No emoji. Must be unique per session — if "Pastoral first" already exists, pick a different name.
- lens_question: 1..240 chars (optional). The framing question the lens answers ("Which school has the strongest pastoral architecture?").
- view_spec: same shape as C above.

═══ SEEDED ROWS (do NOT duplicate as add_row; OK to reweight in C/D) ═══
...
EXAMPLE D — parent asked to SAVE a pastoral lens:
"Through a pastoral-first lens, Wellington's wellbeing-officer-per-house structure leads, with Marlborough close behind on house autonomy, and Eton further back on pastoral architecture though strong on tutorial system."

PARENT QUESTION: "create a lens that prioritises pastoral care"
ACTIVE BASE LENS: general
ACTIVE ROWS: Total pupils, Boarding ratio, House system, Pastoral wellbeing offering

You output:
{"proposed_actions":{"pastoral_first":{"kind":"propose_create_lens","lens_name":"Pastoral-first","lens_question":"Which school has the strongest pastoral architecture?","view_spec":{"base_lens_kind":"general","weights":{"Pastoral wellbeing offering":5,"House system":4,"Boarding ratio":3}}}}}
...
// emit propose_re_rank / propose_create_lens with row_name keys that
// match what the parent currently sees. Without these, the model would
// have to guess at which dimensions exist on the table.
function buildExtractorUserMessage({ question, prose, shortlistSlugs, citations, feedback, baseLensKind, activeRowNames }) {
  const lines = [
    ...
    prose,
    '',
    'SHORTLIST (you may ONLY use these slugs as cell_data keys for B propose_add_row):',
    ...shortlistSlugs.map(s => `  - ${s}`),
  ];

  // Slice 6 — context for C/D proposals. Always emit the two lines so the
  // model can't infer that re_rank/create_lens are unavailable when active
  // rows are empty (they ARE unavailable in that case, but the validator
  // catches it; the prompt stays uniform).
  lines.push('', `ACTIVE BASE LENS: ${baseLensKind || 'general'}    (use this in view_spec.base_lens_kind for C/D)`);
  if (Array.isArray(activeRowNames) && activeRowNames.length > 0) {
    lines.push('', 'ACTIVE ROWS (use these labels EXACTLY as keys in view_spec.weights / visible_rows for C/D):');
    for (const rn of activeRowNames.slice(0, 30)) lines.push(`  - ${rn}`);
  } else {
    lines.push('', 'ACTIVE ROWS: (none currently — propose_re_rank / propose_create_lens not viable)');
  }
...
async function extractProposedActionsTwoPass({
  ...
  // can produce coherent C/D proposals even before the runner threads the
  ...
  // validator to keep C/D weights/visible_rows scoped to existing rows.
  ...
  const empty = (reason, usage = null) => ({
    ...
    kept:    { total: 0, byKind: { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0 } },
    usage,
  });
  ...
  if (parsed2 == null) {
    return {
      ...
      kept:  result1.kept || { total: 0, byKind: { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0 } },
      usage: usageBoth,
    };
  }
...
// Slice 6 r2 P2: prompt explicitly says "Choose ONE of FOUR outputs / At most
...
// re-rank/create-lens added, two-proposal UX gets confusing and the prompt
// no longer encourages it.
...
// Shared view_spec validator for C/D. Returns { ok, view_spec, reason }.
...
function validateViewSpec(spec, ctx) {
  ...
  // Refuse C/D entirely without a canonical-label map. See header comment.
...
export function validateProposedActions(meta, ctx) {
  ...
  const reasons = {};
  const keptByKind = { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0 };
  let totalSeen = 0;
  ...
  // The C/D validator looks up by normalized key but writes the canonical
  ...
  // rule. re_rank/create_lens don't depend on shortlist length but they
  // also wouldn't make sense without ≥ 2 schools to compare. Hard-skip.
...
export async function* runProseFromIntent({ supabase, question, intentMatch, callClaude, opts = {} }) {
  ...
  // emit propose_re_rank / propose_create_lens with row labels that
  // match what's currently on the comparison table. Defaults are
  // permissive — runner callers (route, eval harness) wire these from
  // research_sessions.active_lens_id + comparison_rows when available.
  // When activeRowNames is empty, the validator REFUSES C/D entirely
  // (no shape-only fallback — Codex r2 P1.2). B propose_add_row is
  // unaffected and continues to fire from this code path.
