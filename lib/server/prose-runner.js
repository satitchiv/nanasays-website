...
Choose ONE of FOUR outputs. At most ONE proposal per call (cap = 1 globally).
...
async function extractProposedActionsTwoPass({
  ...
  const empty = (reason, usage = null) => ({
    ...
    kept:    { total: 0, byKind: { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0, propose_add_to_letter: 0 } },
    usage,
  });
  ...
  if (parsed2 == null) {
    return {
      ...
      kept:  result1.kept || { total: 0, byKind: { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0, propose_add_to_letter: 0 } },
      usage: usageBoth,
    };
  }
...
export function validateProposedActions(meta, ctx) {
  ...
  const reasons = {};
  const keptByKind = { propose_add_row: 0, propose_re_rank: 0, propose_create_lens: 0, propose_add_to_letter: 0 };
  let totalSeen = 0;
