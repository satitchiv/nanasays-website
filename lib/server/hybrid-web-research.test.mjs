...
test('exact-document resolver trusts an official Markdown page link when no native citation exists', async () => {
  const page = 'https://www.millfieldschool.com/admissions/prospectuses'
  const brochure = 'https://view.digital-hub.global/9-at-millfield-brochure/p/1'
  const result = await resolveExactDocumentLink({
    question: 'Find the exact official Year 9 prospectus',
    parentContext: '',
    citations: [],
    prose: `[Millfield publications](${page})`,
    anchors: [{
      slug: 'millfield-school',
  ...
  assert.equal(result?.kind, 'exact_document')
  assert.ok(
    applyExactDocumentLink(`Read this: \`${brochure}\``, result)
      .includes(`[Interactive Nine at Millfield](${brochure})`),
  )
})
...
test('runner applies router settings, streams safe provisional prose, and returns cited evidence', async () => {
  ...
  assert.equal(final.payload.parsed.hybrid_poc.model, 'gpt-5.4-mini')
  assert.equal(final.payload.parsed.hybrid_poc.token_usage.context_tokens, 1000)
  assert.equal(final.payload.parsed.hybrid_poc.token_usage.total_tokens, 1120)
  assert.equal(final.payload.parsed.hybrid_poc.context_used_pct, 0.8)
