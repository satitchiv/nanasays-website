...
export function routeIntent(question, ctx = {}) {
  ...
  if (shortlistSlugs.length >= 2 && SHORTLIST_RE.test(q) && !isProximityGlobalDiscovery(q)) {
    ...
    if (QUANT_STRUCTURED_FACT_RE.test(q)) {
      ...
      if (dimension) {
        ...
        if (dimension !== 'fees_value' && VALUE_SIGNAL_RE.test(q)) dims.push('fees_value');
  ...
  const VERB_DISCOVERY_OPT_IN_DIMS = new Set(['small_class_size']);
  ...
  if (
    ...
    const tools = [
      { name: 'rankSchools', args: { dimension: effectiveDim, limit: n || 8 } },
    ];
    if (proxDiscovery && dimension && dimension !== 'proximity_to_heathrow') {
      tools.push({ name: 'rankSchools', args: { dimension, limit: n || 8 } });
    }
    return {
