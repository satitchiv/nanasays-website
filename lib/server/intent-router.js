...
function detectDimension(q) {
  // Multi-word phrases first (longest, most specific).
  const phrases = ['a-level', 'a level'];
  for (const p of phrases) {
    if (q.includes(p)) {
      const dim = DIMENSION_ALIASES[p];
      if (dim && DIMENSIONS[dim]) return dim;
    }
  }
  ...
  for (const [keyword, dimName] of Object.entries(DIMENSION_ALIASES)) {
    if (!DIMENSIONS[dimName]) continue;
...
const SHORTLIST_RE    = /\b(shortlist|my schools|these schools|rank them|compare them|which of these|my options)\b/i;
