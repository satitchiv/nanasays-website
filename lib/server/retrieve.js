...
export async function retrieveChunks(supabase, slug, question, opts = {}) {
  ...
  // P0.4 defence-in-depth (2026-05-15): if the school's country dictates a
  // local currency (GBP for UK, CHF for Switzerland), force `fees_currency`
  // and nested `fees_by_grade.currency` to match — even if the stored value
  // says USD. The 2026-05-15 migration cleaned the DB once; this guard
  // prevents new drift (re-extractions, manual edits, future imports) from
  // re-introducing the USD lie in the chat-context structured block. Thailand
  // is intentionally absent because Thai intl schools legitimately publish USD.
  if (structured) {
    const { data: schoolRow } = await supabase
      ...
      .eq('slug', slug)
      .maybeSingle();
    const forced = ({ 'United Kingdom': 'GBP', 'Switzerland': 'CHF' })[schoolRow?.country];
    if (forced) {
      if (structured.fees_currency && structured.fees_currency !== forced) {
        warnings.push(`fees_currency override: ${structured.fees_currency}→${forced} for ${schoolRow.country} school`);
  ...
  if (profileRow) {
    // P0.4 (2026-05-15): strip stale fee-line summaries from the pinned
    // profile blob. These were seeded from a pre-Nanasays Doris directory and
    // hardcoded USD labels onto UK/CH schools whose published fees are GBP/CHF.
    // The structured_data layer (school_structured_data.fees_currency +
    // fees_by_grade.currency) carries the authoritative fees; profile-blob
    // fee lines are derivative summaries and never load-bearing. Stripping
    // whole-line, currency-agnostic, on common label prefixes.
    const stripped = (profileRow.content || '')
      .split('\n')
      .filter(line => !/^\s*Annual fees:/i.test(line))
      .filter(line => !/^\s*Boarding fees(\s|:|\()/i.test(line))
      .filter(line => !/^\s*Application fee(\s|:|\()/i.test(line))
      .filter(line => !/^\s*Admission deposit(\s|:|\()/i.test(line))
      .join('\n');
    const cleanedProfile = { ...profileRow, content: stripped };
    const w = countWords(cleanedProfile.content);
