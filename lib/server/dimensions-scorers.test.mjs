...
// T4.16-pre-enable-2 regression — locks in `enabled: false` on the
// remaining 2 disabled dims. tools.js compareSchools (lines 400-408)
// and rankSchools (via hasRequiredData) both gate on this flag; if
// either dim is accidentally flipped to true without going through
// the proper enable review chain, these tests fail before the change
// ships.
//
// T4.16 Gap B (2026-05-09): ethos_match / intl_share / device_policy
// promoted to enabled:true in the same commit that wired ctx.parent
// from onboarding/Brief — locked in below as ENABLED_DIMS so a
// regression that re-disables them also fails loudly.
const DISABLED_DIMS = ['safeguarding_integrity', 'weekend_life'];
const ENABLED_DIMS  = ['ethos_match', 'intl_share', 'device_policy'];
...
for (const dimName of ENABLED_DIMS) {
  test(`${dimName} stays enabled:true (locked post-Gap-B)`, () => {
    assert.equal(DIMENSIONS[dimName].enabled, true,
      `${dimName} must stay enabled — disabling regresses T4.16 Gap B`);
  });
}
