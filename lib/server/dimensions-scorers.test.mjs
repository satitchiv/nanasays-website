...
// ── D-step-1 + D-step-2 (2026-05-26): enabled flag + competition-weight tilt ─

test('subject_strengths is now enabled (D-step-1 — chat-side wiring landed)', () => {
  assert.equal(DIMENSIONS.subject_strengths.enabled, true);
});

test('subject_strengths competition bonus: each competition_result item adds +0.2 up to +1.0', () => {
  const row = {
    subject_strengths: {
      maths: {
        items: [
          { category: 'competition_result', source_url: 'https://x/1' },
          { category: 'competition_result', source_url: 'https://x/2' },
          { category: 'signature_programme', source_url: 'https://x/3' },
          { category: 'departmental_claim',  source_url: 'https://x/4' },
          { category: 'signature_programme', source_url: 'https://x/5' },
        ],
      },
    },
  };
  // 5 items → base 1.0. 2 competition items → +0.4 bonus. Total = 1.4.
  const score = DIMENSIONS.subject_strengths.rank(row, { subject_intents: new Set(['maths']) });
  assert.equal(score, 1.4, `expected 1.4, got ${score}`);
});

test('subject_strengths competition bonus caps at +1.0 even with 10 competition items', () => {
  const row = {
    subject_strengths: {
      maths: {
        items: new Array(10).fill({ category: 'competition_result', source_url: 'https://x/c' }),
      },
    },
  };
  // 10 items → base 1.0. 10 competition items would be +2.0 but cap at +1.0. Total = 2.0.
  const score = DIMENSIONS.subject_strengths.rank(row, { subject_intents: new Set(['maths']) });
  assert.equal(score, 2.0, `expected 2.0 (capped bonus), got ${score}`);
});

test('subject_strengths gives no bonus when items lack category field (backward compat)', () => {
  const row = {
    subject_strengths: {
      maths: { items: new Array(5).fill({ source_url: 'https://x/a' }) },
    },
  };
  // 5 items, no category on any → base 1.0, bonus 0. Same as legacy behavior.
  const score = DIMENSIONS.subject_strengths.rank(row, { subject_intents: new Set(['maths']) });
  assert.equal(score, 1.0, `expected 1.0 (no bonus), got ${score}`);
});

test('subject_strengths: school with all competition wins outranks school with same item count of departmental claims', () => {
  const competitionsHeavy = {
    subject_strengths: {
      maths: {
        items: new Array(5).fill({ category: 'competition_result', source_url: 'https://x/c' }),
      },
    },
  };
  const claimsOnly = {
    subject_strengths: {
      maths: {
        items: new Array(5).fill({ category: 'departmental_claim', source_url: 'https://x/d' }),
      },
    },
  };
  const intent = { subject_intents: new Set(['maths']) };
  const cScore = DIMENSIONS.subject_strengths.rank(competitionsHeavy, intent);
  const dScore = DIMENSIONS.subject_strengths.rank(claimsOnly, intent);
  assert.ok(cScore > dScore, `competition-heavy (${cScore}) should outrank claims-only (${dScore})`);
  assert.equal(cScore, 2.0, `competition-heavy should be 2.0, got ${cScore}`);
  assert.equal(dScore, 1.0, `claims-only should be 1.0, got ${dScore}`);
});

test('subject_strengths competition bonus is per-subject, summed across requested intents', () => {
  const row = {
    subject_strengths: {
      maths: {
        items: new Array(5).fill({ category: 'competition_result', source_url: 'https://x/m' }),
      },
      physics: {
        items: new Array(5).fill({ category: 'competition_result', source_url: 'https://x/p' }),
      },
    },
  };
  // maths: 1.0 + 1.0 = 2.0. physics: 1.0 + 1.0 = 2.0. Total = 4.0.
  const score = DIMENSIONS.subject_strengths.rank(row, { subject_intents: new Set(['maths', 'physics']) });
  assert.equal(score, 4.0, `expected 4.0, got ${score}`);
});
...
// D-step-1 (2026-05-26): subject_strengths moved from DISABLED to ENABLED.
// agentic-loop.js now populates ctx.subject_intents from the parent's
// question via SUBJECT_INTENT_RE — same regex Build Mode uses on the brief.
const ENABLED_DIMS = ['weekend_life', 'safeguarding_integrity', 'ethos_match', 'proximity_to_heathrow', 'small_class_size', 'subject_strengths'];
...
// Scorer reads row.class_size_facts (loaded by loadNotionClassSizeBundles in
// tools.js, sourced from school_notion_backfill.parsed.class_size). Returns
// 1000/(1+midpoint) on the senior > sixth > average preferred sub-field.
// ────────────────────────────────────────────────────────────────────────────

test('small_class_size scores senior scalar 15 at 62.5 (preferred sub-field)', () => {
  const row = { class_size_facts: { senior: 15, sixth: 12, average: 18 } };
  const score = DIMENSIONS.small_class_size.rank(row);
  // 1000 / (1 + 15) = 62.5. Senior wins over sixth even though sixth is smaller.
  assert.equal(score, 62.5);
});

test('small_class_size falls back to sixth when senior absent', () => {
  const row = { class_size_facts: { sixth: 10, average: 18 } };
  const score = DIMENSIONS.small_class_size.rank(row);
  // 1000 / (1 + 10) ≈ 90.91
  assert.ok(score > 90.9 && score < 91, `expected ~90.91, got ${score}`);
});

test('small_class_size falls back to average when senior + sixth absent', () => {
  const row = { class_size_facts: { average: 20 } };
  const score = DIMENSIONS.small_class_size.rank(row);
  // 1000 / (1 + 20) ≈ 47.62
  assert.ok(score > 47 && score < 48, `expected ~47.62, got ${score}`);
});

test('small_class_size scores {min,max} senior range by midpoint (22.5)', () => {
  // Eton-shape: senior {20,25}. Midpoint 22.5, not optimistic 20.
  const row = { class_size_facts: { senior: { min: 20, max: 25 } } };
  const score = DIMENSIONS.small_class_size.rank(row);
  // 1000 / (1 + 22.5) ≈ 42.55
  assert.ok(score > 42.5 && score < 42.6, `expected ~42.55, got ${score}`);
});

test('small_class_size scalar-15 outranks {20,25}-range (midpoint, not min)', () => {
  // Guards against optimistic-min scoring: a {20,25} range school should NOT
  // tie or beat a scalar-15 school just because its min is 20.
  const a = DIMENSIONS.small_class_size.rank({ class_size_facts: { senior: 15 } });
  const b = DIMENSIONS.small_class_size.rank({ class_size_facts: { senior: { min: 20, max: 25 } } });
  assert.ok(a > b, `scalar-15 (${a}) must outrank {20,25}-range (${b})`);
});

test('small_class_size returns 0 when class_size_facts missing entirely', () => {
  assert.equal(DIMENSIONS.small_class_size.rank({}), 0);
  assert.equal(DIMENSIONS.small_class_size.rank({ class_size_facts: null }), 0);
});

test('small_class_size returns 0 when class_size_facts is empty object', () => {
  assert.equal(DIMENSIONS.small_class_size.rank({ class_size_facts: {} }), 0);
});

test('small_class_size returns 0 when sub-field values are invalid (non-numeric / bogus range)', () => {
  // Defensive: projector should already drop these, but the scorer must not
  // crash on stray strings or half-formed ranges.
  assert.equal(DIMENSIONS.small_class_size.rank({ class_size_facts: { senior: 'small' } }), 0);
  assert.equal(DIMENSIONS.small_class_size.rank({ class_size_facts: { senior: { min: 20 } } }), 0);
  assert.equal(DIMENSIONS.small_class_size.rank({ class_size_facts: { senior: NaN } }), 0);
});

test('small_class_size format renders scalar senior with sub-field label', () => {
  const summary = DIMENSIONS.small_class_size.format(
    { class_size_facts: { senior: 15 } },
    { name: 'Eton College' },
  );
  assert.equal(summary, 'Eton College — class size: 15 (senior).');
});

test('small_class_size format renders {min,max} range with en-dash', () => {
  const summary = DIMENSIONS.small_class_size.format(
    { class_size_facts: { senior: { min: 20, max: 25 } } },
    { name: 'Sevenoaks School' },
  );
  assert.equal(summary, 'Sevenoaks School — class size: 20–25 (senior).');
});

test('small_class_size format gracefully handles missing data', () => {
  const summary = DIMENSIONS.small_class_size.format({}, { name: 'Test School' });
  assert.match(summary, /class size unknown/);
});

test('small_class_size.citations returns empty array (Notion umbrella source)', () => {
  assert.deepEqual(DIMENSIONS.small_class_size.citations({}), []);
});

test('small_class_size.keywords matches expected parent phrasings', () => {
  const re = DIMENSIONS.small_class_size.keywords;
  for (const q of [
    'smallest class',
    'smallest classes',
    'small class size',
...
test('small_class_size.keywords does NOT false-fire on benign phrases', () => {
  const re = DIMENSIONS.small_class_size.keywords;
  for (const q of [
    'small school',
    'tiny school',
    ...
    assert.ok(!re.test(q.toLowerCase()), `keywords must NOT match "${q}"`);
  }
});
