...
function formatFeesLine(meta) {
  ...
  // Bug 1 fix: prefer the school's own published local-currency fee text
  // (schools.fees_original, threaded through as meta.fees_text) over any
  // USD-converted numbers, so the model never quotes a conversion as if it
  // were the local price.
  if (typeof meta.fees_text === 'string' && meta.fees_text.trim()) {
    return `fees ${meta.fees_text.trim()} (as published by the school)`
  }
  ...
  // USD reaching here is only ever the converted fallback (SSD/local-text
  // both absent). Flag it so the model doesn't state it as the local price.
  const approx = meta.fees_currency === 'USD' ? ' (converted, approx)' : ''
  ...
  if (min != null && max != null) {
    if (min === max) return `fees ${fmt(min)}/yr${approx}`
    return `fees ${fmt(min)}–${fmt(max)}/yr${approx}`
  }
  if (min != null) return `fees from ${fmt(min)}/yr${approx}`
  return `fees up to ${fmt(max)}/yr${approx}`
}
...
export function buildPackContextString(pack) {
  ...
  // Shortlist (slice matches the assembler's 10-school inScope ceiling)
  if (Array.isArray(pack.shortlist) && pack.shortlist.length > 0) {
    lines.push(`Shortlist (${pack.shortlist.length}): ${pack.shortlist.slice(0, 10).join(', ')}.`)
  }
  ...
  // 2026-06-11 (whole-landscape slice): cap raised 5 → 10 to match the
  // assembler's inScope ceiling. The old slice(0, 5) silently hid schools
  // 6-10 from the model even though the assembler had fetched them — live
  // data shows 11 of 21 shortlists hold 6+ schools.
  const schools = pack.schools && typeof pack.schools === 'object' ? pack.schools : {}
  const slugs = Object.keys(schools)
  if (slugs.length > 0) {
    const summaries = slugs.slice(0, 10).map((slug) => {
  ...
  // 2026-06-11 (whole-landscape slice): when the assembler's overflow
  // reducers dropped context to fit the token budget, SAY SO. Without this
  // the model reads "absent from pack" as "the school doesn't have it" and
  // answers with false confidence. Action names are internal constants from
  // applyOverflowReducers — safe to echo verbatim.
  const overflow = Array.isArray(pack.meta?.overflow_actions) ? pack.meta.overflow_actions.filter((a) => typeof a === 'string') : []
  if (overflow.length > 0) {
    lines.push(
      `Context omitted for space: parts of the supporting data were dropped to fit the prompt budget (${overflow.join(', ')}). ` +
      'Treat missing detail as NOT LOADED, never as "the school does not offer this". ' +
      'If lookup tools are available, check them before answering; otherwise say the detail needs checking rather than implying it does not exist.',
    )
  }
