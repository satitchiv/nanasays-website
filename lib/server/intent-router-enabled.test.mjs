...
// T4.16 Gap B (2026-05-09): ethos_match / intl_share / device_policy
// are now enabled (parent prefs flow from onboarding/Brief). The
// detectDimension contract for these flips from "returns null" to
// "returns the dim" — they're routable now.
...
test('enabled intl_share returns for international keywords', () => {
  assert.equal(detectDimension('how many international pupils does the school have?'), 'intl_share');
});

test('enabled device_policy returns for phone keywords', () => {
  assert.equal(detectDimension('what is the phone policy?'), 'device_policy');
});
...
test('compare_two_on_dim with weekend keyword does not pass weekend_life into compareSchools', () => {
  // weekend_life is still disabled (no producer wired yet). Routing a
  // compare query with a disabled-dim keyword must NOT leak the dim
  // into the plan — the agentic loop handles it instead.
...
test('top_n_for_dim falls through (returns null) when only-matching dim is disabled', () => {
  // "best schools for weekend life" — weekend keyword maps to weekend_life
  // (still disabled). detectDimension returns null, top_n_for_dim requires
  // dimension, so routeIntent falls through and the agentic loop handles it.
  // (Pre-Gap-B this test used 'ethos' as the disabled-dim trigger; ethos_match
  // is now enabled so we use weekend_life as the still-disabled fixture.)
