...
// ────────────────────────────────────────────────────────────────────────────
// 2026-05-25 small_class_size slice (Codex r1 P1b — verb-discovery routing).
// "which/find/show schools have small classes" detects small_class_size via
// dim.keywords but fails TOP_N_RE — without GLOBAL_DISCOVERY_VERB_RE alt-gate,
// rule 3 would silently fall through. Now: rule 3 fires for any detected dim
// when verb-discovery is present, so the agentic loop receives a concrete
// rankSchools tool call instead of planning from scratch.
// ────────────────────────────────────────────────────────────────────────────

test('routeIntent "which schools have the smallest classes" → top_n_for_dim(small_class_size)', () => {
  const m = routeIntent('which schools have the smallest classes', {});
  assert.ok(m, 'expected a match');
  assert.equal(m.intent, 'top_n_for_dim');
  const call = m.plan.tools.find(t => t.name === 'rankSchools');
  assert.ok(call, 'expected a rankSchools tool call');
  assert.equal(call.args.dimension, 'small_class_size');
});

test('routeIntent "find schools with small classes" → top_n_for_dim(small_class_size)', () => {
  const m = routeIntent('find schools with small classes', {});
  assert.ok(m);
  assert.equal(m.intent, 'top_n_for_dim');
  assert.equal(m.plan.tools.find(t => t.name === 'rankSchools')?.args?.dimension, 'small_class_size');
});

test('routeIntent "best schools for small classes" → top_n_for_dim(small_class_size) (TOP_N_RE path)', () => {
  const m = routeIntent('best schools for small classes', {});
  assert.ok(m);
  assert.equal(m.intent, 'top_n_for_dim');
  assert.equal(m.plan.tools.find(t => t.name === 'rankSchools')?.args?.dimension, 'small_class_size');
});

test('routeIntent "which schools have small classes" with shortlist → top_n_for_dim (global wins)', () => {
  // GLOBAL_DISCOVERY_VERB_RE escapes rule 2c → falls through to rule 3 →
  // verb-discovery alt-gate fires for small_class_size dim.
  const m = routeIntent('which schools have small classes', { shortlistSlugs: SHORTLIST });
  assert.ok(m);
  assert.equal(m.intent, 'top_n_for_dim',
    'global "which schools" with shortlist must NOT hijack to shortlist_quant_facts');
  assert.equal(m.plan.tools.find(t => t.name === 'rankSchools')?.args?.dimension, 'small_class_size');
});

test('routeIntent verb-discovery does NOT route "which schools have high fees" to fees_value (polarity guard)', () => {
  // Codex r2 P1 regression guard: GLOBAL_DISCOVERY_VERB_RE matches "which
  // schools have high fees", but fees_value scores LOW fees HIGHER — opting
  // fees_value into verb-discovery would silently return cheapest-first
  // results to a parent asking for most-expensive. Allowlist restricts
  // verb-discovery to dims whose "have X" polarity matches rank() (currently
  // only small_class_size).
  //
  // Codex r3 P2 hardening: assert NO rankSchools call in the plan uses
  // fees_value, regardless of intent — future routing changes could send
  // fees_value through a different branch and we'd miss the regression.
  const m = routeIntent('which schools have high fees', {});
  const rankDims = m?.plan?.tools
    ?.filter(t => t.name === 'rankSchools')
    .map(t => t.args?.dimension) ?? [];
  assert.ok(!rankDims.includes('fees_value'),
    '"which schools have high fees" must not rank by fees_value (low=better polarity mismatch)');
});

test('routeIntent verb-discovery does NOT route "what schools have expensive fees" to fees_value', () => {
  // Same polarity guard for the "what schools have …" variant. Codex r3 P2:
  // assert against any rankSchools(fees_value) in the plan, not just the
  // top_n_for_dim branch.
  const m = routeIntent('what schools have expensive fees', {});
  const rankDims = m?.plan?.tools
    ?.filter(t => t.name === 'rankSchools')
    .map(t => t.args?.dimension) ?? [];
  assert.ok(!rankDims.includes('fees_value'),
    '"what schools have expensive fees" must not rank by fees_value');
});
