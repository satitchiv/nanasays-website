...
test('routeIntent mixed "nearest schools to Heathrow with strong academics" + shortlist → top_n_for_dim(proximity_to_heathrow)', () => {
  ...
  assert.equal(m.plan.tools.find(t => t.name === 'rankSchools')?.args?.dimension, 'proximity_to_heathrow',
    'rule 3 must FORCE proximity_to_heathrow when isProximityGlobalDiscovery fires');
});

test('routeIntent mixed "schools near Heathrow with strong academics" no shortlist → top_n_for_dim(proximity_to_heathrow)', () => {
...
test('routeIntent mixed "schools near Heathrow for rugby" → top_n_for_dim(proximity_to_heathrow)', () => {
  const m = routeIntent('schools near Heathrow for rugby', { shortlistSlugs: SHORTLIST });
  assert.ok(m);
  assert.equal(m.intent, 'top_n_for_dim');
  assert.equal(m.plan.tools.find(t => t.name === 'rankSchools')?.args?.dimension, 'proximity_to_heathrow',
    'proximity wins even when detectDimension picks rugby_standing');
});
