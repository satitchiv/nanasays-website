...
export function expandFamousShortNames(question, existing = []) {
  ...
  // Match possessives instead of deleting apostrophes. Deletion turns
  // "Millfield’s" into "millfields", which no longer matches "millfield".
  // Reed's is the inverse spelling case: the alias key is "reeds", while
  // normal prose usually spells the school "Reed's".
  const lower = question.toLowerCase();
  const hits = [];
  for (const [name, slug] of Object.entries(FAMOUS_SHORT_NAMES)) {
    const escapedName = escapeRegExp(name);
    const namePattern = name === 'reeds'
      ? `(?:${escapedName}(?:['‘’])?|reed['‘’]s)`
      : `${escapedName}(?:['‘’]s)?`;
    const re = new RegExp(`\\b${namePattern}\\b`, 'i');
