...
test('research verdict v3 model constants are present in source', () => {
  ...
  // v3.5 (2026-05-26 — Yoko smoke followup): bumped 8 → 9 when the
  // 50% coverage threshold filter was dropped from path selection so
  // Path A = recommender #1 unconditionally.
  assert.match(generator, /version:\s*9,/)
