...
// ── Bug #5 (2026-05-22) — prompt "anywhere" extraction ─────────────

test('Bug #5: home_region "anywhere" indirect phrasings extracted as confirmed', () => {
  const prompt = buildSystemPrompt({
    childName:    'Jack',
    progress:     emptyProgress(),
    brief:        ZERO_BRIEF,
  ...
  const anchors = [
    ...
    'not tied to',
    'not stuck to',
    'happy to consider any UK region',
    "we'd look nationally",
    'no regional preference',
  ]
  for (const phrase of anchors) {
    assert.ok(
      prompt.includes(phrase),
      `Expected prompt to include "${phrase}" — was the home_region "anywhere" block accidentally removed?`,
    )
  }
})

test('Bug #5: prompt includes 3 BAD examples (history + sequenced + hypothetical)', () => {
  const prompt = buildSystemPrompt({
    childName:    'Jack',
    progress:     emptyProgress(),
    brief:        ZERO_BRIEF,
  ...
  assert.ok(
    prompt.includes('looking nationally for years'),
    'Missing history BAD example ("looking nationally for years")',
  )
  assert.ok(
    prompt.includes('looked nationally before') && prompt.includes('but now want London'),
    'Missing sequenced BAD example (full phrase "looked nationally before, but now want London")',
  )
  assert.ok(
    prompt.includes("in theory we'd consider anywhere, but realistically London"),
    'Missing hypothetical BAD example (exact phrase "in theory we\'d consider anywhere, but realistically London")',
  )
  assert.ok(
    prompt.includes('HYPOTHETICAL'),
    'Missing HYPOTHETICAL boundary header (the prompt should list HISTORY, CURRENT SEARCH STATE, or a HYPOTHETICAL)',
  )
})

test('Bug #5: anywhere block does not loosen existing strict-confidence rule', () => {
  const prompt = buildSystemPrompt({
    childName:    'Jack',
    progress:     emptyProgress(),
    brief:        ZERO_BRIEF,
  ...
  assert.ok(
    prompt.includes('vague/inferred contradictions') ||
    prompt.includes('indirect mentions'),
    'Global confidence-gate rule must still be present',
  )
})
