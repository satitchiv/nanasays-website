...
export async function resolve(specifier, context, nextResolve) {
  ...
  if ((specifier.startsWith('./') || specifier.startsWith('../')) && !path.extname(specifier)) {
    ...
    for (const ext of ['.ts', '.mts', '.tsx']) {
      ...
      if (existsSync(cand)) {
        // 2026-06-11: format must be 'module-typescript', not 'module' — on
        // Node 23+ a forced 'module' bypasses the built-in type stripping and
        // `import type` lines throw SyntaxError at load.
        return { url: pathToFileURL(cand).href, format: 'module-typescript', shortCircuit: true }
      }
    }
  }
