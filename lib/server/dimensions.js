import { SUBJECT_INTENT_RE } from './subject-intents.mjs';
import { GENERIC_SPORTS, genericSportDimName, scoreSportOffering, scoreSportBreadth } from './generic-sport.mjs';

// Codex P1.4 (2026-05-26): the previous subject_strengths.keywords regex
// used coarse stems (`biolog`, `chemist`, `physics`) that BOTH over-fired
// on bystander words ("chemist on site" = pharmacist; "family business") AND
// under-fired on real subjects ("biology", "chemistry" — the `\b...\b`
// boundaries on the stems never matched). Rebuild dim.keywords from the
// canonical SUBJECT_INTENT_RE patterns in subject-intents.mjs — those are
// already tight ("biology|biological|biomedical"; "english\s+literature"
// not bare "english") and shared with ctx.subject_intents extraction, so
// the suggestion and ranking layers stay in lockstep.
const SUBJECT_STRENGTHS_KEYWORDS_RE = new RegExp(
  // Each source wrapped in (?:...) so future SUBJECT_INTENT_RE alternation
  // shape changes can't bleed across union boundaries (Codex r1 nit).
  Object.values(SUBJECT_INTENT_RE).map((r) => `(?:${r.source})`).join('|'),
  'i',
);
...
export const DIMENSIONS = {
  tennis_strength: {
    ...
    // Thai alternate sits OUTSIDE \b(...)\b — Thai codepoints are non-word
    // chars so \b can never anchor around them (2026-07-05 Thai routing).
    // (?<!เทเบิล) stops เทนนิส substring-matching inside เทเบิลเทนนิส (table
    // tennis) — those must route to table_tennis_offering.
    keywords:        /\b(tennis|youll|aberdare|lta|wimbledon|atp|wta|glanville)\b|(?<!เทเบิล)เทนนิส/i,
  ...
  rugby_standing: {
    ...
    keywords:        /\b(rugby|scrum|lineout|dmt|daily mail trophy|continental tyres)\b|รักบี้/i,
  ...
  football_strength: {
    ...
    keywords:        /\b(football|soccer|isfa|esfa|first xi|first eleven|premier league)\b|ฟุตบอล/i,
  ...
  cricket_strength: {
    ...
    keywords:        /\b(cricket|cricketer|ecb|t20|wisden|county cricket|test cricket|first xi)\b|คริกเก็ต/i,
  ...
  hockey_strength: {
    ...
    keywords:        /\b(hockey|ishc|england hockey|astro|astroturf|gb hockey|olympic hockey)\b|ฮอกกี้/i,
  ...
  academic_strength: {
    ...
    keywords:        /\b(academic|academics|results|grades|gcse|a.?level|a level|ib|oxbridge|oxford|cambridge|russell|university|destinations|exam)\b|วิชาการ|ผลการเรียน|ผลสอบ/i,
  ...
  // Recommender Phase 2 (2026-05-21): per-subject density scorer over the
  // school_structured_data.subject_strengths v2.0 polymorphic blob.
  // ctx.subject_intents must be a Set<string> of subject keys the parent
  // mentioned in free text (maths / biology / physics / chemistry / english /
  // history / modern_languages / computer_science / economics_business). When
  // empty / missing, rank() returns 0 — no penalty, just inert.
  //
  // D-step-1 (2026-05-26): enabled in chat path. agentic-loop.js applies
  // SUBJECT_INTENT_RE to the parent's question before invoking rankSchools,
  // populating toolCtx.subject_intents. Previously only Build Mode populated
  // it from the brief; now "best schools for maths" routes through this dim
  // instead of falling back to exam_results as a proxy.
  //
  // D-step-2 (2026-05-26): competition-result items earn a separate bonus
  // on top of the base banding (Olympiad / national-finals wins are stronger
  // signal than departmental claims or signature programmes). Bonus tops out
  // at +1.0 per subject (5 competition items max contribution).
  subject_strengths: {
    label:          'Subject strengths',
    description:    'Per-subject density over subject_strengths v2.0 (items + Claude-generated summary), with a competition-result bonus.',
    keywords:       SUBJECT_STRENGTHS_KEYWORDS_RE,
    requires_field: 'subject_strengths',
    enabled:        true,

    rank: (row, ctx) => {
      const ss = row?.subject_strengths;
      if (!ss || typeof ss !== 'object') return 0;
      const want = ctx?.subject_intents;
      if (!want || typeof want.size !== 'number' || want.size === 0) return 0;
      let total = 0;
      for (const subject of want) {
        const bucket = ss[subject];
        const items = Array.isArray(bucket?.items) ? bucket.items : [];
        const itemCount = items.length;
        // Base banding: 5+ items = "strong" (1.0). 2-4 = "moderate" (0.5).
        // <2 = 0. Matches renderSubjectStrengthsLines banding so signals
        // + chips stay consistent.
        let base = 0;
        if      (itemCount >= 5) base = 1.0;
        else if (itemCount >= 2) base = 0.5;
        // Competition-result bonus: +0.2 per item with category='competition_result',
        // capped at +1.0 (5 competition items max). Olympiads / national-finals
        // wins should outrank departmental claims at the same item count.
        let compCount = 0;
        for (const it of items) {
          if (it && it.category === 'competition_result') compCount++;
        }
        const compBonus = Math.min(compCount * 0.2, 1.0);
        total += base + compBonus;
      }
      return total;
    },

    // Per-subject summary lines for the ranked output (chat-side ranker uses
    // this when subject_strengths is enabled). Trimmed for prompt-cost.
    // 2026-05-26 follow-up fix: signature is (row, school, ctx) — earlier
    // (_i, _w, ctx) positions never reached the call site, so ctx was
    // undefined and the function fell back to "School — subjects." with no
    // item counts. Smoke surfaced this: Nana said "tool result here only
    // confirms one clear leader" because lower-ranked rows had no evidence
    // strings even though they had populated maths items.
    format: (row, school, ctx) => {
      const ss = row?.subject_strengths;
      const want = ctx?.subject_intents;
      if (!ss || !want || want.size === 0) return `${school.name} — subjects.`;
      const lines = [];
      for (const subject of want) {
        const bucket = ss[subject];
        ...
        const summary = typeof bucket?.summary_paragraph_for_chatbot === 'string'
          ? bucket.summary_paragraph_for_chatbot.slice(0, 200)
          : null;
        const label = subject.replace(/_/g, ' ');
        lines.push(summary
          ? `${label}: ${summary} (${itemCount} items).`
          : `${label}: ${itemCount} items extracted.`);
      }
      return [`${school.name} — subjects.`, ...lines].join(' ');
    },

    citations: (row, ctx) => {
      const ss = row?.subject_strengths;
      const want = ctx?.subject_intents;
      if (!ss || !want || want.size === 0) return [];
      const out = [];
      const seen = new Set();
      for (const subject of want) {
        const items = ss[subject]?.items;
  ...
  fees_value: {
    ...
    keywords:        /\b(fees|fee|cost|costs|cheap|expensive|affordable|scholarship|bursary|price|under)\b|ค่าเทอม|ค่าเล่าเรียน/i,
  ...
  pastoral_model: {
    ...
    // Fallback dim for pastoral/wellbeing questions. ISI-backed pastoral_care
    // (priority 10) wins when both match — pastoral_model covers structural
    // data (house system, staffing, policies) the ISI dim does not.
    priority:        90,
  ...
  // Lets parents discover schools BY distance to Heathrow Airport instead of
  // only looking up a single school's distance. Closes the "top N schools
  // close to Heathrow" gap surfaced in the post-Heathrow-fix browser smoke
  // (parent asked "top 4 close to Heathrow" → Nana ranked only the shortlist
  // because there was no rankSchools dimension for proximity).
  //
  // Data source: school_structured_data.location_profile.airports[] — the
  // same SSD shape the chat projector reads in nana-brain.js for the per-
  // school Heathrow lookup. Scorer logic is intentionally duplicated here
  // (rather than importing _ssdHeathrowMiles from nana-brain.js) because
  // dimensions.js is the source-of-truth for ranking and shouldn't depend
  // on the chat module. If the helper grows or drifts, extract to a shared
  // `lib/server/location-utils.js` module.
  proximity_to_heathrow: {
    enabled: true,
    label:           'Proximity to Heathrow',
    description:     'Distance to London Heathrow in miles (from SSD location_profile.airports[]). Lower miles = higher score. Closes the global "top N schools close to Heathrow" use case for international families.',
    keywords:        /\b(heathrow|lhr|near\s+(?:heathrow|london\s+airport)|close\s+to\s+(?:heathrow|london\s+airport)|airport\s+access|closest\s+to\s+(?:heathrow|london\s+airport)|nearest\s+to\s+(?:heathrow|london\s+airport))\b/i,
    requires_field:  'location_profile',
    rank: (row) => {
      const miles = _heathrowMilesFromLocation(row?.location_profile);
      if (miles == null) return 0;
      // Continuous decay: 1000 / (1 + miles). Always positive (passes the
      // rankSchools `score <= 0` filter). Eton 9 mi → 100, Wellington 17 mi
      // → 55, Millfield 102 mi → ~9.7. Closer schools dominate the rank.
      return 1000 / (1 + miles);
    },
    format: (row, school) => {
      const miles = _heathrowMilesFromLocation(row?.location_profile);
      if (miles == null) return `${school.name} — Heathrow distance unknown.`;
      return `${school.name} — ${miles} miles from Heathrow.`;
    },
    // No source_url for airport data in SSD; rank shows distance as a primary
    // fact rather than needing a citation. Empty array matches the
    // weekend_life pattern when sources are absent.
    citations: () => [],
  },

  // small_class_size — 2026-05-25 (class-size global-discovery slice).
  // Mirrors proximity_to_heathrow shape: a global "smallest" ranker over a
  // SCALAR signal hand-curated in Notion (not yet present in SSD for most
  // UK schools). Closes the "which schools have small classes?" use case
  // that the chat could answer for ONE school via getSchoolFacts but had
  // no way to rank across a shortlist or corpus.
  //
  // Data source: school_notion_backfill.parsed.class_size → projected to
  //   { senior?, sixth?, average? } via projectNotionBackfill (nana-brain.js).
  // Each sub-field is either a scalar number (e.g. 15) or a {min,max} range
  // (e.g. {min:20, max:25}) — see _coerceClassSizeValue. Live data 2026-05-25:
  // ~47 of 72 UK schools have at least one sub-field; 6 use range shape.
  //
  // The bundle is loaded in tools.js loadNotionClassSizeBundles() and folded
  // into row.class_size_facts before rank() runs. We pick the most specific
  // available sub-field (senior > sixth > average) to mirror what the chat
  // renderer already shows parents in `_notionClassSize` (nana-brain.js). For
  // {min,max} ranges, score by midpoint so a {20,25} school doesn't out-rank
  // a scalar-15 school by the optimistic min.
  small_class_size: {
    enabled: true,
    label:           'Small class size',
    description:     'Schools with the smallest typical class size (from hand-curated Notion class_size data: senior > sixth > average). Lower size = higher score.',
    // Covers: smallest|small|tiny|low|smaller + class[(es)|size(s)]; class
    // size(s) (smallest|small|low|smaller); fewer pupils per class; small
    // teaching groups; pupil-teacher ratio. Codex r1 P2 widening — previous
    // regex missed "smaller classes" + "fewer pupils per class" + ratio phrasing.
    keywords:        /\b((?:smallest|small(?:er)?|tiny|low)\s+class(?:\s+size)?(?:es)?|class\s+sizes?\s+(?:smallest|small(?:er)?|low)|fewer\s+pupils\s+per\s+class|small\s+teaching\s+groups?|pupil[- ]teacher\s+ratio)\b/i,
    requires_field:  'class_size_facts',
    rank: (row) => {
      const size = _smallestClassSizeMidpoint(row?.class_size_facts);
      if (size == null) return 0;
      // Continuous decay: 1000 / (1 + size). Always positive (passes the
      // rankSchools `score <= 0` filter). Size 8 → ~111, 15 → 62.5, 25 → 38.5.
      // Smaller classes dominate the rank.
      return 1000 / (1 + size);
    },
    format: (row, school) => {
      const cs = row?.class_size_facts;
      if (!cs) return `${school.name} — class size unknown.`;
      // Mirror the chat renderer's preference order (senior > sixth > average)
      // and units so prose matches what parents already see elsewhere.
      const senior = _formatClassSize(cs.senior);
      if (senior) return `${school.name} — class size: ${senior} (senior).`;
      const sixth = _formatClassSize(cs.sixth);
      if (sixth) return `${school.name} — class size: ${sixth} (sixth form).`;
      const average = _formatClassSize(cs.average);
      if (average) return `${school.name} — class size: ~${average} (school average).`;
      return `${school.name} — class size unknown.`;
    },
    // Notion class_size is hand-curated without a per-row source_url; the
    // verified-facts block already cites Notion as the umbrella source.
    citations: () => [],
  },
  ...
  inclusive_culture: {
    ...
    // Preferred over generic pastoral_model when both match an inclusion
    // question — ISI inspection findings beat structural-fallback for the
    // questions this scorer is designed for.
    priority:        10,
  ...
  pastoral_care: {
    ...
    // Preferred over generic pastoral_model when both match a pastoral
    // question — ISI inspection findings beat structural-fallback.
    priority:        10,
    requires_field:  'isi_deep_facts',
    rank: (row, ctx) => {
      const f = row?.isi_deep_facts;
...
// ── Generic sport-offering dimensions (2026-07-05) ───────────────────────────
// One `<sport>_offering` dimension per GENERIC_SPORTS registry entry (golf,
// swimming, rowing, …). Scored from the GENERIC fields of sports_profile
// (signature_sports / sport_categories / teams_by_sport / competitions_entered
// / facilities / scholarships / sports_offered) rather than the per-sport
// competitive_tier blocks only the big five have. This is what lets "which
// schools are best for golf?" route deterministically through rankSchools
// instead of falling to the agentic loop with no rankable dimension.
//
// doc_group: 'sport_offering' — agentic-loop.js collapses these into ONE
// system-prompt docs line so ~30 new dims don't bloat every chat call.
// Registered AFTER the literal above so detectDimension / suggestDimensions
// keyword loops still give the big-five *_strength dims first crack at
// their sports (insertion order is match priority).
function makeSportOfferingDimension(key, spec) {
  return {
    label:          `${spec.label} programme & offering`,
    description:    `${spec.label} teams, competitions, facilities and signature-sport status from the school's published sport data (offering-level evidence, not competitive tier).`,
    keywords:       spec.re,
    requires_field: 'sports_profile',
    doc_group:      'sport_offering',
    rank:   (row) => scoreSportOffering(row?.sports_profile, key).score,
    format: (row, school) => {
      const { score, evidence } = scoreSportOffering(row?.sports_profile, key);
      if (score <= 0) {
        return `${school.name} — no ${spec.label.toLowerCase()} evidence in the school's published sport data.`;
      }
      return `${school.name} — ${spec.label.toLowerCase()}: ${evidence.join('; ')}.`;
    },
    citations: (row) => {
      const urls = row?.sports_profile?.source_urls;
...
for (const [key, spec] of Object.entries(GENERIC_SPORTS)) {
  const dimName = genericSportDimName(key);
  // Defence: never shadow an existing hand-written dimension.
  if (!DIMENSIONS[dimName]) DIMENSIONS[dimName] = makeSportOfferingDimension(key, spec);
}

// ── sport_breadth (2026-07-05) ───────────────────────────────────────────────
// Overall sport-programme strength for questions that name NO specific sport
// ("strong academics plus sports", "best sporty schools", "กีฬา"). Before this
// dim, those asks had no rankable sport side: detectDimension picked only
// academic_strength and the sport half of the question silently vanished.
// Registered AFTER the big-five and generic offering dims so a question that
// names a real sport ("golf") always resolves to the specific dim first
// (insertion order = detectDimension match priority).
DIMENSIONS.sport_breadth = {
  label:          'Overall sport programme',
  description:    'Breadth & depth of the whole sport programme — signature sports, academy/major programmes, team counts, competitions, facilities, scholarships. Use when the parent asks about "sport(s)" generally without naming one.',
  keywords:       /\b(?:sports?|sporty|sporting)\b|กีฬา/i,
  requires_field: 'sports_profile',
  rank:   (row) => scoreSportBreadth(row?.sports_profile).score,
  format: (row, school) => {
    const { score, evidence } = scoreSportBreadth(row?.sports_profile);
    if (score <= 0) return `${school.name} — no sport-programme evidence in the school's published data.`;
    return `${school.name} — sport programme: ${evidence.join('; ')}.`;
  },
  citations: (row) => {
    const urls = row?.sports_profile?.source_urls;
    return Array.isArray(urls) ? urls : [];
 ...
 *
 * Ordering: dims with lower `priority` (default 50) come first, so when
 * multiple dims share a question (e.g. pastoral_care + pastoral_model both
 * matching "pastoral care"), the ISI-backed dim (priority 10) is suggested
 * ahead of the structural fallback (priority 90).
 */
export function suggestDimensions(question) {
  ...
  for (const [name, dim] of Object.entries(DIMENSIONS)) {
    ...
    if (dim.keywords?.test(question)) {
      matched.push({ name, priority: dim.priority ?? 50 });
    }
  }
...
// Codex P1.1 (2026-05-26): subject_strengths needs a tighter gate than the
// generic null/empty-string check. Schools occasionally land in SSD with
// subject_strengths={} or with every bucket carrying an empty items[] (e.g.
// extractor returned a shell object). Those rows pass the generic gate, run
// scoring, score 0, and get dropped by the score<=0 filter — wasted work
// that becomes load-bearing under unlocked Build Mode ranking across all UK
// schools. Reject them at the gate instead.
function hasNonEmptySubjectStrengthsBucket(ss) {
  if (!ss || typeof ss !== 'object') return false;
  // Canonical SSD shape is { maths: {items}, physics: {items}, ... } — a
  // plain object. Reject top-level arrays (malformed extractor output)
  // defensively; without this, `[{ items: [...] }]` would slip through
  // because Object.values iterates array elements (Codex r1 P1.1 nit).
  if (Array.isArray(ss)) return false;
  for (const bucket of Object.values(ss)) {
    if (bucket && Array.isArray(bucket.items) && bucket.items.length > 0) return true;
  }
  return false;
}
...
export function hasRequiredData(row, dimensionName) {
  ...
  if (dimensionName === 'subject_strengths' && !hasNonEmptySubjectStrengthsBucket(v)) return false;
  return true;
}
...
export function listDimensions() {
  return Object.entries(DIMENSIONS)
    ...
    .map(([name, d]) => ({
      ...
      // 2026-07-05: lets agentic-loop.js collapse the ~30 generic
      // sport-offering dims into a single system-prompt docs line.
      ...(d.doc_group ? { doc_group: d.doc_group } : {}),
    }));
}
...
export function fieldsForDimension(dimensionName) {
  switch (dimensionName) {
    ...
    // 2026-05-25 proximity_to_heathrow — needs SSD location_profile.
    case 'proximity_to_heathrow':
      return ['location_profile'];
    // 2026-05-25 small_class_size — bundle source is school_notion_backfill
    // (NOT an SSD column), so this returns null. Bundle is folded by
    // loadNotionClassSizeBundles() in tools.js; getSchoolFacts already
    // surfaces class_size via notion_backfill on every call regardless of
    // requested fields. Codex r1 P2.
    case 'small_class_size':
      return null;
    // 2026-07-05 sport_breadth — whole-programme scorer over sports_profile;
    // facilities included so "sports facilities" fact lookups keep parity
    // with the old bare-'sports' branch in intent-router rule 8.
    case 'sport_breadth':
      return ['sports_profile', 'facilities'];
    default:
      // 2026-07-05: generic sport-offering dims all score from the
      // sports_profile blob — mirror the big-five case above so
      // compare_two_on_dim plans fetch the right fields.
      if (DIMENSIONS[dimensionName]?.doc_group === 'sport_offering') {
        return ['sports_profile'];
      }
      return null;
  }
...
/**
 * Extract Heathrow distance in miles from an SSD location_profile.
 * Sources (in priority order):
 *   1. location_profile.heathrow_miles (direct miles, rare)
 *   2. airports[] entry where name=/heathrow/i or code/iata === 'LHR'
 *      with distance_miles (direct) or distance_km (× 0.621371).
 * Drive-time-only entries are NOT used because they're a different unit
 * (minutes) than what we score (miles).
 *
 * Same contract as `_ssdHeathrowMiles` in nana-brain.js. Duplicated rather
 * than imported because dimensions.js shouldn't depend on the chat module.
 * If the helper grows or drifts, extract to a shared `lib/server/
 * location-utils.js` module — both modules can then import it.
 */
/**
 * Pick the smallest-class-size signal from a projected Notion class_size
 * bundle ({senior?, sixth?, average?}) and return it as a numeric midpoint
 * (range → (min+max)/2; scalar → as-is). Returns null when nothing usable.
 *
 * Preference order mirrors `_notionClassSize` in nana-brain.js so prose and
 * ranking agree on which sub-field "represents" a school's class size:
 *   1. senior  — most common, applies whole-school
 *   2. sixth   — used only when senior absent
 *   3. average — last-resort generic average
 *
 * Midpoint (rather than min) keeps {20,25}-range schools from out-ranking
 * scalar-15 schools by the optimistic min.
 */
function _smallestClassSizeMidpoint(cs) {
  if (!cs || typeof cs !== 'object') return null;
  for (const key of ['senior', 'sixth', 'average']) {
    const v = cs[key];
    const n = _classSizeValueMidpoint(v);
    if (n != null) return n;
  }
  return null;
}

function _classSizeValueMidpoint(v) {
...
/**
 * Format a class_size sub-field for ranking prose. Scalar → "15"; range →
 * "20–25" (en-dash, matches the chat renderer's `_formatClassSizeValue`).
 * Returns null when the value isn't a usable scalar or {min,max} range.
 */
function _formatClassSize(v) {
...
function _heathrowMilesFromLocation(lp) {
  ...
  for (const a of lp.airports) {
    ...
    if (typeof a.distance_km === 'number' && Number.isFinite(a.distance_km)) {
      ...
      // and nana-brain.js _ssdHeathrowMiles.
