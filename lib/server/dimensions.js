...
export const DIMENSIONS = {
  ...
  //
  // enabled:false because chat-side dim routing has no way to populate
  // subject_intents yet (suggestDimensions reads the question, not the brief).
  // Build Mode recommender invokes rank() directly with a hand-built ctx and
  // does NOT consult enabled, mirroring how the brief-driven scorer uses
  // pastoral_care + inclusive_culture there.
  subject_strengths: {
    label:          'Subject strengths',
    description:    'Per-subject density over subject_strengths v2.0 (items + Claude-generated summary). Gated on parent-mentioned subject intents in build-mode brief.',
    ...
    rank: (row, ctx) => {
      ...
      for (const subject of want) {
        ...
        // 5+ items = "strong" (1.0). 2-4 = "moderate" (0.5). <2 = 0.
        // Matches the same banding renderSubjectStrengthsLines uses in
        // buildStructuredBlock so signals + chips stay consistent.
        if      (itemCount >= 5) total += 1.0;
        else if (itemCount >= 2) total += 0.5;
      }
      return total;
    },
