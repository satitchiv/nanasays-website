...
export const DIMENSIONS = {
  ...
  ethos_match: {
    ...
    rank: (row, ctx) => {
      ...
      if (!f) return 0;
      const want = ctx?.parent?.ethos_pref ?? null; // optional; fed by tools.js
      let score = 1; // baseline for having any ethos data
      if (want && f.ethos_label === want) score += 4;
      else if (want && f.ethos_label === 'mixed_faith') score += 2;
      return score;
    },
    format: (row, school) => {
      const f = row?.ethos_facts;
  ...
  intl_share: {
    ...
    rank: (row, ctx) => {
      ...
      // Score is shaped to parent preference — high or low is "good" depending
      // on what the parent is asking for. Default: more diversity = higher.
      const want = ctx?.parent?.intl_pref ?? 'high';
  ...
  device_policy: {
    ...
    rank: (row, ctx) => {
      ...
      if (!f) return 0;
      // Score is shaped to parent preference — strict or flexible.
      const want = ctx?.parent?.phone_pref ?? 'strict';
      const map = {
      ...
      let score = map[f.phone_policy] ?? 0;
      if (want === 'flexible') score = 6 - score; // invert for parents who want lighter touch
      return Math.max(0, score);
    },
    format: (row, school) => {
      const f = row?.device_policy_facts;
