...
test('emits header + facts when at least one field is present', () => {
  ...
  // 2026-05-25 rename: "HAND-CURATED FACTS" → "VERIFIED SCHOOL FACTS" once
  // heathrow_distance could be SSD-derived (Codex Heathrow-fix nit).
...
test('renderer: header omits [notion-curated] bracket-cite (Codex r1 P2 fix)', () => {
  ...
  // 2026-05-25 rename — see "emits header + facts" test above for the rationale.
  assert.match(lines[0], /VERIFIED SCHOOL FACTS/);
});
...
// 2026-05-25 Heathrow data-gap fix: projector now SURFACES SSD Heathrow miles
// rather than just suppressing Notion when SSD has it. Earlier tests asserted
// "SSD has Heathrow → projected = null" (suppress only); these are rewritten
// to assert "SSD has Heathrow → projected.heathrow_distance = ssdMiles"
// (surface). This fixes 69/72 UK schools that had Notion heathrow_distance
// NULL via `extractor_wins` precedence but the SSD value was never read.
test('projector heathrow 2026-05-25: SSD location_profile.heathrow_miles surfaces (overrides Notion)', () => {
  const ssd = { location_profile: { heathrow_miles: 22 } };
  const projected = projectNotionBackfill({ heathrow_distance: 25 }, ssd);
  assert.equal(projected.heathrow_distance, 22, 'SSD direct miles wins over Notion');
});

test('projector heathrow 2026-05-25: SSD airports[] distance_km converts to miles (Wellington shape)', () => {
  // Live Wellington data: airports[0]={name:"Heathrow", distance_km:27}.
  // 27 km × 0.621371 ≈ 16.78 → rounded 17.
  const ssd = {
    location_profile: {
      airports: [{ name: 'Heathrow', distance_km: 27, drive_time_min_estimate: 45 }],
    },
  };
  const projected = projectNotionBackfill({}, ssd);
  assert.equal(projected.heathrow_distance, 17,
    'km→miles conversion (27 km ≈ 17 mi) matches load-verdict-school-facts.ts');
});

test('projector heathrow 2026-05-25: SSD airports[] with "London Heathrow Airport" name + distance_km', () => {
  const ssd = {
    location_profile: {
  ...
  // 35 km × 0.621371 ≈ 21.75 → 22. SSD wins over Notion.
  assert.equal(projected.heathrow_distance, 22);
});

test('projector heathrow 2026-05-25: SSD airports[] LHR code + distance_miles direct', () => {
  const ssd = {
    location_profile: {
  ...
  assert.equal(projected.heathrow_distance, 18, 'SSD miles wins over Notion');
});

test('projector heathrow 2026-05-25: SSD drive-time-only Heathrow falls through to Notion', () => {
  // Drive-time alone is wrong unit — projector ignores, falls through to Notion.
  const ssd = {
    location_profile: {
  ...
  assert.equal(projected.heathrow_distance, 30, 'Notion scalar used when SSD has no usable miles/km');
});

test('projector heathrow 2026-05-25: SSD Gatwick only (no Heathrow) falls through to Notion', () => {
  const ssd = {
    location_profile: {
  ...
  assert.equal(projected.heathrow_distance, 45, 'Notion used — SSD has wrong airport');
});

test('projector heathrow 2026-05-25: Notion wrapper {distance_mi: 19} unwraps correctly', () => {
  // 3 schools (St Paul\'s London, St Catherine\'s Bramley, Royal Masonic) have
  // parsed.heathrow_distance = {distance_mi: 19} from an older sync. Without
  // unwrap they were silently dropped (projector required scalar number).
  const projected = projectNotionBackfill({ heathrow_distance: { distance_mi: 19 } }, null);
  assert.equal(projected.heathrow_distance, 19);
});

test('projector heathrow 2026-05-25: SSD wins over Notion wrapper too', () => {
  const ssd = { location_profile: { airports: [{ name: 'Heathrow', distance_miles: 17 } ] } };
  const projected = projectNotionBackfill({ heathrow_distance: { distance_mi: 19 } }, ssd);
  assert.equal(projected.heathrow_distance, 17);
});

test('projector heathrow 2026-05-25: nothing anywhere → no heathrow_distance in output', () => {
  const projected = projectNotionBackfill({ class_size: { senior: 15 } }, null);
  assert.equal(projected.heathrow_distance, undefined);
});
...
// 2026-05-25: REVERSED earlier Codex r3 P2 decision after browser smoke
// surfaced 6 production schools (Eton, Sevenoaks, Oundle, Queenswood, CLC,
// St Catherine's Bramley) with range objects — silently dropping them made
// Eton rank "smallest at 10" by surfacing only sixth-form scalar. Ranges
// are now preserved + rendered "20–25".
test('projector 2026-05-25: class_size range objects {min, max} are PRESERVED', () => {
  const projected = projectNotionBackfill(
    {
      class_size: {
        senior: { min: 20, max: 25 },        // Eton-shape
        sixth: { min: 8, max: 10 },          // Sevenoaks-shape
        average: { min: 12, max: 15 },       // CLC-shape
      },
    },
    null,
  );
  assert.deepEqual(projected.class_size, {
    senior: { min: 20, max: 25 },
    sixth: { min: 8, max: 10 },
    average: { min: 12, max: 15 },
  });
});

test('projector 2026-05-25: degenerate range (min === max) collapses to scalar', () => {
  const projected = projectNotionBackfill(
    { class_size: { senior: { min: 15, max: 15 } } },
    null,
  );
  assert.deepEqual(projected.class_size, { senior: 15 });
});

test('projector 2026-05-25: invalid range (max < min) is dropped', () => {
  const projected = projectNotionBackfill(
    { class_size: { senior: { min: 25, max: 20 } } },
    null,
  );
  assert.equal(projected, null);
});

test('projector 2026-05-25: range with out-of-bounds values dropped', () => {
  // Sane bounds 4-35. A range like {min: 2, max: 50} → dropped.
  const projected = projectNotionBackfill(
    { class_size: { senior: { min: 2, max: 50 } } },
    null,
  );
  assert.equal(projected, null);
});

test('projector 2026-05-25: class_size with mixed scalar + range keeps both', () => {
  const projected = projectNotionBackfill(
    {
      class_size: {
        senior: 15,
...
test('renderer 2026-05-25: range class_size renders as "20–25 (senior)"', () => {
  // Eton-shape input through full render pipeline.
  const lines = renderNotionBackfillLines({
    class_size: { senior: { min: 20, max: 25 }, sixth: 10 },
  });
  // senior wins precedence over sixth.
  assert.match(lines.join('\n'), /Average class size: 20–25 \(senior\)/);
  assert.doesNotMatch(lines.join('\n'), /sixth form/, 'senior wins over sixth when both present');
});

test('renderer 2026-05-25: sixth range "8–10 (sixth form)" when no senior', () => {
  const lines = renderNotionBackfillLines({
    class_size: { sixth: { min: 8, max: 10 } },
  });
  assert.match(lines.join('\n'), /Average class size: 8–10 \(sixth form\)/);
});

test('renderer 2026-05-25: average range "~12–15 (school average)" when only average', () => {
  // CLC / St Catherine's Bramley shape.
  const lines = renderNotionBackfillLines({
    class_size: { average: { min: 12, max: 15 } },
  });
  assert.match(lines.join('\n'), /Average class size: ~12–15 \(school average\)/);
});
