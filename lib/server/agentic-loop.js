...
function buildSystemPrompt({ verbosity = 'chat' } = {}) {
  ...
  // 2026-07-05: the ~30 generic `<sport>_offering` dims (golf_offering,
  // swimming_offering, …) share one behaviour, so they collapse into a
  // single docs line — one line instead of thirty keeps the prompt small
  // and the model's dimension menu readable.
  const allDims = listDimensions();
  const sportOfferingDims = allDims.filter(d => d.doc_group === 'sport_offering');
  const dimensionDocs = [
    ...allDims
      .filter(d => d.doc_group !== 'sport_offering')
      .map(d => `  ${d.name}: ${d.description}`),
    ...(sportOfferingDims.length
      ? [`  <sport>_offering: evidence of a school's programme for a sport WITHOUT a dedicated *_strength dimension — teams, competitions, facilities, signature-sport status. Available: ${sportOfferingDims.map(d => d.name).join(', ')}. Use the exact name (e.g. "golf_offering") as the rankSchools/compareSchools dimension.`]
      : []),
  ].join('\n');
...
1. PLAN BEFORE YOU ACT. Pick the right starting tool by question shape:
   - QUANTITATIVE STRUCTURED FACTS (highest priority — supersedes the other shapes below when the question is one of these): class size, total pupils, boarder count, international pupil count, boarding share (% boarding), GCSE 9-7 %, A-Level A*-A %, Heathrow distance, lowest boarding entry year. → getSchoolFacts per slug. Read the "notion_backfill" field in the result — those facts ARE hand-curated and ARE present for most UK schools. Do NOT use compareSchools for these (its dimensions don't cover class_size etc), and do NOT use searchSchoolText (free-text search returns boarding-house sizes instead of academic class sizes). TURN-BUDGET CAVEAT: each tool_call uses a turn and you only have 4 total (with turn 4 reserved for final_answer). For a 4+ school shortlist, fetch the top 3 most relevant slugs first (or top 2 + the outlier) — name the un-fetched schools in what_we_dont_know rather than skipping the answer. For single-school questions this caveat doesn't apply.
   ...
   - "compare these N schools on X" / shortlist comparison (when X is one of the rankable DIMENSIONS below, NOT a quantitative structured fact from the list above) → compareSchools FIRST with the named slugs and the dimension(s) implied by X (tennis → tennis_strength, rugby → rugby_standing, academics → academic_strength, fees/value → fees_value, pastoral care / mental health / bullying / PSHE / wellbeing support → pastoral_care, LGBTQ+ / inclusion / diversity / pupil voice / belonging → inclusive_culture, house system / tutor model / wellbeing staffing / pastoral structure / policies → pastoral_model, any other sport (golf, swimming, rowing, netball, …) → the matching <sport>_offering dimension, "sport(s)" in general with NO named sport → sport_breadth). Then getSchoolFacts on the top 1-2 schools or outliers most relevant to the answer.
...
7. NARRATIVE vs QUANTITATIVE routing:
   - NARRATIVE-ONLY questions ("what is the culture like at Eton?", "what's the vibe?", "what do current pupils say?") → searchSchoolText with the slug. NOT rankSchools, NOT getSchoolFacts.
   - QUANTITATIVE STRUCTURED FACTS (class size, pupil count, boarder/intl count, boarding share, GCSE/A-Level %, Heathrow distance, lowest entry year) → getSchoolFacts per slug (see Rule 1). NOT searchSchoolText — free-text returns the wrong category (e.g. boarding-house sizes for class-size questions).
   - Mixed questions ("compare the culture AND class sizes") → both tools, but call getSchoolFacts FIRST so you have the structured facts before colouring them with narrative excerpts.
...
export async function* runAgenticLoop({
  ...
  //
  // D-step-1 (2026-05-26): subject_intents populated from the parent's
  // question via SUBJECT_INTENT_RE — drives DIMENSIONS.subject_strengths.rank()
  // when the question mentions a subject ("best schools for maths"). Empty
  // Set is safe: the dim returns 0 and falls through to other ranking signals.
  const toolCtx = {
  ...
  // Empty-rank-result throttle state (Codex P1.3 2026-05-26). When
  // rankSchools returns 0 schools the model historically cascaded into 3-4
  // diagnostic getSchoolFacts calls — the root cause of the 10× chat
  // slowness reported on 2026-05-26 morning. Cap diag fact calls at
  // EMPTY_RANK_DIAG_BUDGET (1) and steer toward final_answer with
  // what_we_dont_know instead. See empty-rank-throttle.mjs for the
  // helpers + tests.
  const emptyRankState = makeEmptyRankState();
  ...
  for (turn = 1; turn <= MAX_TURNS; turn++) {
    ...
    // Empty-rank fallback throttle (Codex P1.3 2026-05-26, r1 fix #1):
    // cap diagnostic getSchoolFacts calls at EMPTY_RANK_DIAG_BUDGET after
    // rankSchools returned no scoreable schools, then push the model toward
    // final_answer. This is intentional STEERING, not a loop bug — do NOT
    // bump unproductiveTurns or thread into agent_stuck; MAX_TURNS plus the
    // force-final fallback already cap the worst case.
    const throttle = shouldThrottleFactCall(toolName, emptyRankState);
    if (throttle.throttle) {
      console.log(
        `[empty-rank-throttle] reject dimension=${emptyRankState.activatedDimension ?? 'unknown'} toolName=${toolName} diag_calls=${emptyRankState.diagFactCallsAfterEmpty} budget=${EMPTY_RANK_DIAG_BUDGET} turn=${turn}`,
      );
      messages.push(`SYSTEM NOTE: ${throttle.reason}`);
      continue;
    }
    ...
    let toolResult;
    try {
      toolResult = await TOOLS[toolName](supabase, effectiveArgs, toolCtx);
    } catch (e) {
    ...
    // Update the empty-rank throttle state with this turn's outcome (Codex
    // P1.3 2026-05-26). Empty rankSchools → start throttling; non-empty
    // rankSchools → clear; getSchoolFacts while throttling → count it.
    // effectiveArgs is forwarded so the helper can capture the dimension
    // that activated the cascade for telemetry (Codex r2 fix).
    const emptyRankSeenBefore = emptyRankState.emptyRankSeen;
    updateEmptyRankState(toolName, toolResult.result, emptyRankState, effectiveArgs);
    if (!emptyRankSeenBefore && emptyRankState.emptyRankSeen) {
      // Activation telemetry (Codex r1 fix #2 + r2 harmonised fields):
      // dimension + toolName + budget + counter + turn so log lines
      // correlate cleanly with subsequent reject events.
      const slugsCount = Array.isArray(effectiveArgs?.restrict_to_slugs)
        ? effectiveArgs.restrict_to_slugs.length
        : 0;
      console.log(
        `[empty-rank-throttle] activated dimension=${emptyRankState.activatedDimension ?? 'unknown'} toolName=${toolName} diag_calls=${emptyRankState.diagFactCallsAfterEmpty} budget=${EMPTY_RANK_DIAG_BUDGET} restrict_to_slugs_count=${slugsCount} turn=${turn}`,
      );
    }
    ...
    //
    // EMPTY_RANK_HINT appends a one-fact-call-then-final_answer steer when
    // rankSchools returned 0 schools, so the model doesn't try to compensate
    // with a cascade of per-shortlist-school fact lookups (Codex P1.3).
    const emptyRankHint = isEmptyRankResult(toolName, toolResult.result) ? EMPTY_RANK_HINT : '';
    messages.push(
      `TOOL RESULT [${toolName}]:\n${injectToolResult(toolName, toolResult.result)}${emptyRankHint}\n\nDecide your next action. JSON only.`
    );
  }
