...
// This is the Research Room's visible operating budget for prompt context,
// not a claim about an OpenAI model's undocumented hard maximum. Keeping the
// denominator explicit lets the UI explain what its percentage means.
const HYBRID_CONTEXT_BUDGET_TOKENS = 128_000;
...
export async function resolveExactDocumentLink({
  ...
  // OpenAI's web-search annotations are not guaranteed when the model writes
  // a normal Markdown link. Treat an official-school Markdown page as a
  // trusted discovery starting point too; the final external brochure still
  // has to be found and health-checked through that official page chain.
  const officialMarkdownCandidates = extractMarkdownLinks(prose)
    .map(link => {
  ...
  const firstPass = await inspectDocumentCandidates(
    [...officialCitationCandidates, ...officialMarkdownCandidates, ...anchorCandidates, ...storedCandidates]
      .sort((a, b) => b.score - a.score),
...
export function applyExactDocumentLink(prose, exactDocument) {
  ...
  const title = cleanPromptText(exactDocument.title, 160)
    .replace(/[[\]()]/g, '')
    || (exactDocument.kind === 'exact_document' ? 'Official document' : 'Official document page');
  const markdownLink = `[${title}](${url})`;
  const alreadyLinked = new RegExp(`\\]\\(${escapeRegExp(url)}\\)`, 'i').test(prose);
  const linkBareUrl = value => {
    if (new RegExp(`\\]\\(${escapeRegExp(url)}\\)`, 'i').test(value)) return value;
    const codeUrl = new RegExp('`' + escapeRegExp(url) + '`', 'i');
    if (codeUrl.test(value)) return value.replace(codeUrl, markdownLink);
    return value.replace(url, markdownLink);
  };
  if (exactDocument.kind !== 'exact_document') {
    const linkedProse = prose.includes(url) ? linkBareUrl(prose) : prose;
    if (linkedProse !== prose || alreadyLinked) {
      return `Note: this is an official document page; the exact current file was not verified.\n\n${linkedProse}`;
    }
    return `Official document page (the exact current file was not verified): ${markdownLink}\n\n${prose}`;
  }
  if (prose.includes(url)) return linkBareUrl(prose);
  ...
  return `Official document: ${markdownLink}\n\n${prose}`;
}
...
export async function* runHybridWebResearchStream(supabase, question, opts = {}) {
  ...
  const renderedModelProse = renderCitationsAsMarkdown(finalText, evidence.citations);
  const exactDocument = await resolveExactDocumentLink({
    question,
    parentContext: opts.parentContext,
    citations: citationSources,
    prose: renderedModelProse,
    anchors,
    documentCandidates,
  ...
  let prose = applyExactDocumentLink(renderedModelProse, exactDocument);
  ...
  const cost = computeHybridCost(usage, responseModel, evidence.searchCallCount);
  const contextUsedPct = Number(Math.min(
    100,
    (inputTotal / HYBRID_CONTEXT_BUDGET_TOKENS) * 100,
  ).toFixed(1));
    ...
    payload: {
      parsed: {
        ...
        hybrid_poc: {
          ...
          model: responseModel,
          token_usage: {
            context_tokens: inputTotal,
            input_tokens: usage?.input_tokens || 0,
            output_tokens: usage?.output_tokens || 0,
            cache_read_tokens: cacheRead,
            cache_write_tokens: cacheWrite,
            total_tokens: inputTotal + (response.usage?.output_tokens || 0),
          },
          context_budget_tokens: HYBRID_CONTEXT_BUDGET_TOKENS,
