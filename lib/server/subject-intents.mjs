...
export const SUBJECT_INTENT_RE = {
  ...
  // P1.4 (2026-05-26): require -ry/-ries suffix. Bare `chemist`/`chemists`
  // is the pharmacist noun ("chemist on site" = drugstore) — never the
  // school subject. False positive removed; `chemistry`/`chemistries` still
  // match.
  chemistry:          /\b(?:chemistr(?:y|ies)|chemical\w*)\b/i,
  ...
  // P1.4 (2026-05-26): require `economics` (with -s) for the subject.
  // Bare `economic` (adjective: "economic class", "economic background",
  // "economic crisis") is sociology/news vocabulary, not the school subject.
  // `economy` / `econometric*` / `economist(s)` already disambiguate
  // separately. False positives removed; `economics` / `microeconomics` /
  // `macroeconomics` still match.
