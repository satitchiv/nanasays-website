...
export function compactToolResult(toolName, result) {
  ...
  if (toolName === 'rankSchools' && Array.isArray(result.schools)) {
    const cm = result.coverage_meta;
    // P5: lead with coverage so the LLM frames "based on N schools that
    // disclose X" instead of implying the whole cohort was considered.
    const header = cm
      ? `coverage: ${cm.scored} schools scored of ${cm.covered} that disclose this dimension (${cm.eligible} eligible in cohort); returning top ${cm.returned}.`
      : null;
    const body = result.schools.map((s, i) => {
  ...
  if (toolName === 'compareSchools' && Array.isArray(result.schools)) {
    const cm = result.coverage_meta;
    // P5: flag any compared school × dimension cell that came back empty so
    // the LLM can name the gap inline ("Marlborough has no pastoral_care data").
    const gaps = cm && cm.per_dimension
      ? Object.entries(cm.per_dimension).filter(([_, n]) => n < cm.requested)
      : [];
    const header = gaps.length
      ? `coverage: ${cm.requested} schools requested; data gaps: ${gaps.map(([d, n]) => `${d} (${n}/${cm.requested})`).join(', ')}.`
      : null;
    const body = result.schools.map(s => {
      if (!s) return '(no data)';
