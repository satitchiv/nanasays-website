...
import { renderSportsProfileLines, renderNotionBackfillLines } from './nana-brain.js';

const FIELD_BUDGET_DEFAULT = 400;
const FALLBACK_BUDGET      = 2000;
// Slice-6 nana-reasoning fix: 4096 was tuned for slice 5 single-school
// answers but tail-truncates 6-school comparisons asymmetrically — the
// last 1-2 schools' academic data gets cut while the first ones keep
// it, and Nana then says "no academic data" for the truncated schools.
// 8192 buys coverage for 6-school comparisons. The proper fix is
// per-school slot allocation in the compactor so truncation hits
// proportionally rather than at the tail (queued in TASKS.md).
const HARD_LIMIT_DEFAULT   = 8192;   // last-resort cap at injection sites
...
/**
 * location_profile carve-out (2026-07-05). The generic compactor rendered
 * the commute-critical fields as shape summaries — `airports: [3 ×
 * {name,distance_km,drive_time_min_estimate}]` — and pushed nearest_town /
 * setting_note behind "+9 more keys". A parent's commute/location table
 * column can't be built from that. This renders the fields parents actually
 * ask about (setting, nearest town/city, airport drive times) as bullet
 * lines, mirroring the sports_profile carve-out below. crime_summary is
 * intentionally excluded — its interpretation prose is ~500 chars/school
 * and has its own UI surface planned (police.uk link, TASKS.md 3.5.8).
 */
function renderLocationProfileLines(lp) {
  if (!lp || typeof lp !== 'object') return [];
  const lines = [];
  const settingBits = [];
  if (lp.setting) settingBits.push(String(lp.setting));
  if (lp.region) settingBits.push(String(lp.region));
  if (lp.postcode) settingBits.push(String(lp.postcode));
  if (settingBits.length) lines.push(`  • setting: ${settingBits.join(', ')}`);
  for (const key of ['nearest_town', 'nearest_city']) {
    const t = lp[key];
    if (t && typeof t === 'object' && t.name) {
      const dist = typeof t.distance_km === 'number' ? ` (${t.distance_km} km)` : '';
      lines.push(`  • ${key.replace('_', ' ')}: ${t.name}${dist}`);
    }
  }
  if (Array.isArray(lp.airports) && lp.airports.length) {
    const airports = lp.airports.slice(0, 4).map((a) => {
      if (!a || typeof a !== 'object' || !a.name) return null;
      const km = typeof a.distance_km === 'number' ? `${a.distance_km} km` : '';
      const drive = typeof a.drive_time_min_estimate === 'number' ? `~${a.drive_time_min_estimate} min drive` : '';
      const detail = [km, drive].filter(Boolean).join(', ');
      return detail ? `${a.name} (${detail})` : a.name;
    }).filter(Boolean);
    if (airports.length) lines.push(`  • airports: ${airports.join('; ')}`);
  }
  if (typeof lp.setting_note === 'string' && lp.setting_note.trim()) {
    lines.push(`  • area: ${lp.setting_note.trim().slice(0, 220)}`);
  }
  return lines;
}
...
export function compactToolResult(toolName, result) {
  ...
  if (toolName === 'rankSchools' && Array.isArray(result.schools)) {
    // Empty result must be visible to the model — a blank TOOL RESULT block
    // historically triggered a 3-4 call getSchoolFacts cascade (Codex P1.3,
    // 2026-05-26). The agentic-loop layer adds EMPTY_RANK_HINT after this
    // string to steer toward final_answer.
    if (result.schools.length === 0) {
      return `(rankSchools returned 0 schools — dimension="${result.dimension ?? 'unknown'}"; structured data is sparse for this question)`;
    }
  ...
  if (toolName === 'getSchoolFacts' && result.data) {
    ...
    // Notion sidecar (Codex r1 P1.3 — agentic parity). Surface after meta and
    // before requested fields so the model sees hand-curated facts in the
    // same compact prose shape the chat surface uses. `notion_backfill` is
    // already projected (extractor-wins, fees stripped, units normalised).
    if (result.notion_backfill && typeof result.notion_backfill === 'object') {
      const notionLines = renderNotionBackfillLines(result.notion_backfill).filter((l) => l.trim().startsWith('•'));
      if (notionLines.length) {
        lines.push(`  notion_backfill:`);
        for (const nl of notionLines) lines.push(`  ${nl}`);
      }
    }
    for (const [k, v] of Object.entries(result.data)) {
      ...
      if (k === 'location_profile' && v && typeof v === 'object') {
        const locLines = renderLocationProfileLines(v);
        if (locLines.length) {
          lines.push(`  location_profile:`);
          lines.push(...locLines);
          continue;
        }
        // Fall through to the generic compactor when the profile carries
        // none of the known fields (defensive — shape drift).
      }
