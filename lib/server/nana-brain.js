...
export function buildStructuredBlock(structured) {
  ...
  if (structured.scholarships_available?.length) lines.push(`Scholarships: ${structured.scholarships_available.length} listed`);

  if (structured.sports_profile && typeof structured.sports_profile === 'object') {
    const sports = Object.entries(structured.sports_profile)
      .filter(([k, v]) => v && typeof v === 'object' && !['extracted_at', 'evidence_urls'].includes(k));
    if (sports.length > 0) {
      lines.push('\nSPORTS PROFILE (per-sport authoritative data — use these facts directly):');
      for (const [sport, data] of sports) {
        const parts = [];
        if (data.notes)            parts.push(data.notes);
        if (data.head_coach?.name) parts.push(`Head coach: ${data.head_coach.name}${data.head_coach.title ? ` (${data.head_coach.title})` : ''}`);
        if (data.coaches?.length)  parts.push(`${data.coaches.length} coaching staff`);
        if (data.teams_count)      parts.push(`${data.teams_count} teams`);
        ...
        if (data.cup_results?.length) {
          const wins = data.cup_results.filter(r => r.result === 'winner');
          if (wins.length) parts.push(`Cup wins: ${wins.map(w => `${w.tournament} ${w.year}`).join(', ')}`);
        }
        if (data.alumni?.length)   parts.push(`Notable alumni: ${data.alumni.length} listed`);
        if (parts.length === 0) continue;
        lines.push(`  • ${sport.toUpperCase()}: ${parts.join(' · ')}`);
      }
    }
  }
