...
// don't carry). Lines are labelled "HAND-CURATED FACTS" so the LLM knows the
// provenance without needing a bracket cite the parser would reject.
const NOTION_BLOCK_HEADER =
  '\nHAND-CURATED FACTS (verified UK school data — treat as authoritative):';
...
export function projectNotionBackfill(parsed, structured) {
  ...
  // Heathrow SSD-wins (Codex r2 P1.2). SSD writes Heathrow under either
  // `location_profile.heathrow_miles` (rare) OR `location_profile.airports[]`
  // with a name matching /heathrow/i or code/iata === 'LHR'. Mirrors the
  // detection logic in load-verdict-school-facts.ts so chat + verdict agree.
  if (isFiniteNum(parsed.heathrow_distance) && !_ssdHasHeathrow(ssd)) {
...
// SSD Heathrow detector — mirrors load-verdict-school-facts.ts so chat
// suppression aligns with verdict-page treatment. Returns true when SSD has
// any usable Heathrow distance (direct miles, km via airport list, or LHR
// code/iata match). Drive-time-only entries don't count because they're a
// different unit than parsed.heathrow_distance (which is miles).
function _ssdHasHeathrow(ssd) {
  if (!ssd || typeof ssd !== 'object') return false;
  const lp = ssd.location_profile;
  if (!lp || typeof lp !== 'object') return false;
  if (typeof lp.heathrow_miles === 'number' && Number.isFinite(lp.heathrow_miles)) return true;
  if (!Array.isArray(lp.airports)) return false;
  for (const a of lp.airports) {
    ...
    if (typeof a.distance_miles === 'number' && Number.isFinite(a.distance_miles)) return true;
    if (typeof a.distance_km === 'number' && Number.isFinite(a.distance_km)) return true;
  }
  return false;
}
