...
// P6 — persistence shape for validationIssues. The streaming event already
// includes `validationIssues` at top level for live consumers (telemetry,
// the live final-event handler), but the route persists ONLY `parsed` to
// `research_session_messages.parsed_answer`, and the client stores only
// `evt.payload.parsed`. So historical messages read back via the bubble
// can't see top-level fields. By folding the same array into `parsed`
// here (additively, only when non-empty), NanaBubble + NanaPanel + the
// shared-answer share page all get the citationFailure gate uniformly
// — one regex contract across every runner path. Prose-runner.js does
// the same fold at its emit site.
function withValidationIssues(parsed, validationIssues) {
  if (!parsed || !Array.isArray(validationIssues) || validationIssues.length === 0) {
    return parsed;
  }
  return { ...parsed, validationIssues };
}
...
function sameCanonicalSlug(a, b) {
  return !!a && !!b && canonicalSlugBase(a) === canonicalSlugBase(b);
}
...
function lookupSlugName(nameBySlug, slug) {
  if (!slug) return null;
  if (nameBySlug instanceof Map) return nameBySlug.get(slug) || null;
  if (nameBySlug && typeof nameBySlug === 'object') return nameBySlug[slug] || null;
  return null;
}

// Codex r2 P2-2 / r3 P6-B / r4 P6-B: detect host school name (or distinctive
// bare token from it) appearing in the question text. FAMOUS_SHORT_NAMES only
// covers ~14 famous schools, so without this check `namedPairExcludesHost`
// wrongly fires when a parent says "How does Wellington compare to Eton and
// Harrow?" on the Wellington College report — "Wellington" isn't in the
// famous list, so the slug detector misses it and the warning falsely claims
// the host wasn't named.
//
// r3 / r4 fixes:
//   - Acronym hosts (LVS, AKS, RGS, KCS) need preservation — `length >= 4`
//     dropped them, leaving such schools with no usable token and re-triggering
//     the misfire. Now we keep tokens length ≥ 4 OR all-uppercase length 2-3
//     (excluding generic "ST" which would over-match every Saint school).
//   - Reuses SCHOOL_STOP_WORDS (defined later in module — referenced lazily
//     at call time, fine under hoisting). Adds a few extras SCHOOL_STOP_WORDS
//     doesn't cover (of, mens, womens, co-ed, coed, institute, foundation,
//     grammar). Without this, generic host tokens like "international" /
//     "royal" falsely marked the host as implicit (Codex r3 reproduced live
//     on "ACS International School Cobham" + "international pupils" question).
// Codex r5 P6-B: locality words (`city`, `london`) added to the extras since
// they overlap with the SCHOOL_STOP_WORDS class — Codex reproduced
// "City of London School" host + "city families" question falsely marking
// the host as implicit. Also adds a phrase-level check: if the question
// contains the full host name (or its non-suffix prefix) as a substring,
// mark the host as named regardless of individual token genericity. So
// "City of London" appearing in the question DOES count, but a bare "city"
// alone doesn't.
const _hostNameStopwordsExtra = new Set([
  'of', 'mens', 'womens', 'co-ed', 'coed', 'institute', 'foundation', 'grammar',
  'city', 'london',
]);
function hostNameInQuestion(hostName, question) {
  if (!hostName || !question) return false;
  const q = String(question).toLowerCase().replace(/['‘’]/g, '');

  // Phrase-level check first. Strip trailing School/College/Academy/etc so
  // "City of London School" → "city of london". If that phrase appears in the
  // question, the host is named, even if individual tokens are generic.
  //
  // Codex r6 P6-B: bare `q.includes(phrase)` bypassed word boundaries —
  // "Bradley" wrongly matched "Radley" (Radley College host). Now: normalise
  // hyphens/slashes to spaces in both sides, build a word-boundary regex
  // with flexible whitespace.
  const normalise = (s) => s.replace(/[-_/]/g, ' ').replace(/\s+/g, ' ').trim();
  const qNorm = normalise(q);
  const hostLower = normalise(String(hostName).toLowerCase().replace(/['‘’]/g, ''));
  const phraseCandidates = new Set();
  if (hostLower.length >= 4) phraseCandidates.add(hostLower);
  const stripped = normalise(hostLower.replace(/\s+\b(school|college|academy|institute|foundation)\b.*$/i, ''));
  if (stripped && stripped !== hostLower && stripped.length >= 4 && stripped.includes(' ')) phraseCandidates.add(stripped);
  // Codex r7 P6-B: allow optional trailing `s` for possessive forms — the
  // question text has apostrophes already stripped, so `City of London's`
  // arrives as `city of londons`. Without the optional `s` the phrase regex
  // would fail. Leading `\b` is preserved so substring matches (Bradley vs
  // Radley) are still rejected.
  for (const phrase of phraseCandidates) {
    const escaped = phrase.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\s+/g, '\\s+');
    if (new RegExp(`\\b${escaped}s?\\b`).test(qNorm)) return true;
  }

  // Token-level check fallback.
  const raw = String(hostName).replace(/['‘’]/g, '').split(/[^A-Za-z0-9]+/).filter(Boolean);
  const tokens = [];
  for (const r of raw) {
    const lower = r.toLowerCase();
    if (SCHOOL_STOP_WORDS.has(lower) || _hostNameStopwordsExtra.has(lower)) continue;
    const isAcronym = r.length >= 2 && r.length <= 3 && r === r.toUpperCase() && lower !== 'st';
    if (lower.length >= 4 || isAcronym) tokens.push(lower);
  }
  if (!tokens.length) return false;
  // Codex r7 P6-B: same trailing-`s?` for token-level possessives
  // ("Wellington's results" → "wellingtons results").
  return tokens.some(t => new RegExp(`\\b${t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}s?\\b`).test(q));
}

export function buildNamedSchoolScopeWarningFromSlugs({
  hostSlug,
  hostName,
  question,
  comparisonSlug = null,
  ...
  const named = [...new Set((mentionedSlugs || []).filter(Boolean))];
  if (named.length === 0) return null;

  const loaded = [hostSlug, comparisonSlug].filter(Boolean);
  const missing = named.filter(slug => !loaded.some(loadedSlug => sameCanonicalSlug(slug, loadedSlug)));
  if (missing.length === 0) return null;

  const hostIsNamed = named.some(slug => sameCanonicalSlug(slug, hostSlug));
  const hostIsImplicit = /\b(this school|this report|here)\b/i.test(String(question || ''))
    || hostNameInQuestion(hostName, question);
  const namedPairExcludesHost = named.length >= 2 && !hostIsNamed && !hostIsImplicit;
  const missingNames = missing.map(slug => lookupSlugName(nameBySlug, slug) || humaniseSlug(slug));
  const loadedNames = loaded.map(slug => {
    if (sameCanonicalSlug(slug, hostSlug)) return `${hostName || humaniseSlug(slug)} (${slug})`;
    return `${lookupSlugName(nameBySlug, slug) || humaniseSlug(slug)} (${slug})`;
  });

  return [
    'NAMED SCHOOL SCOPE WARNING:',
    `The parent named ${missingNames.join(', ')}, but the loaded context only contains ${loadedNames.join(' and ')}.`,
    'Say explicitly that the missing named school is outside this report scope.',
    'Do NOT use the host school as a substitute for the missing named school.',
    'Do NOT relabel host-school facts as if they belonged to the missing named school.',
    namedPairExcludesHost
      ? 'The parent named a comparison pair that does not include the host school; do not turn the answer into a host-vs-target comparison. Do not cite or summarize host-school figures as a consolation answer.'
      : null,
  ].filter(Boolean).join('\n');
}

async function buildNamedSchoolScopeWarning(supabase, hostSlug, hostName, question, comparisonSlug = null) {
  if (!supabase || !hostSlug || !question) return null;
  const detected = await detectMentionedSlugs(supabase, question);
  const mentionedSlugs = expandFamousShortNames(question, detected);
  if (!mentionedSlugs.length) return null;

  const schools = await loadSchoolNames(supabase);
  const nameBySlug = new Map((schools || []).map(s => [s.slug, s.name]));

  return buildNamedSchoolScopeWarningFromSlugs({
    hostSlug,
    hostName,
    question,
    comparisonSlug,
...
6. SCOPE: this answer covers ONE school only — UNLESS a "COMPARISON SCHOOL:
   <name> (<slug>)" section appears inside the UMBRELLA CONTEXT block. When
   present, the parent has explicitly asked a comparison question and we
   have loaded facts for both schools. In that case:
     - ANSWER the comparison directly, citing concrete facts from BOTH
       schools' sections (HOST SCHOOL and COMPARISON SCHOOL).
     - Treat the COMPARISON block's URLs as quotable in sources_used.
     - **MANDATORY**: when HOST SCHOOL and/or COMPARISON SCHOOL sections
       are in UMBRELLA CONTEXT, EVERY entry in sources_used whose URL or
       fact comes from either school MUST populate school_slug with the
       slug copied verbatim from that section's header (e.g.
       'harrow-school' for the HOST SCHOOL block, 'eton-college' for
       the COMPARISON SCHOOL block). This applies to host citations too
       — do NOT default the host's school_slug to omitted. A missing
       school_slug on a host-URL citation is treated as a contract
       violation by the validator.
     - The 3 you_might_also_ask follow-ups MAY include one cross-school
       follow-up; the other two stay scoped to the host school.
     - NO SCHOOL SUBSTITUTION: if the parent names a pair of schools and
       one of those named schools is NOT the HOST SCHOOL or COMPARISON SCHOOL
       in the UMBRELLA CONTEXT, say that the named school is outside this
       report's loaded scope. Do NOT use the host school's facts as a proxy
       for the missing named school, and do NOT relabel host facts as if they
       belonged to the missing school. Example: on a Wellington College report,
       if the parent asks "How does Eton compare to Harrow?" but Harrow is
       not loaded, answer: "I don't have Harrow's full breakdown in this
       report's scope — ask on Harrow's report page for a like-for-like."
       If the named comparison pair does not include the HOST SCHOOL, do not
       volunteer host-school figures as a consolation answer; mention the host
       only to explain why this report page cannot answer the named pair.
   When NO COMPARISON SCHOOL section is present, the old single-school rule
   stands: don't suggest comparison questions, all follow-ups answerable
   with this school's own data, and school_slug is OPTIONAL on sources_used.
...
10. THIN-DATA SIGNAL DENSITY. When the parent asks a question whose answer
    rests on a small subset of UK independent schools (e.g. "best value
    boarding" when only N schools publish per-term fee tables), name the
    ...
    schools that publish X], [observation]." Sounds like an advisor who
    knows what's documented and what isn't, not a search box. When the
    information is fully published for the one school you're answering
    about (e.g. its fees), omit the denominator framing — it would feel
    forced.
...
C. ON UNCERTAINTY, NAME THE GAP + REDIRECT. Never just "I don't have
   that information" and NEVER say the word "dataset". Sound like a school
   advisor reading from the school's records, not an AI describing its own
   memory. Pattern: "the school's published profile doesn't cover [X], but
   [Y] is on record showing [fact]." Variants that all read advisor-y:
   "the published records don't go into [X], but [Y]…" / "[X] isn't
   documented here yet, but [Y] is — want to dig into that?" / "[X]
   would be a question for the school directly; what IS published is [Y]…"
   GOOD: "ISI inspection details aren't in the published profile here,
   but the 2023 exam results show 78% A*-A — want to dig into those?"
   BAD: "I don't have specific information about that."
   BAD: "X isn't in my dataset/data." (sounds like an AI, not an advisor)
...

{
  "sections": {
    ...
    "you_might_also_ask": "(REQUIRED) exactly 3 follow-up questions. Default scope: this school only. EXCEPTION: when the UMBRELLA CONTEXT includes a COMPARISON SCHOOL section, at most ONE of the three follow-ups MAY be a cross-school follow-up about these two named schools; the other two stay scoped to the host. Rule 11 (high-stakes advisor-style next-move) still takes priority over the cross-school slot when both apply."
  },
  "sources_used": [{ "section_id": "", "section_label": "<Umbrella: use the exact bracketed key from the cited line (profile field name or ISI fact_type); never 'PROFILE FIELDS' or 'ISI INSPECTION FACTS'. Chunks: title/heading.>", "source_url": "<URL copied verbatim from supplied context (EXCERPT, REGULATORY RECORDS, or UMBRELLA CONTEXT), or empty if structured-only>", "source_type": "", "school_slug": "<Comparison umbrella only: REQUIRED for every HOST/COMPARISON source; copy the slug from that school's header. Omit otherwise.>" }],
...
6. SCOPE: this answer covers ONE school only — UNLESS a "COMPARISON SCHOOL:
   ...
   have loaded facts for both schools. In that case:
     ...
     - **MANDATORY**: when HOST SCHOOL and/or COMPARISON SCHOOL sections
       are in UMBRELLA CONTEXT, EVERY entry in sources_used whose URL or
       fact comes from either school MUST populate school_slug with the
       slug copied verbatim from that section's header. Host citations
       require it just as target citations do — do NOT default the host
       to omitted slug.
     - At most ONE of the 3 you_might_also_ask follow-ups MAY be a
       cross-school question about these two schools; the others stay
       scoped to the host.
     - NO SCHOOL SUBSTITUTION: if the parent names a pair of schools and
       one of those named schools is NOT the HOST SCHOOL or COMPARISON SCHOOL
       in the UMBRELLA CONTEXT, say that the named school is outside this
...
9. COMPARISON TABLE. Populate "comparison_table" whenever your answer
   ...
   NEVER include other named schools as table rows — UNLESS the UMBRELLA
   CONTEXT includes a COMPARISON SCHOOL section, in which case a two-row
   table (host vs comparison school) is the most useful shape and rows
   MAY name both schools. Benchmark figures (e.g. "ISC average for UK
   independent boarding fees ~£42k") may go in the footer field but
   never as table rows.
...
11. BALANCED FRAMING. Lead with the strongest fact the parent asked about, but
    do not omit a genuine concern when the data shows one. A school with
    strong A-level results AND a 2023 ISI compliance flag is a different
    recommendation than one with strong A-levels alone — surface both. The
    tradeoff field is the right home for concerns; use it whenever the data
    genuinely supports one, not just when it's convenient. Do NOT manufacture
    negatives to look balanced — if the data shows only positives on the
    asked question, leave tradeoff as the literal "Nothing to flag here". The
    rule is: don't suppress real concerns the data exposes.

12. THIN-DATA SIGNAL DENSITY. When the parent asks a question whose answer
    rests on a small subset of UK independent schools (e.g. "best value
    boarding" when only N schools publish per-term fee tables), name the
    denominator inline: "Based on the 38 schools that publish per-term
    fees, this falls in the bottom quartile." Pattern: "Based on [N
    schools that publish X], [observation]." Sounds like an advisor who
    knows what's documented and what isn't, not a search box. ONLY cite a
    denominator N when N is present in the retrieved context (e.g. a tool
    result count, a retrieval summary). NEVER invent a denominator. When
    the information is fully published for the one school you're answering
    about (e.g. its fees), omit the denominator framing — it would feel
    forced.

13. HIGH-STAKES FOLLOW-UPS. For questions about money, safety, or fit
    ("worth the fees?", "is my shy son safe here?", "right for an
    ambitious athlete?"), the third follow_ups entry should be an
    advisor-style next move the parent can ACT on, not just another
    information question. Patterns:
      Money: "Ask the bursar about scholarship deadlines for next entry."
      Safety: "Ask to tour the pastoral house and meet the duty matron."
      Fit:    "Ask current parents in your year-group whether their kids
               felt the [stretch/competition/pace] suited them."
    The other two follow_ups stay as data-grounded questions about this
    school. Apply this rule only when the question's TOPIC is money/safety/
    fit — not when the parent is asking about timetables or curriculum.
  ...
  "sources_used": [
    {
      ...
      "section_label": "(string — Umbrella: use the exact bracketed key from the cited line (profile field name or ISI fact_type); never 'PROFILE FIELDS' or 'ISI INSPECTION FACTS'. Chunks: title/heading.)",
      ...
      "school_slug":   "(string — Comparison umbrella only: REQUIRED for every HOST/COMPARISON source; copy the slug from that school's header. Omit otherwise.)"
    }
  ],
...
you_might_also_ask → exactly three follow-up questions. Default scope: this school only. EXCEPTION: when the UMBRELLA CONTEXT includes a COMPARISON SCHOOL section, AT MOST ONE follow-up MAY be a cross-school question about these two named schools; rule 11 (high-stakes advisor next-move) still takes priority over the cross-school slot when both apply.
...
export function validateAnswer(parsed, retrieval, opts = {}) {
  ...
  if (Array.isArray(parsed.sources_used) && parsed.sources_used.length > 0) {
    ...
    //
    // Three cases (Codex r1 + r3, 2026-05-15):
    //   - claim slug + URL slugs both populated, mismatch → fire
    //   - claim slug missing AND URL provenance has exactly one slug →
    //     fire UNLESS opts.umbrellaComparisonChat is set AND the LLM has
    //     not populated school_slug on ANY source_used entry. The relaxed
    //     mode is scoped strictly to comparison-mode chat where the schema
    //     marks school_slug advisory. The agentic path + default chat
    //     keep strict trust enforcement (Codex r3 P1: do not weaken the
    //     shared validator outside N3 comparison cases).
    //   - else → fail-open (cross-school URL provenance, or claim that
    //     genuinely refers to no specific school)
    //
    // Provenance lookup is exact-or-prefix (Codex r3 P2): the URL guard
    // above accepts same-host path-prefix citations, so the slug check
    // must too — otherwise a model citing a same-host subpath could
    ...
    if (opts.citationProvenance instanceof Map && opts.citationProvenance.size > 0) {
      ...
      for (const s of parsed.sources_used) {
        ...
        // Exact match first, then same-host path-prefix fallback so the
        // slug check covers the same URL surface as the allowlist guard.
          ...
          try {
            ...
            for (const [provUrl, entry] of opts.citationProvenance.entries()) {
              try {
                ...
                if (isSameOrSub(citedUrl.pathname, pUrl.pathname)) {
                  prov = entry;
                  break;
                }
              } catch { /* skip malformed provenance URL */ }
            }
          } catch { /* cited URL malformed — earlier guard handles */ }
        }
        ...
        if (claimedSlug && !prov.slugs.has(claimedSlug)) {
          ...
          // Strict default fires when LLM omits school_slug on a single-
          // slug-provenance URL. Comparison-chat relaxation: skip the
          // fire only when no source attempted to populate slug at all.
...
export async function runOneQuestion(supabase, slug, question, opts = {}) {
  ...
  let umbrellaProvenance = null;
  let umbrellaComparisonChat = false;
  let umbrellaComparisonSlug = null;
  const extras = [];
  if (umbrellasEnabled()) {
    ...
    if (umb) {
      ...
      umbrellaProvenance = umb.citationProvenance instanceof Map ? umb.citationProvenance : null;
      umbrellaComparisonChat = !!umb.comparisonDetected;
      umbrellaComparisonSlug = umb.comparisonSlug || null;
    }
  }
  const namedSchoolScopeWarning = await buildNamedSchoolScopeWarning(
    supabase,
    slug,
    schoolName,
    question,
    umbrellaComparisonSlug,
  );
  if (namedSchoolScopeWarning) {
    extras.push(namedSchoolScopeWarning);
  }
  if (extras.length) {
  ...
  const validationIssues = parsed
    ? validateAnswer(parsed, retrieval, {
        verbosity,
        citationProvenance: umbrellaProvenance,
        // Codex r4 P1: relaxation applies ONLY to chat surfaces. The
        // non-streaming entrypoint defaults verbosity to 'report' (CLI gold
        // tests + deep-report generator), where the strict missing-slug
        // gate must remain. Gate the flag on verbosity here.
        umbrellaComparisonChat: umbrellaComparisonChat && verbosity === 'chat',
      })
    : [];

  return {
    parsed: withValidationIssues(parsed, validationIssues),
    raw, attempt, parseError, claudeError, validationIssues,
...
export async function* runOneQuestionStream(supabase, slug, question, opts = {}) {
  ...
  //
  // N3 (2026-05-15, Codex r1 P1 + r3 P1): when the parent asked a comparison
  // question we also receive citationProvenance — a Map<source_url, {slugs}>
  // — so validateAnswer can detect a target URL backing a host claim or
  // vice versa. We stash it on retrieval for downstream validation and pass
  // an explicit umbrellaComparisonChat flag so the validator's relaxed
  // "missing school_slug on every source" tolerance applies ONLY here, not
  // to the agentic / single-school paths.
  let umbrellaProvenance = null;
  let umbrellaComparisonChat = false;
  let umbrellaComparisonSlug = null;
  if (umbrellasEnabled()) {
  ...
  const validationIssues = parsed
    ? validateAnswer(parsed, retrieval, {
        ...
        citationProvenance: umbrellaProvenance,
        // Codex r4 P1: relaxation applies ONLY to chat surfaces. The
        // streaming path may be called with verbosity='report' by future
        // callers; gate the relaxation on verbosity to keep agentic/report
        // paths strict.
        umbrellaComparisonChat: umbrellaComparisonChat && verbosity === 'chat',
      })
    : [];
...
9. BALANCED FRAMING. Lead with the strongest fact the parent asked about for
   each named school, but do not omit a genuine concern when the data shows
   one. School A's strong A-levels with a 2023 ISI flag is a different
   recommendation than School B's strong A-levels alone — surface both. Use
   the tradeoff field when the data genuinely supports one. Do NOT
   manufacture negatives to look balanced — if the data shows only positives
   on the asked question, OMIT tradeoff. The rule is: don't suppress real
   concerns the data exposes.

10. THIN-DATA SIGNAL DENSITY. ALWAYS surface metric-coverage gaps even on
    2-school comparisons. If any named school lacks the asked-about metric,
    or publishes it in a different form (e.g. annual fees only when the
    other school publishes per-term), say so explicitly: "Of the four named
    schools, three publish per-term fees; School D's profile lists annual
    fees only." Pattern: "Of the [N] named schools, [M] publish [X];
    [excluded list] [does not / is annual only]." On 2-school comparisons:
    "Both publish [X]" or "Only [School A] publishes [X]; [School B]'s
    profile doesn't include it." ONLY cite N/M when those counts are present
    in the retrieved context — NEVER invent counts. Sounds like an advisor
    reading from records, not a search box.

11. HIGH-STAKES FOLLOW-UPS. For comparison questions about money, safety, or
    fit ("which is better value?", "which is safer for a shy boy?", "which
    suits an ambitious athlete?"), the third you_might_also_ask follow-up
    should be an advisor-style next move the parent can ACT on, not just
    another comparison question. Patterns:
      Money: "Ask both bursars about scholarship deadlines and award sizes
              before committing to a deposit."
      Safety: "Ask each school for the head of pastoral and request to meet
               the duty matron during your tour."
      Fit:    "Ask current parents at each school whether their kids felt
               the pace suited them — same year group ideally."
    The other two follow-ups stay as data-grounded comparison questions.
    Apply this rule only when the question's TOPIC is money/safety/fit — not
    timetables or curriculum.

12. NO SCHOOL SUBSTITUTION. If the parent names a school that is NOT in the
    loaded multi-school comparison set, say explicitly that the named school
    is outside this comparison's loaded scope. Do NOT use another loaded
    school's facts as a proxy for the missing named school, and do NOT relabel
    one school's facts as if they belonged to the missing school. Example:
    if the loaded set is Eton + Harrow and the parent asks "How does Eton
    compare to Winchester on academics?", answer the Eton side from Eton's
    data, then say: "Winchester isn't in this comparison's loaded scope —
    open a Winchester comparison for a like-for-like." Never substitute
    Harrow's figures under the Winchester label.
...
C. ON UNCERTAINTY, NAME THE GAP + REDIRECT. Never just "I don't have
   ...
   aren't in School A's published profile, but the 2023 exam results show…"
...
10. BALANCED FRAMING. Lead with the strongest fact for each named school, but
    do not omit a genuine concern when the data shows one. School A's strong
    A-levels with a 2023 ISI flag is a different recommendation than School B's
    strong A-levels alone — surface both. Use the tradeoff field for concerns
    the data genuinely exposes; do NOT manufacture negatives to look balanced.
    If the data shows only positives on the asked question, leave tradeoff as
    the literal "Nothing to flag here". The rule: don't suppress real concerns.

11. THIN-DATA SIGNAL DENSITY. ALWAYS surface metric-coverage gaps even on
    2-school comparisons. If any named school lacks the asked-about metric,
    or publishes it in a different form, say so INSIDE confirmed_facts: "Of
    the four named schools, three publish per-term fees; School D's profile
    lists annual fees only." Pattern: "Of the [N] named schools, [M] publish
    [X]; [excluded list] [does not / is annual only]." On 2-school comparisons:
    "Both publish [X]" or "Only [School A] publishes [X]; [School B]'s profile
    doesn't include it." ONLY cite N/M when those counts are present in the
    retrieved context — NEVER invent counts. Sounds like an advisor reading
    from records.

12. HIGH-STAKES FOLLOW-UPS. For comparison questions about money, safety, or
    fit, the third follow_ups entry should be an advisor-style next move the
    parent can ACT on, not just another comparison question. Patterns:
      Money: "Ask both bursars about scholarship deadlines and award sizes
              before committing to a deposit."
      Safety: "Ask each school for the head of pastoral and request to meet
               the duty matron during your tour."
      Fit:    "Ask current parents at each school whether their kids felt
               the pace suited them — same year group ideally."
    The other two follow-ups stay as data-grounded comparison questions.
    Apply only when the question's TOPIC is money/safety/fit.

13. NO SCHOOL SUBSTITUTION. If the parent names a school that is NOT in the
    loaded multi-school comparison set, say explicitly that the named school
    is outside this comparison's loaded scope. Do NOT use another loaded
    school's facts as a proxy for the missing school, and do NOT relabel one
    school's facts as if they belonged to the missing school. Example:
    if the loaded set is Eton + Harrow and the parent asks "How does Eton
    compare to Winchester on academics?", answer Eton from Eton's data,
    then say: "Winchester isn't in this comparison's loaded scope — open a
    Winchester comparison for a like-for-like." Never substitute Harrow's
    figures under the Winchester label.
...
export const GLOBAL_SYSTEM_PROMPT = `You are Nana, an AI advisor for parents researching UK independent schools.
...
11. BALANCED FRAMING. Lead with the strongest fact about each named school
    in the answer, but do not omit a genuine concern when the data shows one
    for that school. A recommended school with strong A-levels AND a 2023
    ISI flag is a different recommendation than one with strong A-levels
    alone — surface both. The tradeoff field is the right home for
    cross-school concerns; if a recommended_schools entry has a non-trivial
    risk, also surface it via the entry's "concern" field. Do NOT
    manufacture negatives to look balanced.

12. THIN-DATA SIGNAL DENSITY. When the answer rests on a small subset of
    the 140 schools (e.g. "best value boarding" when only N publish per-term
    fee tables), name the denominator inline: "Based on the 38 schools that
    publish per-term fees, [recommendations] fall in the bottom quartile."
    Pattern: "Based on [N schools that publish X], [observation]." This is
    the most-applicable rule on the global path — most ranking questions
    rest on a partial sample of schools that publish the metric. ONLY cite
    a denominator N when N is present in the retrieved context — NEVER
    invent counts.

13. HIGH-STAKES FOLLOW-UPS. For questions about money, safety, or fit, the
    third follow_ups entry should be an advisor-style next move the parent
    can ACT on, not just another global discovery question. Patterns:
      Money: "Ask each shortlisted school's bursar about scholarship
              deadlines and award sizes before committing."
      Safety: "Ask each school's head of pastoral for the duty-matron rota
               and a tour of the boarding houses."
      Fit:    "Ask current parents at the top 2-3 schools whether the
               pace suited their kids — same year group ideally."
    The other two follow-ups stay as data-grounded questions about the
    recommended schools or the broader topic. Apply this rule only when
    the question's TOPIC is money/safety/fit.

14. NO SCHOOL SUBSTITUTION. If the parent names a school whose data is NOT
    in the retrieved context for this answer, say explicitly that the named
    school is outside this answer's data scope. Do NOT use another school's
    facts as a proxy for the missing named school, and do NOT relabel one
    school's facts as if they belonged to the missing school. Example: if
    the retrieved context covers Eton and Harrow but the parent asks
    "How does Winchester compare to Eton on academics?", answer Eton from
    Eton's data, then say: "Winchester isn't in the retrieved data for this
    answer — open a Winchester-specific question for its breakdown." Never
    substitute Harrow's figures under the Winchester label.
