...
export function projectNotionBackfill(parsed, structured) {
  ...
  // class_size — no SSD overlap; pass through the safe sub-fields only.
  if (parsed.class_size && typeof parsed.class_size === 'object') {
    const cs = {};
    if (isFiniteNum(parsed.class_size.senior))  cs.senior  = parsed.class_size.senior;
    if (isFiniteNum(parsed.class_size.sixth))   cs.sixth   = parsed.class_size.sixth;
    if (isFiniteNum(parsed.class_size.average)) cs.average = parsed.class_size.average;
...
function _notionClassSize(cs) {
  ...
  if (cs.senior != null) return `${cs.senior} (senior)`;
  if (cs.sixth  != null) return `${cs.sixth} (sixth form)`;
  if (cs.average != null) return `~${cs.average} (school average)`;
  return null;
}
