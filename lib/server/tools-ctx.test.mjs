...
// D-step-1 fix (2026-05-26): subject_strengths column must be fetched by
// rankSchools for the subject_strengths dim. Without it, hasRequiredData
// filters every row out and rankSchools returns 0 schools for any subject
// question. Smoke surfaced this; this test prevents regression.
//
// P1.2 (2026-05-26): cols moved into structured-cols.mjs; column-presence
// asserted in tools-cols-contract.test.mjs via colsForRankSchools(). This
// stub stays as a marker that the original concern remains addressed.
test('subject_strengths column is reachable via colsForRankSchools (see tools-cols-contract.test.mjs)', () => {
  const structuredColsSrc = readFileSync(new URL('./structured-cols.mjs', import.meta.url), 'utf8');
  assert.match(structuredColsSrc, /['"]subject_strengths['"]/);
});
